...
*
* $RCSfile: gsn_wpa_authif_priv_patch.h,v $
*
...
GsnWpaAuthIf_AddSta_Patch( GSN_WPA_AUTHIF_T *pAuthIf, const UINT8 *addr );

PUBLIC VOID
GsnWpaAuthIf_RemoveSta_Patch( GSN_WPA_AUTHIF_T *pAuthIf, const UINT8 *addr );

PUBLIC GSN_WPA_STA_INFO_T *
GsnWpaAuthIf_FindSta_Patch( GSN_WPA_AUTHIF_T *pAuthIf, const UINT8 *addr );

PUBLIC void GsnWapAuthIf_AuthConf_Patch( GSN_WPA_AUTHIF_T *pWpaAuthIf,
    GSN_WPAIF_BASIC_CFG_T *pCfg, struct wpa_auth_config *wconf);

PUBLIC VOID
GsnWpaAuthIf_Disconnect_Patch( VOID *pWpaAuthIf,
    UINT8 const*pMac, UINT16 reason );

PUBLIC VOID
GsnWpaAuthIf_MicFail_Patch(void *ctx, const u8 *addr);

PUBLIC VOID
GsnWpaAuthIf_SetEapol_Patch(void *ctx, const u8 *addr,
                       wpa_eapol_variable var, int value);

PUBLIC INT32
GsnWpaAuthIf_GetEapol_Patch(void *ctx, const u8 *addr,
                      wpa_eapol_variable var);

PUBLIC const u8*
GsnWpaAuthIf_GetPsk_Patch(void *ctx, const u8 *addr,
                       const u8 *prev_psk);
PUBLIC INT32
GsnWpaAuthIf_GetMsk_Patch(void *ctx, const u8 *addr, u8 *msk,
                    size_t *len);
PUBLIC int
GsnWpaAuthIf_SetKey_Patch(void *ctx, int vlan_id, enum wpa_alg alg,
                    const u8 *addr, int idx, u8 *key,
                    size_t key_len);
PUBLIC int
GsnWpaAuthIf_GetSeqnum_Patch(void *ctx, const u8 *addr, int idx,
                       u8 *seq);
PUBLIC INT32
GsnWpaAuthIf_SendEapol_Patch(void *ctx, const u8 *addr,
                       const u8 *data, size_t data_len,
                       int encrypt);
PUBLIC INT32
GsnWpaAuthIf_SendEther_Patch(void *ctx, const u8 *addr,u16 proto, const u8 *data,
    size_t data_len );

PUBLIC int
GsnWpaAuthIf_ForEachSta_Patch(
    void *ctx, int (*cb)(struct wpa_state_machine *sm, void *ctx),
    void *cb_ctx);
PUBLIC INT32
GsnWpaAuthIf_IfaceIter_Patch(VOID *iface, void *ctx);
PUBLIC INT32
GsnWpaAuthIf_ForEachAuth_Patch(
    void *ctx, int (*cb)(struct wpa_authenticator *sm, void *ctx),
    void *cb_ctx);
/*PUBLIC INT32
GsnWpaAuthIf_Ft_Iter_Patch(struct hostapd_iface *iface, void *ctx);*/

#ifdef CONFIG_IEEE80211R

PUBLIC INT32
GsnWpaAuthIf_SendFtAction_Patch(void *ctx, const u8 *dst,
                       const u8 *data, size_t data_len);
PUBLIC struct wpa_state_machine *
GsnWpaAuthIf_AddSta_Patch(void *ctx, const u8 *sta_addr);
