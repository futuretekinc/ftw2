...
#include "netx/nx_udp.h"
#ifndef FEATURE_NX_IPV6
#include "netx/nx_ip.h"
#else
#include "netx/nx_ipv4.h"
...
#define NX_BOOTP_PAYLOAD_LENGTH         384     
...
#define NX_DHCP_MIN_RETRANS_TIMEOUT     (nx_dhcp_min_retrans_timeout * _nx_system_ticks_per_second)
#endif
...
#define NX_DHCP_MAX_RETRANS_TIMEOUT     (nx_dhcp_max_retrans_timeout * _nx_system_ticks_per_second)
#endif
...
#define NX_DHCP_MIN_RENEW_TIMEOUT       (nx_dhcp_min_retrans_timeout * _nx_system_ticks_per_second)
#endif
...
#define NX_DHCP_TIMEOUT_DECREMENTS      (1)/*DHCP Client checks its receive queue every 1 second*/
#endif
...
#define NX_DHCP_STATE_IDLE				0xFF	/*GAINSPAN: Put the dhcp thread in indefinite IDLE state*/
...
#define NX_DHCP_LEASE_EXPIRED_IN_SLEEP  0x9E    /* Lease has been expired when the Node is in Sleep*/ 


#define NX_DHCP_RENEWING_AFTER_SLEEP    0x9F    /* Lease has been expired when the Node is in Sleep*/ 
#define NX_DHCP_REBINDING_AFTER_SLEEP   0x10    /* Lease has been expired when the Node is in Sleep*/ 

typedef enum NX_DHCP_CONF_OPTION
{
	NX_DHCP_CONF_OPTION_MAX_RETRIES = 1,
	NX_DHCP_CONF_OPTION_SYSQUAL_TASK_ID,	
	NX_DHCP_CONF_OPTION_HOST_NAME,
	NX_DHCP_CONF_OPTION_REQUEST_LEASE_TIME,
	NX_DHCP_CONF_OPTION_MIN_RETRANS_TIMEOUT,
    NX_DHCP_CONF_OPTION_MAX_RETRANS_TIMEOUT,
}NX_DHCP_CONF_OPTION_T;
...
typedef struct NX_DHCP_STRUCT 
{
    ...
    VOID (*nx_dhcp_state_change_callback)(struct NX_DHCP_STRUCT *dhcp_ptr, UCHAR new_state, void *notifyCtx);

    /* This pointer is reserved for application specific use.  */
    void            *nx_dhcp_reserved_ptr;/*GainSpan: Used for storing the context pointer to returned with 
											nx_dhcp_state_change_callback()*/
	ULONG			nx_dhcp_max_retries;/*Maximum number of retyries Discover/Request before
											declaring DHCP failed*/
	UINT			nx_dhcp_sysQualTaskId;
	UCHAR			nx_dhcp_client_identifier[7];											
} NX_DHCP;
...
UINT        nx_dhcp_state_change_notify(NX_DHCP *dhcp_ptr, VOID (*dhcp_state_change_notify)(NX_DHCP *dhcp_ptr, UCHAR new_state, void *notifyCtx), void *notifyCtx);
...
UINT        nx_dhcp_reinitialize(NX_DHCP *dhcp_ptr,UINT flag);
...
UINT        _nxe_dhcp_state_change_notify(NX_DHCP *dhcp_ptr,  VOID (*dhcp_state_change_notify)(NX_DHCP *dhcp_ptr, UCHAR new_state, void *notifyCtx), void *notifyCtx);
UINT        _nx_dhcp_state_change_notify(NX_DHCP *dhcp_ptr, VOID (*dhcp_state_change_notify)(NX_DHCP *dhcp_ptr, UCHAR new_state, void *notifyCtx), void *notifyCtx);
...
UINT        _nxe_dhcp_reinitialize(NX_DHCP *dhcp_ptr,UINT clearIpAddr);
UINT        _nx_dhcp_reinitialize(NX_DHCP *dhcp_ptr,UINT clearIpAddr);
...
#ifdef NX_DHCP_CLIENT_RESTORE_STATE
UINT        _nx_dhcp_client_create_record(NX_DHCP *dhcp_ptr,NX_DHCP_CLIENT_RECORD *nx_dhcp_client_record);
#endif /* NX_DHCP_CLIENT_RESTORE_STATE */

#define nx_dhcp_conf_set _nx_dhcp_conf_set

UINT  nx_dhcp_conf_set(NX_DHCP *dhcp_ptr, NX_DHCP_CONF_OPTION_T option, VOID *val, UINT len);

UINT  _nx_dhcp_conf_set(NX_DHCP *dhcp_ptr, NX_DHCP_CONF_OPTION_T option, VOID *val, UINT len);
