...
#define NX_ARP_MAXIMUM_RETRIES      2
#endif
...
#define NX_LINK_USER_COMMAND_SET_PHY_ADDR	51
...
/*ARP Record structure used for storing and restoring arp entries accross standby cycles */
typedef struct ARP_REC_STRUCT
{
    ULONG ip_address;
    ULONG msw;
    ULONG lsw;
...
typedef struct NX_TCP_SOCKET_STRUCT
{
    ...
    /* Define NX_BSD_SOCKET_ERROR callback due to retransmission timeout */
    VOID (*nx_tcp_socket_error_callback)(struct NX_TCP_SOCKET_STRUCT *socket_ptr);
...
#endif /* NX_IPSEC_ENABLE */
	UINT ipv6_auto_conf_flag;
    ULONG gsnFlag;
    void *pRpl;
    /**< Handles received data packet. This will either forward the packet if
    it is not meant for him else strip the RPL header. The rest of the header
    processing remains same. */
    int (*hopByhopcb)( struct NX_IP_STRUCT *ip_ptr, void* pRpl, NX_PACKET *pPkt );
    /*< Handles RPL relatated ICMP packets */
    int (*icmpv6cb)( struct NX_IP_STRUCT *ip_ptr, void* pRpl, NX_PACKET *pPkt );
    /*< Handles packet sending procedure. Adds RPL header, finds next hop 
    address and sends to the direver interface. */
    int (*pktSendcb)( struct NX_IP_STRUCT *ip_ptr, void* pRpl, NX_PACKET *pPkt );
    /**< Finds the interface to send the packet. */
    UINT (*ifFindCb)( struct NX_IP_STRUCT *ip_ptr, NXD_ADDRESS *dest_address, 
        NXD_IPV6_ADDRESS **ipv6_addr );

	/*UINT sysQualTskID;*/
} NX_IP;

#define GSN_NETX_FLAG_DISABLE_DAD ( 1 << 0 )
#define GSN_NETX_FLAG_DISABLE_NS_SEND ( 1 << 1 )
#define GSN_NETX_FLAG_DISABLE_RS_SEND ( 1 << 2 )
...
#define nx_tcp_socket_disconnect                _nxe_tcp_socket_disconnect_patch
#ifndef NX_DISABLE_EXTENDED_NOTIFY_SUPPORT
...
UINT nx_arp_table_get(NX_IP *ip_ptr, VOID *buffer, ULONG buffer_size, ULONG *recCount);
UINT nx_arp_table_set(NX_IP *ip_ptr, UCHAR *buffer, ULONG recCount);
#ifdef FEATURE_NX_IPV6
 ...
 ****************************************************************************************************************/
#define IP_PROTOTCP               3     /* Define the option category */
#define TCP_MAXRT                 41    /* Maximum retransmission time */
#define TCP_MAX_REXMIT            42    /* Maximum number of retransmissions without any response from the remote */
#define TCP_REX_TIMER_RATE        43    /* Retransmission timer rate*/
#define TCP_REX_TIME_SHIFT        44    /* Timeout shift between successive retries */
#define TCP_MAX_TX_Q_DEPTH        46  
#define TCP_REX_TIMER_RATE_IN_NW_TICKS  47

#define CONNECT_TIMEOUT_TM        0x40    /* connect() call timeout for tcp client */
#define TCP_OPTION_MAX			CONNECT_TIMEOUT_TM	 
   
VOID  _nx_system_ticks_per_second_update(UINT system_ticks_per_second);

/*
when "auto arp entry" feture is enabled,  arp cache is updated with ne entry based on ARP request information
*/

VOID _nx_arp_auto_arp_entry_enable(ULONG enable);
