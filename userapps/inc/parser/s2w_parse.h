...
UINT8 AppS2wParse_AddrPort(UINT8 **temp, FTW_NETDATA_T *peer_data);
UINT8 AppS2wParse_WepKey(UINT8 *ptr, FTW_WEPDATA_T *wep, UINT8 idx);
UINT8
...
UINT8 AppS2wParse_Wcmd(UINT8 *ptr, FTW_WLANDATA_T *params, enum pwc_command cmd);
UINT8 AppS2wParse_Nset(UINT8 **temp, FTW_NETCONF_T *config);
UINT8 AppS2wParse_Mac(UINT8 *s, UINT8 *mac_addr);

#define is_valid_port(x) ((x) > 0 && (x) <= 65535)

#ifdef FTW_HAVE_STRNCASECMP
#include <strings.h>
#else
