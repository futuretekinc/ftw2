...
APP_RtcInit(UINT8 clearRtcMem)
{
	...
	else if(clearRtcMem == 2)
	{
		...
		/* Reset secondary memory in the case of ext reset. Required if host always do reset */
		memset((INT8 *)GSN_RTC_SRAM_BLOCK0_ADDR, 0, GSN_RTC_SRAM_BLOCK_SIZE);
	}
	
	param.sysStatsLocation = (UINT32)&pAppRtcLtchMemInfo->sysStats;
	param.sysResetInfoRtcLocn = (UINT32)&pAppRtcLtchMemInfo->sysResetInfo;
	param.cryptoSeedLocn = (UINT32)&PAppRtcSecMemInfo->cryptoSeed[0];
