...
typedef enum FTW_L2DEV_FILTER
{
    FTW_L2DEV_SEND_NONE,
    FTW_L2DEV_SEND_STACK,
    FTW_L2DEV_SEND_UART,
    FTW_L2DEV_SEND_BOTH,
}FTW_L2DEV_FILTER_T;
 ...
 ******************************************************************************/
#ifdef FTW_IP2WIFI_SUPPORT
UINT8 s2wRawBufs[FTW_NUM_RAW_BUFS][FTW_RAW_BUFF_SIZE];
#endif
...
{
    FTW_L2_DEV_T *ps2wL2Dev;

    ps2wL2Dev = (FTW_L2_DEV_T *)pL2Dev;
 ...
 ******************************************************************************/
PUBLIC GSN_STATUS
s2wL2Dev_Init(FTW_L2_DEV_T *ps2wL2Dev,
                 char *nwIfName,
                 GSN_L2_DEV_T *pwdd)
{
    ...
    GsnRingBuf_Init( &ps2wL2Dev->s2wRawBufQ, sizeof(UINT32), FTW_NUM_RAW_BUFS+1,
        ps2wL2Dev->s2wRawBufQBuf);
    for (i = 0; i  < FTW_NUM_RAW_BUFS; i++)
    {
        pBuf = s2wRawBufs[i];
...
AppS2wHal_NrawSet(UINT32 mode)
{
    GSN_STATUS retCode = GSN_SUCCESS;
    INT32 i=0, j=0;
    UINT8 *pBuf[FTW_NUM_RAW_BUFS];
    s2wNRAWState = mode;
    if((mode > FTW_NRAW_NONE) && ( !s2wRawBuffAlloc))
    {
        GsnRingBuf_Init( &s2wappMainTaskCtxt->s2wL2Dev.s2wRawBufQ, sizeof(UINT32), FTW_NUM_RAW_BUFS+1,
             &s2wappMainTaskCtxt->s2wL2Dev.s2wRawBufQBuf);
        for (i = 0; i  < FTW_NUM_RAW_BUFS; i++)
        {

            pBuf[i] = gsn_malloc(FTW_RAW_BUFF_SIZE);
            if(pBuf[i] == NULL)
            {
...
{
    UINT8 status = FTW_L2DEV_SEND_NONE;
#ifndef FTW_IP2WIFI_SUPPORT
    UINT16 protocol;
    UINT8 l3protocol;
    UINT16 udpport;
        ...
        case FTW_NRAW_NONE:
            {
                ...
                else
                {
                    ...
                    {
                        status = FTW_L2DEV_SEND_STACK;
                    }
                }
            }
            break;
        case FTW_NRAW_NONSNAP:
            {
                if (protocol == 0x0000)
                {
                    status = FTW_L2DEV_SEND_UART;
                }
                else
                {
                    if ((l3protocol == GSN_NW_UDP_PROTO) &&
 ...
 ******************************************************************************/
PRIVATE VOID
s2wL2Dev_RecvProcess(FTW_L2_DEV_T *ps2wL2Dev, INT32 pktLen, VOID *pFrame)
{
    UINT8 status;
    FTW_MSG_T recvMsg;
    UINT8 *rawbuf;
    ...
    switch (status)
    {
        ...
        case FTW_L2DEV_SEND_UART:
        {
            if (GSN_SUCCESS == GsnRingBuf_Get( &ps2wL2Dev->s2wRawBufQ, &rawbuf))
            {
               ...
               recvMsg.msgType = FTW_RECV_TYPE_RAWDATA;
