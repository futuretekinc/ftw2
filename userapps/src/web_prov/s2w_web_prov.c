...
//#define FTW_SSL_SERVER_TEST
#ifdef FTW_SSL_SERVER_TEST
...
//extern FTW_MIB_PROFILE_T s2wProfileParm;
FTW_MIB_PROFILE_T s2wProfileParm;
	...
	switch(event)
	{
		...
		case START_FILE:
			...
			if(fileInfo.nameLen)
			{
				...
				memset(tagName, 0, FTW_CERT_MAX_NAME_LENGTH+8);
				if(status == F_NO_ERROR || status == F_ERR_DUPLICATED )
				{
					if(strncmp(fileInfo.name,TAG_SSLCERT,fileInfo.nameLen) == 0)
					{
						if(fileInfo.filenameLen > FTW_CERT_MAX_NAME_LENGTH)
							fileInfo.filenameLen = FTW_CERT_MAX_NAME_LENGTH-1;
		...
		case END_FILE:
		{
			...
			if(strcmp((const char *)tagName,FTW_CERT_NAME_CA) == 0)
			{
...
S2wWebProv_WpsCbFn(char* mode, char* pin)
{
	...
	GsnTaskSleep(500);
	if(FTW_WEB_PARAMS_STORE == 	storeParams || FTW_WEB_PARAMS_SND_AND_STORE == storeParams)
...
#define FTW_HTTPD_PKT_POOL  (32*1024)
PUBLIC VOID
...
WebProvUriRegister:
  ...
  //  provConfig.pWdd = &(s2wappMainTaskCtxt->wdd);
   ...
   // tfHttpdUserSendFile (1,"/html.html",0);
    ...
    GsnHttpdApi_FwInit(wlanVer, FTW_SW_VERSION);//FTW_BUILD_VERSION);
...
#ifdef FTW_EXT_FLASH_FWUP_PUSH_METHOD
#ifdef ADK_OTAFU
    ...
    switch (event)
	{
		case TM_HTTPD_EVENT_ALL_RECV:
		{
			...
			certName = AppS2w_CertNameGet((UINT8 *)uri);
				if(certName != NULL)
					s2wappMainTaskCtxt->fileOperationStatus = FTW_CERT_delete(certName);			
			if(strcmp((const char*)certName,FTW_CERT_NAME_CA) == 0) 			
			{
        ...
        case TM_HTTPD_EVENT_SEND_READY:
		{
            ...
            {
            	if(s2wappMainTaskCtxt->fileOperationStatus == FTW_SUCCESS)
											...
											ttHttpdUserConHandle conHandle)
{
    ...
    switch (event)
    {
		...
		{
        	if (GsnHttpd_GetHttpMethod(conHandle) == TM_HTTP_METHOD_GET)
               	bufferLen = sprintf(buffer,"<time>%lld</time>",FTW_SYS_TIME_get(NULL));	
			else		
				bufferLen = sprintf(buffer, "<status>ok</status>"); 		
    	    GsnHttpd_SendBufferAndLastChunk(conHandle, buffer, bufferLen, TM_BLOCKING_ON, &errorCode);			
		}
...
/*.................................................*/
#define FTW_WEB_EAPAUTH_TBL_ENTRY	6
/*Outer type, inner type*/
UINT32 eapAuthTable[FTW_WEB_EAPAUTH_TBL_ENTRY][3]=
...
S2wWeb_EapAuthTypeGet(UINT32 eapType, UINT32 *outrAuth, UINT32 *innerAuth)
{
	...
	for(i = 0; i < FTW_WEB_EAPAUTH_TBL_ENTRY; i++)
		{
...
/*.................................................*/
#define FTW_WEB_EAPAUTH_TBL_ENTRY	7
/*Outer type, inner type*/
UINT32 eapAuthTable[FTW_WEB_EAPAUTH_TBL_ENTRY][2]=
{
	{43, 26,},	/*  EAP- FAST */
	{21, 26},	/*EAP-TTLS*/
	{13, 26},	/*EAP- TLS*/
...
{
	UINT8 i;
	UINT32 retVal = FTW_WEB_EAPAUTH_TBL_ENTRY;
	for(i = 0; i < FTW_WEB_EAPAUTH_TBL_ENTRY; i++)
		{
...
{
	if(eapType < FTW_WEB_EAPAUTH_TBL_ENTRY)
	{
...
{
    UINT32 ipAddr =0;
    FTW_FLASH_PARAMS_T flashParams;
    ...
    memset(&flashParams, 0, sizeof(FTW_FLASH_PARAMS_T));    
...
/*wep key*/
	    ...
	    else if (pConfig->networkConfig.clientConfig.securityType == GSN_WLAN_WPA_PERSONAL)
    	{
			...
			memset((VOID *)pS2wProfile->security.wpa.wpaPassPhrase,0,FTW_PSK_PASSPHRASE_LEN_MAX);
			memset((VOID *)pS2wProfile->security.wpa.ssid,0,FTW_MAX_SSID_LEN);
	    ...
	    else if (pConfig->networkConfig.clientConfig.securityType == GSN_WLAN_WPA_ENTERPRISE)
	    {
		    ...
		    pS2wProfile->security.eap.passwordLen = pConfig->networkConfig.clientConfig.eapPassword.Length;
             ...
             memcpy(pS2wProfile->security.eap.password, pConfig->networkConfig.clientConfig.eapPassword.Pwd,
              ...
              pS2wProfile->security.validKeys |= FTW_SECURITY_VALID_EAP; 	 			 
...
/*L2 configurations*/
    ...
    if(FTW_WEB_PARAMS_SND_TO_HOST == storeParams || FTW_WEB_PARAMS_SND_AND_STORE == storeParams)
    {
        /* Page 2 values - Basic WiFi*/
	S2w_Printf("SSID=");
	FTW_SERIAL_put(FTW_SERIAL_0, pConfig->networkConfig.clientConfig.ssid.array,pConfig->networkConfig.clientConfig.ssid.length);
        S2w_Printf("\r\nCHNL=%d\r\nCONN_TYPE=%d\r\nMODE=%d\r\nSECURITY=%d\r\n",
        ...
        if (pConfig->networkConfig.clientConfig.securityType == GSN_WLAN_WEP)
        {
		 ...
		 FTW_SERIAL_put(FTW_SERIAL_0, pConfig->networkConfig.clientConfig.passphrase.Passphrase,
							pConfig->networkConfig.clientConfig.passphrase.Len);
		 S2w_Printf("\r\n");
             }
	     else if (pConfig->networkConfig.clientConfig.securityType == GSN_WLAN_WPA_ENTERPRISE)
		      ...
		      FTW_SERIAL_put(FTW_SERIAL_0, pConfig->networkConfig.clientConfig.eapUsername.Name,
			pConfig->networkConfig.clientConfig.eapUsername.Length);
		
		      S2w_Printf("\r\nEAP_PASSWORD=");
		      FTW_SERIAL_put(FTW_SERIAL_0, pConfig->networkConfig.clientConfig.eapPassword.Pwd,
                             pConfig->networkConfig.clientConfig.eapPassword.Length);
	...
	{
	S2w_Printf("AP-SSID=");
	FTW_SERIAL_put(FTW_SERIAL_0, pConfig->networkConfig.apConfig.ssid.array,pConfig->networkConfig.apConfig.ssid.length);
    S2w_Printf("\r\nAP-CHNL=%d\r\nAP-BEACON-INTRL=%d\r\nAP-SECURITY=%d\r\n",
         ...
         if (pConfig->networkConfig.apConfig.securityType == GSN_WLAN_WPA_PERSONAL)
         {
			...
			FTW_SERIAL_put(FTW_SERIAL_0, pConfig->networkConfig.apConfig.passphrase.Passphrase,
							  pConfig->networkConfig.apConfig.passphrase.Len);
			S2w_Printf("\r\n");
         }
         else if (pConfig->networkConfig.apConfig.securityType == GSN_WLAN_WEP)
	...
	if(s2wCurrent.mode == FTW_WLANDATA_MODE_PROV_AP)
		...
		memcpy(&flashParams.profile[0], pS2wProfile, sizeof(FTW_PROFILE_T));
...
S2wConfig_GetValues(GSN_SYS_CONFIG_T *pConfig)
{
	...
	pConfig->networkConfig.apConfig.securityType  = GSN_WLAN_SECURITY_NONE;
		...
		if ((pWlanConfig->securityCfg.mode== FTW_AUTH_MODE_WPA2) \
			|| (pWlanConfig->securityCfg.mode  == FTW_AUTH_MODE_WPA) )
		{	
			pConfig->networkConfig.apConfig.securityType =GSN_WLAN_WPA_PERSONAL;
		...
		else if (pWlanConfig->securityCfg.mode   == FTW_AUTH_MODE_WEP)
		...
		else if(pWlanConfig->securityCfg.mode   == FTW_AUTH_MODE_WPA2_ENTERPICE)
	    ...
	    for(i = 0; i< pWlanConfig->securityCfg.wepConfig.KeyLen; i++)
    {
		...
		else if(pWlanConfig->securityCfg.mode	 == FTW_AUTH_MODE_WPA_ENTERPICE ||
			    pWlanConfig->securityCfg.mode	 == FTW_AUTH_MODE_WPA2_ENTERPICE   )
    	{
...
#ifdef FTW_WEB_SRVR_STACK_STATIC
...
{
#ifndef FTW_WEB_SRVR_STACK_STATIC
	gsn_free(stackPtr);
#endif
}
#endif
