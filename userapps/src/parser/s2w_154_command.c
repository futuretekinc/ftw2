...
/*****************************************************************************
  *  Public Variables
  *****************************************************************************/
#define FTW_154_ASSOC_STAT_FREE       0
#define FTW_154_ASSOC_STAT_NOT_ASSOC  1
#define FTW_154_ASSOC_STAT_ASSOC      2
#define FTW_154_ASSOC_STAT_DONT_ASSOC 3

#define FTW_154_MAX_ASSOC_LIST   8
#define GSN_WLP_MAX_SEC_ENTRIES 10

UINT8 semRelease = FALSE;      
      
typedef struct FTW_154_ASSOC_LIST
{
    UINT16 shortAddr;
    UINT8 extendedAddr[8];
    UINT8  status;
}FTW_154_ASSOC_LIST_T;
typedef struct FTW_154
{
    UINT8 devType;
    UINT8 assocState;
    UINT8 operationStatus;
    GSN_OSAL_SEM_T blockingSem;
    FTW_154_ASSOC_LIST_T assocChild[ FTW_154_MAX_ASSOC_LIST ];
...
AppS2wCmd_154Channel(UINT8 *ptr)
{
    ...
    */
    if(ptr[0] == '?' && ( ptr[1] == '\0'))
    {
        if( FTW_154_ASSOC_STAT_ASSOC != s2w154Ctx.assocState )
        {
            return FTW_EINVAL;
        }
        else
...
AppS2wParse_Mac64(UINT8 *s, UINT8 *mac_addr)
{
    ...
    return i == 8 ? FTW_SUCCESS : FTW_EINVAL;
}
PUBLIC UINT8
AppS2wParse_LookupData( UINT8 *s, UINT8 *data, UINT8 length)
{
    ...
    return i == length ? FTW_SUCCESS : FTW_EINVAL;
}
PUBLIC UINT8
AppS2wParse_Key( UINT8 *s, UINT8 *data )
{
    ...
    return ((i == 16) ? FTW_SUCCESS : FTW_EINVAL);
}
...
AppS2wParse_SecI( UINT8 *ptr, GSN_WLRPAN_SEC_I_T *pSecI )
{
	...
	if(!p)      return FTW_EINVAL;

	status = AppS2wParse_Hex(p, &param);
	if( ( FTW_SUCCESS != status )||( param > 7 ) ) 
		return FTW_EINVAL;
...
AppS2wCmd_154ExtendedAddr(UINT8 *ptr)
{
    ...
    */    
    ...
    else
    {
        ...
        if( FTW_SUCCESS != AppS2wParse_Mac64( ptr, setReq.Value )) 
            return FTW_EINVAL;
...
AppS2wCmd_154RetryCount(UINT8 *ptr)
{
    ...
    */
    ...
    else
    {
        ...
        {
            GSN_WLRPAN_SET_REQ_T setReq;
            if( FTW_SUCCESS != AppS2wParse_Hex(p, &param)) return FTW_EINVAL;
...
AppS2wCmd_154SupreFrmCfg(UINT8 *ptr)
{
    ...
    */
    ...
    else
    {
        ...
        if( p != NULL && ( p[0] != '\0') )
        {           
            if( FTW_SUCCESS != AppS2wParse_Hex(p, &param1))return FTW_EINVAL;
            s = TRUE;
            p = AppS2wParse_NextParamGet(&ptr);
            if( p != NULL && ( p[0] != '\0'))
            {
                if( FTW_SUCCESS != AppS2wParse_Hex(p, &param2) ) 
                    return FTW_EINVAL;
                b = TRUE;
            }
        }
...
AppS2wCmd_154ScanNetwork(UINT8 *ptr)
{
    ...
    */
    ...
    status = AppS2wParse_Hex(p, &param);
    if( status != FTW_SUCCESS || param > 0x3 )
    {
        return FTW_EINVAL;
    }    
    ...
    {
        status = AppS2wParse_Hex(p, &param);
        if(( status != FTW_SUCCESS ) || ( param > 0xE ))
        {
            return FTW_EINVAL;
        }
...
AppS2wCmd_154JoinNetwork(UINT8 *ptr)
{
    ...
    */
    ...
    status = AppS2wParse_Hex(p, &param);
    if( status != FTW_SUCCESS || param < 0xc || param > 0x17 )
    {
        return FTW_EINVAL;
    }
    ...
    if((p) && (*p != '\0'))
    {
        ...
        if( status != FTW_SUCCESS )
        {
            if( FTW_SUCCESS != AppS2wParse_Mac64( p, (UINT8*)addr ))
                return FTW_EINVAL;
            else
            {
    ...
    {
        status = AppS2wParse_Hex(p, &param);
        if( status != FTW_SUCCESS )return FTW_EINVAL;
        else
    ...
    if( s2w154Ctx.operationStatus == FTW_SUCCESS )
    {
...
AppS2wCmd_154AddNode(UINT8 *ptr)
{
    ...
    */
    UINT8 status, i, freeIdx = FTW_154_MAX_ASSOC_LIST;
    UINT32 eaddr[2], param;
    UINT16 saddr;
    UINT8 *p = AppS2wParse_NextParamGet(&ptr);
    if(!p)return FTW_EINVAL;
        if( FTW_SUCCESS != AppS2wParse_Mac64( p, (UINT8*)eaddr ))
            return FTW_EINVAL;
    
    p = AppS2wParse_NextParamGet(&ptr);
    if(!p)return FTW_EINVAL;
    status = AppS2wParse_Hex(p, &param);
    if ( status != FTW_SUCCESS || param > 0xFFFE )
        return FTW_EINVAL;
    
    saddr = ( UINT16 )param;
    
    /**< Check if we have this address listed */
    for( i = 0; i < FTW_154_MAX_ASSOC_LIST; i++ )
    {
        if( s2w154Ctx.assocChild[ i ].status == FTW_154_ASSOC_STAT_FREE )
        {
            freeIdx = i;
            break;
        }
        ...
        {
            /**< It is present */
            if( s2w154Ctx.assocChild[ i ].status == FTW_154_ASSOC_STAT_ASSOC )
            {
                return FTW_EINVAL;
            }
            else
...
AppS2wCmd_154StartNetwork(UINT8 *ptr)
{
    ...
    */
    ...
    status = AppS2wParse_Hex(p, &param);
    if( status != FTW_SUCCESS ||  (param < 0x0c) || (param > 0x17) )
        return FTW_EINVAL;
    ...
    status = AppS2wParse_Hex(p, &param);
    if ( status != FTW_SUCCESS || param > 0xF )
    {
        return FTW_EINVAL;
    }    
    ...
    status = AppS2wParse_Hex(p, &param);
    if ( status != FTW_SUCCESS || param > 0x1 )
    {
        return FTW_EINVAL;
    }    
...
AppS2wCmd_154LeaveNetwork(UINT8 *ptr)
{
    ...
    */
    ...
    status = AppS2wParse_Hex(p, &param);
    if( (status != FTW_SUCCESS) || (param < 0x01) || (param > 0x02) )
    {
        return FTW_EINVAL;
    }    
    ...
    status = AppS2wParse_Hex(p, &param);
	if( status != FTW_SUCCESS || param > 0x01 )
    {
        return FTW_EINVAL;
    } 
...
AppS2wCmd_154DataSend(UINT8 *ptr)
{
    ...
    */
    ...
    status = AppS2wParse_Hex(p, &param);
    if ( status != FTW_SUCCESS || param > 0xFF )
    {
        return FTW_EINVAL;
    }    
    ...
    status = AppS2wParse_Hex(p, &param);
    if ( status != FTW_SUCCESS || param > 0x7 )
    {
        return FTW_EINVAL;
    }    
    ...
    status = AppS2wParse_Hex(p, &param);
    if( status != FTW_SUCCESS || param > 127 )
    {
        return FTW_EINVAL;
    }    
...
AppS2wCmd_154RxEnable(UINT8 *ptr)
{
    ...
    {
        status = AppS2wParse_Hex(p, &param);
        if( ( status != FTW_SUCCESS )||( param > 0x00FFFFFF ) )
        {
            return FTW_EINVAL;
        }
...
AppS2wCmd_154OrphanRsp(UINT8 *ptr)
{
    ...
    if ((p) && (*p != '\0'))
    {
        ...
        if( FTW_SUCCESS != AppS2wParse_Mac64( ptr, (UINT8*)addr ))
                return FTW_EINVAL;
        else
...
AppS2wCmd_154GTSReq(UINT8 *ptr)
{
    ...
    {
        status = AppS2wParse_Hex(p, &param);
        if (( status != FTW_SUCCESS ) || ( param == 0 ) || ( param > 7 ) )
        {
            return FTW_EINVAL;
        }
...
UINT8 AppS2wCmd_154TmrReq(UINT8 *ptr)
{
    ...
    status = AppS2wParse_Hex(p, &param);
    if( status != FTW_SUCCESS || param > 0xFFFFFF )
    {
        return FTW_EINVAL;
    } 
...
AppS2wCmd_154SetReq(UINT8 *ptr)
{
    ...
    if( ( status != FTW_SUCCESS ) || ( param > macKey ) )
    {
        return FTW_EINVAL;
    }    
    ...
    switch( setReq.Attribute )
    {
        ...
        case macAutoReqSeurityLevel:
			status = AppS2wParse_Hex( p, &param );
			if( ( status != FTW_SUCCESS ) || ( param > 7 ) )
				return FTW_EINVAL;
			setReq.Value[0] = param;
			break;
        case macAutoReqKeyIdMode:
			status = AppS2wParse_Hex( p, &param );
			if( ( status != FTW_SUCCESS ) || ( param > 3 ) )
				return FTW_EINVAL;
			setReq.Value[0] = param;
			break;
		case macAutoReqKeyIndex:
			status = AppS2wParse_Hex( p, &param );
			if( ( status != FTW_SUCCESS ) || ( param == 0 ) )
				return FTW_EINVAL;
			setReq.Value[0] = param;
			break;
		...
		case macKeyUsageListEntries:
            status = AppS2wParse_Hex( p, &param );
			if( ( status != FTW_SUCCESS )||( param > GSN_WLP_MAX_SEC_ENTRIES ) )
				return FTW_EINVAL;
			setReq.Value[0] = param;
			break;
		...
		case macPANCoordExtendedAddress:
            if( FTW_SUCCESS != AppS2wParse_Mac64( p, (UINT8*)setReq.Value ))
                return FTW_EINVAL;
            break;
        ...
        default:
            s2w154Ctx.operationStatus = FTW_FAILURE;
            return FTW_EINVAL;
...
AppS2w_WLRPANAssocCfm( VOID* ctx, UINT32 magId, UINT16 msgLen, VOID *pMsg )
{
    ...
    s2w154Ctx.operationStatus = FTW_SUCCESS;
    if(pCfm->Status == GSN_SUCCESS)
    {
        s2w154Ctx.assocState = FTW_154_ASSOC_STAT_ASSOC;
    }
...
AppS2w_WLRPANDisassocInd(VOID* ctx, UINT32 magId, UINT16 msgLen, VOID *pMsg)
{
	s2w154Ctx.assocState = FTW_154_ASSOC_STAT_NOT_ASSOC; 
