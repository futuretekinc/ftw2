 ...
 ******************************************************************/
PUBLIC const FTW_EXTFLASH_CMD_T s2wExtFlashCmdTable[] =
{
...
UINT8 AppS2wCmd_readFile(UINT8 *ptr)
{
    ...
    do
    {
		...
		if(readBytes > 0)
		{
//		    S2w_Printf("%c%c%d%04d", FTW_ESC, 'F', fileInfo.fd, readBytes);
			FTW_SERIAL_put(FTW_SERIAL_0, s2wappMainTaskCtxt->appExtFsCtx.extDataBuffFs, readBytes);
		}
    }while(readBytes > 0);
...
AppS2wHal_FileWriteDataGet(F_FILE *fd, UINT32 dataLen)
{
    ...
    while(dataLen)
    {
        ...
        FTW_SERIAL_get(FTW_SERIAL_0, s2wappMainTaskCtxt->appExtFsCtx.extDataBuffFs,writeLen);
		/* write in to file */
