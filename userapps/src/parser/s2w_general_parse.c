 ...
 * @return FTW_SUCCESS on success, FTW_EINVAL if the parameter is invalid.
 ************************************************************************/
PUBLIC UINT8
 ...
 * @return FTW_SUCCESS on success, FTW_EINVAL on failure.
 ************************************************************************/
PUBLIC UINT8
 ...
 *
 * @return FTW_SUCCESS on success, FTW_EINVAL if the address is invalid.
 ************************************************************************/
PUBLIC UINT8
AppS2wParse_Mac(UINT8 *s, UINT8 *mac_addr)
{
   ...
   // UINT8 dummy1[6]={0xFF,0xFF,0xFF,0xFF,0xFF,0xFF}; // broadcast mac
    memset(mac_addr, 0, FTW_MAC_ADDR_SIZE);
...
//        return FTW_EINVAL;
//    }
    return i == 6 ? FTW_SUCCESS : FTW_EINVAL;
}
 ...
 ************************************************************************/
PUBLIC UINT8
AppS2wParse_AddrPort(UINT8 **temp, FTW_NETDATA_T *peer_data)
{
    UINT8 *p;
    UINT32 port;//,len;
    UINT8 status;
...
AppS2wParse_Ssid(UINT8 *p, UINT8 *ssid, UINT8 *lenp)
{
    ...
    memset(ssid, 0, FTW_MAX_SSID_LEN);
 ...
 ************************************************************************/
PUBLIC UINT8
AppS2wParse_WepKey(UINT8 *ptr, FTW_WEPDATA_T *wep, UINT8 idx)
{
    UINT8 temp[13];
    UINT32 len;

    FTW_ASSERT(idx >= 1 && idx <= 4);
 ...
 ************************************************************************/
PUBLIC UINT8
AppS2wParse_Wcmd(UINT8 *ptr, FTW_WLANDATA_T *params, enum pwc_command cmd)
{
    UINT8 *pt = ptr;
    UINT8 *temp = ptr;
    UINT32 tempint;
    ...
    if (!pt)
    {
        ...
        //return (cmd == pwc_wauto) ? FTW_SUCCESS/*FTW_EINVAL*/ : FTW_SUCCESS;
    }
    ...
    switch(domain)
    {
     ...
     if (tempint && (tempint < FTW_WLANDATA_CHANNEL_MIN
                || tempint > chMax))
    {
        return FTW_EINVAL;
    }
    ...
    if (cmd == pwc_scan)
    {
        ...
        {
            status = AppS2wParse_Int(pt, &tempint);
            if (status != FTW_SUCCESS || tempint > 10000)// maximum 10 seconds
            {
                return FTW_EINVAL;
            }
	...
	{	/* optional argument for wa cmd*/
		if (cmd == pwc_assoc)
		{
			...
			if(AppS2wParse_Boolean(pt,&globalUseHRssi) != FTW_SUCCESS)
			    return FTW_EINVAL;
		}
	}
    return FTW_SUCCESS;
 ...
 ************************************************************************/
PUBLIC UINT8
AppS2wParse_Nset(UINT8 **temp, FTW_NETCONF_T *config)
{
    UINT8 status;
    UINT8 *p;
 ...
 ************************************************************************/
PUBLIC UINT8
AppS2wParse_Fwup(UINT8 *temp, FTW_FWUP_CONF_T* params)
{
    UINT8 status;
    UINT8 *p;
