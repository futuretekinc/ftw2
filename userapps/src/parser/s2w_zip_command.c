...
static const UINT8 FTW_DEFAULT_154_MAC_ADDR[8] =
...
const GSN_ZIP_CONST_CFG_T zipConstCfg = 
{
    ...
    .maxNbE = FTW_MAX_NUM_OF_NEIGHBOR_SUPPORTED,
    .maxParents = FTW_MAX_NUM_OF_PARENT_SUPPORTED,
    .maxRtE = FTW_MAX_NUM_OF_NODE_SUPPORTED   
};
...
VOID App_ZipInit(APP_MAIN_CTX_T *pAppCtx, APP_IF_INFO_T *pIfInfo)
{
    ...
    if(otpMac_Read(( UINT8* )zipCfg.macAdd, GSN_OTP_MM_ID_MAC_ID2 ) != FTW_SUCCESS )
    {
        memcpy(( UINT8* )zipCfg.macAdd, FTW_DEFAULT_154_MAC_ADDR, 
...
UINT8 AppS2wCmd_ZipScan( UINT8 *ptr )
{
    ...
    if( pScanBuf == NULL )return FTW_EINVAL;
