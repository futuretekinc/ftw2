...
FTW_UNSOLICITED_FRAME_T frame;
#endif

#if 0
PRIVATE const char *FTW_ErrList[] =
{
    "NO ERROR",                         /* FTW_ERR_NOERROR */
    "ERR:SSID NOT FOUND",                /* FTW_ERR_SSID_NOT_FOUND */
    "ERR:AUTH FAIL:PSK INVALID",         /* FTW_ERR_PSK_INVALID */
    "ERR:AUTH FAIL:TIMEOUT",             /* FTW_ERR_AUTH_TIMEOUT */
    "ERR:AUTH FAIL:EAP_FAST_FAILURE",     /* FTW_ERR_AUTH_EAP_FAST_FAILURE */
    "ERR:AUTH FAIL:EAP_CRED_FAILURE",     /* FTW_ERR_AUTH_EAP_CRED_FAILURE */
};
#endif
...
void                    *s2wHttpdPktPtr;
#define FTW_HTTPD_PKT_POOL  (10*1024)
...
AppS2wHelp_Print(const char *cmd, const char *desc)
{
    ...
    FTW_ASSERT(desclen <= left);

    FTW_SERIAL_put(FTW_SERIAL_0, "\r\n", 2);
    FTW_SERIAL_put(FTW_SERIAL_0, cmd ? cmd : " ", len);
    ...
    FTW_SERIAL_put(FTW_SERIAL_0, desc, desclen);
}
...
#if defined(FTW_SECURITY_ENTERPRISE_TLS_AVAILABLE) || defined(FTW_SECURITY_ENTERPRISE_FAST_MSCHAPV2_AVAILABLE) || defined(FTW__SECURITY_ENTERPRISE_PEAP_V0_AVAILABLE)|| defined(FTW__SECURITY_ENTERPRISE_TTLS_AVAILABLE) || defined(FTW_SECURITY_ENTERPRISE_FAST_GTC_AVAILABLE)|| defined(FTW__SECURITY_ENTERPRISE_PEAP_V1_AVAILABLE)
...
#endif
#ifdef FTW_WEB_SRVR_LOGO_INTRNL_FLASH
...
#endif    
    status = FTW_PS_pollIntervelSet(interval);
    return status;
}
...
{
    GsnIoMux_Sel(APP_FTW_ASYNC_GPIO, APP_FTW_ASYNC_GPIO_MUX);
    GsnIoMux_PullDownEnable (APP_FTW_ASYNC_GPIO);
    /* Initialise as Output */
    GsnGpio_Init(APP_FTW_ASYNC_GPIO_BITMAP, ~APP_FTW_ASYNC_GPIO_BITMAP);
...
VOID S2w_AsyncMsgGpioOp(UINT8 flag)
{
    if(flag)
    {
        GsnGpio_OutIndvidualSet(APP_FTW_ASYNC_GPIO_BITMAP);
    }
    else
    {
        GsnGpio_OutClear(APP_FTW_ASYNC_GPIO_BITMAP);
    }
}
...
AppS2wHal_FileWriteDataGet(INT32 fd, UINT32 dataLen)
{
	...
	dataBuff = (UINT8 *)gsn_malloc(FTW_WR_BUFF_SIZE);
    if(NULL ==  dataBuff)
    {
        status = FTW_FAILURE;
    ...
    while(dataLen)
    {
        writeLen = dataLen >= FTW_WR_BUFF_SIZE? FTW_WR_BUFF_SIZE:dataLen;

        FTW_SERIAL_get(FTW_SERIAL_0, dataBuff,writeLen);
		/* write in to file */
...
UINT8 FTW_NET_HTTPD_rawContentGet(UINT32 dataLen, UINT8 hCid)
{
    ...
    while(dataLen)
    {
        ...
        FTW_SERIAL_get(FTW_SERIAL_0, (UINT8*)s2wCidList[hCid].httpdCtx.httpdBuffer,s2wCidList[hCid].httpdCtx.httpdBufferLenUsed);
#else
        FTW_SERIAL_get(FTW_SERIAL_1, (UINT8*)s2wCidList[hCid].httpdCtx.httpdBuffer,s2wCidList[hCid].httpdCtx.httpdBufferLenUsed);
#endif
		...
		if(s2wappMainTaskCtxt->gsLinkSendStatus  == FTW_FAILURE)
		{
			status = FTW_FAILURE;
			break;
		}
...
#ifdef FTW_HTTP_CUSTOM_SUPPORT
VOID
AppS2w_HttpStatusLineSet(FTW_HTTPD_CONTEXT_T *httpdCtx,UINT8 statusline)
{
    if(httpdCtx != NULL)
    {
...
VOID
AppS2w_HttpHdrCountSet(FTW_HTTPD_CONTEXT_T *httpdCtx,UINT32 headercount)
{
    if(httpdCtx != NULL)
    {
                     ...
                     INT32* rxBufferOffset)
{
	...
	if(s2wCidList[cid].httpdCtx.methodLineReceived==0)
	{
		...
		headerLen = sprintf(startMark,"%c%c%x%04d", FTW_ESC, 'H', cid,statusCodeLen);
#ifndef FTW_DUAL_INTERFACE_SUPPORT
		FTW_SERIAL_put(FTW_SERIAL_0, startMark, headerLen);
		FTW_SERIAL_put(FTW_SERIAL_0, httpRxDataBuf,statusCodeLen);
#else
		FTW_SERIAL_put(FTW_SERIAL_1, startMark, headerLen);
		FTW_SERIAL_put(FTW_SERIAL_1, httpRxDataBuf,statusCodeLen);
#endif
#ifndef FTW_DUAL_INTERFACE_SUPPORT
	...
	{
		*rxBufferOffset = s2wCidList[cid].xmlParserCtx->bytesTodecode;// FTW_SUCCESS;
	}
...
UINT8
AppS2w_HttpAddHdr(FTW_HTTPD_CONTEXT_T *httpdCtx, ttHttpUserHeaderType httphdrtype, UINT8  *httphdrvalptr)
{
    UINT32   i;
...
UINT8 AppS2w_HttpAddStaticHdr(FTW_HTTPD_CONTEXT_T *httpdCtx, UINT32 totalhdrcount)
{
...
AppS2w_HttpDataProcess(UINT8 cid,UINT8 *dataBuffer,UINT32 inBuflen, UINT32 elementType)
{
    ...
    {
        if(FTW_HTTP_RESPONSE_LINE == elementType)
        {
        ...
        if(FTW_HTTP_HEADER == elementType)
        {
...
AppS2wCmd_RfTestTx99Start(UINT8 *ptr)
{
	...
	UINT8 MacAddrBssid[FTW_MAC_ADDR_SIZE];
    
    UINT8 MacAddrDest[FTW_MAC_ADDR_SIZE];    
...
AppS2wCmd_WebServer(UINT8* ptr)
{
	...
	memset(&s2wappMainTaskCtxt->WebServerCfg,0,sizeof(FTW_WBSERVER_CONFIG_T));
	p = AppS2wParse_NextParamGet(&ptr);
    if (!p)
	{
	   return FTW_EINVAL;
	...
	p = AppS2wParse_NextParamGet(&ptr);
    if (!p || (strlen((const char *)p) > FTW_HTTPD_MAX_USERNAME_LEN))
    {
       	return FTW_EINVAL;
    }
    ...
    p = AppS2wParse_NextParamGet(&ptr);
    if (!p || (strlen((const char *)p) > FTW_HTTPD_MAX_PASSWORD_LEN) )
    {
        return FTW_EINVAL;
    }
    ...
    s2wappMainTaskCtxt->WebServerCfg.sslEnabled = FTW_DFLT_HTTP_SRVR_SSL_ENABLE;
	...
	p = AppS2wParse_NextParamGet(&ptr);
#if (defined(FTW_GSLINK_XML) || defined(FTW_GSLINK_RAW))
		s2wappMainTaskCtxt->respTimeOut = 0;
		s2wappMainTaskCtxt->responseInprogress=0;
		if (p && 0 != strlen ((INT8 *)p))
		{
...
AppS2w_WebServerStart()
{
    ...
    if( s2wappMainTaskCtxt->WebServerCfg.sslEnabled )
    {
        ...
        if(pxCert == NULL)
        {
			s2wappMainTaskCtxt->WebServerCfg.status = FTW_FAILURE;
        ...
        certDetail.ClientCertName = (const char *)pxCert->pName;
        ...
        certDetail.PrivateKeyName = (const char *)pxCert->pName;
    ...
    config.maxConnTotal = FTW_DFLT_HTTP_SRVR_CLIENT_NO;//APP_HTTPD_MAX_TOTAL_CONN;
    ...
    s2wHttpdPktPtr = (void*)gsn_malloc(FTW_HTTPD_PKT_POOL);
    if(NULL == s2wHttpdPktPtr)
    {
		s2wappMainTaskCtxt->WebServerCfg.status = FTW_FAILURE;
		goto webServerExit;
    ...
    ret = nx_packet_pool_create(&trPktPoolHndl, "", 1600,
             s2wHttpdPktPtr, FTW_HTTPD_PKT_POOL);
    if(ret)
    {
		s2wappMainTaskCtxt->WebServerCfg.status = FTW_FAILURE;
		goto webServerExit;
...
#endif
	s2wappMainTaskCtxt->WebServerCfg.status = FTW_SUCCESS;
webServerExit:
...
APPS2wCmd_WebProv(UINT8 *ptr)
{
	...
	if (p && 0 != strlen((INT8*)p))
	{
		...
		if(( s2wappMainTaskCtxt->WebServerCfg.storeParams  == FTW_WEB_PARAMS_SND_TO_HOST)	&& val == 1 )		
		{
			 return FTW_FAILURE;			 
		}
#ifdef  FTW_NCM_SUPPORT_ENABLE
...
extern FTW_MDNS_HN_PARAM_T mdnsHnParam;
__no_init UINT8 mdnsParamBuff[256];
FTW_MDNS_SRV_PARAM_T mdnsSrvcParam;
FTW_MDNS_ANNOUNCE_PARAM_T mdnsAnnceParam;
...
AppS2wCmd_MdnsHostNameReg(UINT8 *ptr)
{
    ...
    mdnsHnParam.ttl = param;
    	...
    	mdnsHnParam.ttl =  FTW_DFLT_MDNS_TTL; 
    ...
    mdnsHnParam.scope = FTW_DFLT_MDNS_SCOPE;
   ...
   if((p=(UINT8*)AppS2wParse_NextParamGet(&ptr)) != NULL)
    ...
    mdnsHnParam.conflctDetect = FTW_DFLT_MDNS_CONFLICT_DETECT;
    ...
    mdnsHnParam.autoCorrect = FTW_DFLT_MDNS_AUTOCORRECT;
...
#ifdef ADK_OTAFU
#define FTW_OTAFU_MDNS_KEY_VAL "api="GSN_HTTP_OTAFU_API_NAME":"GSN_HTTP_OTAFU_API_VERSION":"GSN_HTTP_OTAFU_API_PATH
#endif
#ifdef ADK_PROV
#define FTW_SYS_MDNS_KEY_VAL "api="GSN_HTTP_SYS_API_NAME":"GSN_HTTP_SYS_API_VERSION":"GSN_HTTP_SYS_API_PATH
#endif
...
AppS2wCmd_MdnsServiceReg(UINT8 *ptr)
{
    ...
    p = AppS2wParse_NextParamGet(&ptr);
     ...
     if (p)
     {
		...
		{
#ifdef ADK_PROV
		mdnsSrvcParam.keyVal[KeyValIndx] = FTW_SYS_MDNS_KEY_VAL;
		KeyValIndx++;
#else
		return FTW_EINVAL; 		 
#endif
		...
		{ 
#ifdef ADK_OTAFU
		//mdnsSrvcParam.keyVal[KeyValIndx] = FTW_OTAFU_MDNS_KEY_VAL;
		KeyValIndx++;
#else
		return FTW_EINVAL; 		 
#endif
   ...
   if((p=(UINT8*)AppS2wParse_NextParamGet(&ptr)) != NULL)
    ...
    mdnsSrvcParam.ttl = param;
    	...
    	mdnsSrvcParam.ttl =  FTW_DFLT_MDNS_TTL; 
    ...
    if((p=(UINT8*)AppS2wParse_NextParamGet(&ptr)) != NULL)
    {
    	...
    	mdnsSrvcParam.scope = FTW_DFLT_MDNS_SCOPE;    
   ...
   if((p=(UINT8*)AppS2wParse_NextParamGet(&ptr)) != NULL)
    ...
    mdnsSrvcParam.conflctDetect = (UINT8)param;
    	...
    	mdnsSrvcParam.conflctDetect = FTW_DFLT_MDNS_CONFLICT_DETECT;
    ...
    {    
    	...
    	mdnsSrvcParam.autoCorrect = FTW_DFLT_MDNS_AUTOCORRECT;
...
AppS2wCmd_MdnsAnnounceAll(UINT8 *ptr)
{
  ...
  { // register with default parameter
	  ...
	  memset(tmpBuff,0,255);
		if(s2wCurrent.mode == FTW_WLANDATA_MODE_PROV_AP || pAppRtcLtchInfo->grpProvVerify == 1)
	   	{
			len = sprintf((char*)tmpBuff,"%s,%s,%d,%d,%d,%d",
                    grpProvUuid,FTW_DFLT_MDNS_DOMAIN_NAME,FTW_DFLT_MDNS_TTL,
                    FTW_DFLT_MDNS_SCOPE,FTW_DFLT_MDNS_CONFLICT_DETECT,
                    FTW_DFLT_MDNS_AUTOCORRECT);
	   	}
	   	else
	   	{
	   		/* use configuration from builder*/
			len = sprintf((char*)tmpBuff,"%s,%s,%d,%d,%d,%d",
				   FTW_DFLT_MDNS_HOST_NAME,FTW_DFLT_MDNS_DOMAIN_NAME,FTW_DFLT_MDNS_TTL,
				   FTW_DFLT_MDNS_SCOPE,FTW_DFLT_MDNS_CONFLICT_DETECT,
				   FTW_DFLT_MDNS_AUTOCORRECT);
	   	}
       	tmpBuff[len]='\0';
       AppS2wCmd_MdnsHostNameReg(tmpBuff);
      	...
      	/* service registration */
       	memset(tmpBuff,0,255);
       	if(s2wCurrent.mode == FTW_WLANDATA_MODE_PROV_AP) // for grp provision need to use ssl port
       	{
       		/* group provision start*/
			grpMdnsPort = 443;
		   	len = sprintf((char*)tmpBuff,"%s,%s,%s,%s,%s,%d,%d,%s",
                    FTW_DFLT_MDNS_SER_INST_NAME,FTW_DFLT_MDNS_SER_SUB_TYPE,FTW_DFLT_MDNS_SER_TYPE,
                    FTW_DFLT_MDNS_PROTO,FTW_DFLT_MDNS_DOMAIN_NAME,grpMdnsPort,FTW_DFLT_MDNS_KEY,
                    FTW_DFLT_MDNS_TXT_RECORD );
	   	...
	   	{	   		
      		...
      		len = sprintf((char*)tmpBuff,"%s,%s,%s,%s,%s,%d,%d,%s",
				  	grpProvUuid,FTW_DFLT_MDNS_SER_SUB_TYPE,FTW_GRPPROV_VERIFY_MDNSSER_TYPE,
                    FTW_DFLT_MDNS_PROTO,FTW_DFLT_MDNS_DOMAIN_NAME,grpMdnsPort,FTW_DFLT_MDNS_KEY,
                    FTW_DFLT_MDNS_TXT_RECORD );
	   	...
	   	else
	   	{
	   		...
	   		len = sprintf((char*)tmpBuff,"%s,%s,%s,%s,%s,%d,%d,%s",
					FTW_DFLT_MDNS_SER_INST_NAME,FTW_DFLT_MDNS_SER_SUB_TYPE,FTW_DFLT_MDNS_SER_TYPE,
					FTW_DFLT_MDNS_PROTO,FTW_DFLT_MDNS_DOMAIN_NAME,FTW_DFLT_MDNS_PORT,FTW_DFLT_MDNS_KEY,
					FTW_DFLT_MDNS_TXT_RECORD );
	   	}
  ...
  if(GsnMdns_AnnounceAll(&s2wappMainTaskCtxt->mdns) != FTW_SUCCESS)
  {
    return FTW_FAILURE;
  }
  return FTW_SUCCESS;
...
AppS2wCmd_MdnsSrvcDiscover(UINT8 *ptr)
{
    ...
    UINT16 totalLen = 0;
    UINT32 val;
    FTW_MDNS_SRVCDISCVR_PARAM_T mdnsSrvDscr;
...
AppS2wCmd_Wstatus(UINT8 *ptr)
{
    ...
    if(info.mode == FTW_WLANDATA_MODE_INFRA)
    {
        mode = 0;
    }
    ...
    S2w_Printf("\r\nMODE:%d CHANNEL:%d SSID:\"",
            mode, info.channel);
    FTW_SERIAL_put(FTW_SERIAL_0, info.ssid, info.ssidLen);
    S2w_Printf("\"");
...
AppS2w_RawDataSend(UINT8 cid, UINT8 *buffer,UINT32 httpRxDataLen)
{
    ...
    len=sprintf(startMark,"%c%c%x%04d", FTW_ESC, 'H', cid,httpRxDataLen);


	
#ifndef FTW_DUAL_INTERFACE_SUPPORT
	GsnOsal_SemAcquire(&s2wSyncSemID, GSN_OSAL_WAIT_FOREVER);	
    FTW_SERIAL_put(FTW_SERIAL_0, startMark, len);
#else
	GsnOsal_SemAcquire(&s2wSyncSemID1, GSN_OSAL_WAIT_FOREVER);
    FTW_SERIAL_put(FTW_SERIAL_1, startMark, len);
#endif
	if(buffer != NULL)
    {
#ifndef FTW_DUAL_INTERFACE_SUPPORT
		FTW_SERIAL_put(FTW_SERIAL_0, buffer, httpRxDataLen);
#else
         FTW_SERIAL_put(FTW_SERIAL_1, buffer, httpRxDataLen);
#endif
    }
#ifndef FTW_DUAL_INTERFACE_SUPPORT
    if(s2wappMainTaskCtxt->serialDeviceId == FTW_PORT_FS_SPI)
...
AppS2wCmd_XmlSend(UINT8 *ptr)
{
	...
	if (!p)
	{
   	...
   	len = strlen((const char *)p);
#if defined (FTW_HTTPS_SUPPORT) || defined (FTW_HTTPC_SUPPORT)
...
#if (defined(FTW_GSLINK_XML) &&  defined(FTW_WEB_SERVER))
...
#endif
#if (defined(FTW_GSLINK_XML) || defined(FTW_GSLINK_RAW )) &&  (defined (FTW_HTTPS_SUPPORT) || defined(FTW_HTTPC_SUPPORT))
...
AppS2w_XmlDataProcess(UINT8 cid,UINT8 *dataBuffer,UINT32 inBuflen,UINT32 elementType)
{
		...
		if (s2wCidList[(cid)].httpdCtx.method == GSN_HTTP_METHOD_GETRESP || s2wCidList[(cid)].httpdCtx.method == GSN_HTTP_METHOD_POSTRESP)
	{
		...
		else if(encodeStatus == GSN_BUFFER_FULL)/* NO space in buffer to store the encoded data*/
		{
			...
			if(FTW_FAILURE == s2wappMainTaskCtxt->gsLinkSendStatus)
			{
...
#ifdef FTW_ISOTX_SUPPORT
#define FTW_ISOTX_BLINK_INDEX 6
...
AppS2wCmd_IsoTxStart(UINT8 *ptr)
{
	...
	do
	{
		if(index < FTW_ISOTX_BLINK_INDEX)
		{
			status = AppS2wParse_Int(p, &temp);
			if (status != FTW_SUCCESS)
			{
...
S2w_CmdUnSolicitedTx(UINT8 *ptr)
{
   ...
   /* get the remote mac addr1  */
    ...
    s2wappMainTaskCtxt->ccaThr = (INT8)((~FTW_DFLT_CCA_THR)+1);
	s2wappMainTaskCtxt->ccaTimeout = FTW_DFLT_CCA_TIMEOUT;
    return status;
}


UINT8 s2wUamBufs[FTW_NUM_UAM_BUFS][1500];
UINT8 uamRxInit=0;
PUBLIC GSN_STATUS
s2wUamRx_Init()
{
    ...
    if(!uamRxInit)
    {
    GsnRingBuf_Init( &s2wappMainTaskCtxt->s2wUamBufQ, sizeof(UINT32), FTW_NUM_UAM_BUFS+1,
        s2wappMainTaskCtxt->s2wUamBufQBuf);
    for (i = 0; i  < FTW_NUM_UAM_BUFS; i++)
    {
        pBuf = s2wUamBufs[i];
...
FTW_UnassocDataIndCB(VOID *ctx, INT8 *data, UINT16 dataLen)
{
    ...
    if (GSN_SUCCESS == GsnRingBuf_Get( &s2wappMainTaskCtxt->s2wUamBufQ, &rawbuf))
    {
         ...
         recvMsg.msgType = FTW_RECV_TYPE_UAMDATA;
...
GsnOsal_SemAcquire(&s2wSyncSemID, GSN_OSAL_WAIT_FOREVER);
#ifndef FTW_DUAL_INTERFACE_SUPPORT
    S2w_Printf("%cD", FTW_ESC);
#else
    S2w_Printf1("%cD", FTW_ESC);
#endif

#ifndef FTW_DUAL_INTERFACE_SUPPORT
    FTW_SERIAL_put(FTW_SERIAL_0, buf, len);
#else
    FTW_SERIAL_put(FTW_SERIAL_1, buf, len);
#endif
#ifndef FTW_DUAL_INTERFACE_SUPPORT
    if(s2wappMainTaskCtxt->serialDeviceId == FTW_PORT_FS_SPI)
    {
...
AppS2wCmd_DTSLClose(UINT8 *ptr)
{
    ...
    {
        return FTW_EINVAL;//AppS2w_SslClose(valCid);
    }
    nx_bsd_callback_register(s2wCidList[valCid].sd, NULL, 0);
	if(s2wCidList[valCid].conMode == FTW_NETDATA_MODE_CLIENT)
    {
...
AppS2w_CmdL2ConfigSet(UINT8 *ptr)
{
	...
	else
	{
		...
		if ((status != FTW_SUCCESS) ||((proto & FTW_PROTO_ALL_MASK) == 0))
		{
			return FTW_EINVAL;
		}
		p = AppS2wParse_NextParamGet(&ptr);
		...
		if ((status != FTW_SUCCESS)||(disable > FTW_PROTO_ALL_MASK))
		{
			return FTW_EINVAL;
		}
...
AppS2wCmd_PwmStart(UINT8 *ptr)
{
    ...
    if ((status != FTW_SUCCESS) || (period  == 0)||(period  > 1000))
    {
        return status;
    }
    ...
    if ((status != FTW_SUCCESS) || (phaseDelay01  > 1000))
    {
        return status;
    }
	}
    ...
    if ((status != FTW_SUCCESS) || (phaseDelay12 > 1000))
    {
        return status;
    }
	}
...
AppS2wCmd_BattCheckStart(UINT8 *ptr)
{
    ...
    status = FTW_BAT_CHECK_start(freq);
	if(FTW_RET_OK == status)
    {
...
AppS2wCmd_BattLvlSet(UINT8 *ptr)
{
    ...
    return FTW_BAT_CHECK_levelSet(warningLvl, warningFreq,stdByLvl);
...
AppS2wCmd_BattCheckSet(UINT8 *ptr)
{
    ...
    if (ptr[0] == '?' && ptr[1] == '\0')
    {
        status = FTW_BAT_CHECK_param(&battParam);
    ...
    {
		FTW_BAT_CHECK_set(battParam);	
		if(FTW_RET_OK == FTW_BAT_CHECK_stop() )
		{
			return FTW_BAT_CHECK_start(battParam);
		}
    }
	return FTW_FAILURE;
}
...
AppS2wCmd_BattValueGet(UINT8 *ptr)
{
	...
	if (*ptr)
	   return FTW_EINVAL;
	if(s2wappMainTaskCtxt->battChk.battChckStarted == 1)
		status = FTW_BAT_CHECK_valueGet(&battValue); 
	if(FTW_RET_OK == status)
	{
...
AppS2wCmd_WlanOtpRead (UINT8 *ptr)
{
	...
	status = AppS2wParse_Int(p, &temp);
	if ((status != FTW_SUCCESS) ||(temp > 255))
	{
		return FTW_EINVAL;
	}
	otpGrpId = temp;
...
{
    if (FTW_CERT_flashFileLoad("ca-cert") == NULL)
    {
        INT32 status;
        F_FILE *certFile;
