...
extern FTW_EXTFLASH_CMD_T s2wExtFlashCmdTable[6];
#ifdef FTW_SECURITY_WPS_ENABLE
extern UINT32 wpsConnDone;
#endif
extern UINT8 curCid;
extern APP_MAIN_CTX_T *s2wappMainTaskCtxt;
UINT32 certFormat, certFileSize;
F_FILE *certFile;
FTW_SSL_CERT_SRAM_TABLE_PTR _pxCert = NULL;
...
extern FTW_SERIAL_PORT_HANDLE_T  s2wSerialPortHandle1;

extern GSN_OSAL_SEM_T stbyDelaySemaphore;
#ifdef FTW_NCM_SUPPORT_ENABLE
extern FTW_TIMER_T acNcmTimer;
#endif


NXD_ADDRESS             DNSv6ServerAddr[2];
UINT32                  DNSv4ServerAddr[2];
UINT8                   FTW_DNS_CLIENT_FALGS;

#define FTW_DNS_CLIENT_ENABLED          0x01
#define FTW_DNSv6_SERVER_SET            0x02
#define FTW_DNSv4_SERVER_SET            0x04
#define FTW_DNS_REQ_DEF_TIME_OUT        5
#define FTW_DNS_REQ_DEF_RETRIES         3


UINT8 stbySuccess=0;
UINT32 apModeBeaconIntrl=100;
UINT8 globalUseHRssi=0;
/*__no_init*/ FTW_PROFILE_T s2wCurrent;
 ...
 */
FTW_WLANINFO_T current_wlan_info;
...
extern FTW_SCAN_DISPLAY_INFO_T scanDisplayInfo[20];
...
VOID
AppS2wRegisters_Display(const FTW_PROFILE_T *profile);
PRIVATE UINT8
...
typedef struct FTW_COAP_OPTION
{
	UINT8 optionKey;
	char *optionValue;
	struct FTW_COAP_OPTION * next;
}FTW_COAP_OPTION_T;

FTW_COAP_OPTION_T * optionHead;
...
#ifdef FTW_GSLINK
VOID
AppS2w_HttpdBodyLenSet(FTW_HTTPD_CONTEXT_T *httpdCtx,UINT32 len);

VOID
AppS2w_HttpdMethodSet(FTW_HTTPD_CONTEXT_T *httpdCtx,UINT8 method);
#endif

PUBLIC GSN_STATUS
APPS2w_ExtFlashStart( FTW_OTA_FWUP_CONF_T *pOtaFwupConf);
...
#if defined(FTW_DTLS_CLIENT_SUPPORT)
UINT8
...
typedef enum FTW_ERROR_CODE
{
	FTW_ERROR_CODE_AP_NOT_FOUND = 1,
	FTW_ERROR_CODE_WLAN_FAIL_TO_CONCT = 2,
	FTW_ERROR_CODE_WPA_AUTH_FAILURE = 3,
}FTW_ERROR_CODE_T;
 ...
 ******************************************************************/
PRIVATE const struct FTW_CMD s2wCmdTable[] =
{
    {"+WA=",        AppS2wCmd_Wassoc},
...
#endif    
#ifdef FTW_EXT_FLASH_FWUP_PULL_METHOD    
...
INT8 *headerValues[FTW_HTTPC_MAX_HDR_SUPPORTED];
GSN_HTTP_USER_HEADER_TYPE_T headerTypes[FTW_HTTPC_MAX_HDR_SUPPORTED];
GSN_HTTP_USER_HEADER_TYPE_T headerTypesModified[FTW_HTTPC_MAX_HDR_SUPPORTED];
 ...
 * @param ptr Pointer to the user input string.
 *
 * @return FTW_SUCCESS - operation successful.
 * @return FTW_FAILURE - operation Failed.
 * @return FTW_EINVAL - operation Failed-invalid input
 ******************************************************************/
#if defined(FTW_SSL_CLIENT_SUPPORT)

extern FTW_SSL_CONF_T s2wSslConf;
#endif
 UINT8
AppS2wCmd_HttpClientOpen(UINT8 *ptr)
{
    FTW_HTTP_DATA_T *pHttpData;
    UINT8 *p, i=0, status = FTW_EINVAL, /*ret,*/ endParam = 0, *tmp = NULL;
    UINT32 cid = -1, httpType, portNumber,len;
    INT32 certID;
    ULONG hostIpAddress;
    pHttpData = gsn_malloc(sizeof(FTW_HTTP_DATA_T));
    ...
    memset(pHttpData,0,sizeof(FTW_HTTP_DATA_T));
    p = AppS2wParse_NextParamGet(&ptr);
    ...
    if(!endParam)
    {
        ...
        else
        {
            ...
            {
                pxCert = FTW_CERT_searchRamTable((INT8*)p);
		        if(pxCert == NULL)
        		{
        			pxCert = FTW_CERT_flashFileLoad((INT8*)p);
        		}

                if(pxCert != NULL)
                {
                    pHttpData->caCertLen = *(UINT16 *)pxCert->pCertAddress;
                    pHttpData->pCaCert = pxCert->pCertAddress + 2;
                }
                else
                {
    ...
    if(!endParam)
    {
        ...
        else
        {
            if(0 != strlen((const INT8 *)p))
            {
                ...
                if(pxCert != NULL)
                {
                    pHttpData->clientCertLen= *(UINT16 *)pxCert->pCertAddress;
                    pHttpData->pClientCert= pxCert->pCertAddress + 2;
                }
                else
                {
    ...
    if(!endParam)
    {
        ...
        else
        {
            if(0 != strlen((const INT8 *)p))
            {
        		...
        		if(pxCert != NULL)
                {
                    pHttpData->clientKeyLen = *(UINT16 *)pxCert->pCertAddress;
                    pHttpData->pClientKey = pxCert->pCertAddress + 2;
                }
    ...
    if((httpConfAtCmd != 1) &&(!httpConfInfo.reqHeaderCount))
    {
...
#if defined(FTW_DFLT_HTTPC_AUTH) || defined(FTW_DFLT_HTTPC_HOST_IP)					
        INT8 *pValue;
        UINT32 len;
#endif		
#ifdef FTW_DFLT_HTTPC_AUTH					
        len= sizeof(FTW_DFLT_HTTPC_AUTH);
        ...
        memcpy(pValue, FTW_DFLT_HTTPC_AUTH, len);
        pValue[len] = '\0';
...
#ifdef FTW_DFLT_HTTPC_HOST_IP			
        len= sizeof(FTW_DFLT_HTTPC_HOST_IP);
        pValue = gsn_malloc(len+1);
        if(pValue == NULL)
        {
            return FTW_FAILURE;
        ...
        memset(pValue, 0 , len+1);
        memcpy(pValue, (UINT8*)FTW_DFLT_HTTPC_HOST_IP, len);
        pValue[len] = '\0';
        headerValues[i] = pValue;
	...
	compiled with header type array being defined as array of UINT8 */
	{
		...
		for( i = 0; i < FTW_HTTPC_MAX_HDR_SUPPORTED; i++)
        {
 ...
 ******************************************************************/
 ...
 AppS2wCmd_HttpClientConfig(UINT8 *ptr)
 {
    ...
    if(param == FTW_HTTPC_CFG_PARAM_REQUEST_URL)
    {
        p = AppS2wParse_NextParamGet(&ptr);
        if (!p)
        {
    ...
    else if((param > FTW_HTTPC_CFG_PARAM_INV && param < FTW_HTTPC_CFG_PARAM_HEADER_COUNT1)
        || (param >= FTW_HTTPC_CFG_PARAM_HEADER_CUSTOM))
    {
        pValue = gsn_malloc(len+1);
     ...
     if( i < FTW_HTTPC_MAX_HDR_SUPPORTED)
     {
	     headerValues[i] = pValue;
...
AppS2wCmd_HttpClientConfigDel(UINT8 *ptr)
{
    ...
    if((param > FTW_HTTPC_CFG_PARAM_INV && param < FTW_HTTPC_CFG_PARAM_HEADER_COUNT1)
        || (param >= FTW_HTTPC_CFG_PARAM_HEADER_CUSTOM))
...
UINT8 FTW_NET_HTTPD_contentDataGet(UINT32 dataLen, UINT8 hCid)
{
    ...
    pHttpData = (FTW_HTTP_DATA_T *)AppS2wHal_HttpHandleGet(hCid);
    if(pHttpData != NULL)
    {
    ...
    while(dataLen)
    {
        ...
        if(s2wCidList[hCid].sd != INVALID_CID)
        {
			...
			FTW_SERIAL_get(FTW_SERIAL_0, (UINT8 *)httpConfInfo.reqBodyPtr,httpConfInfo.reqBodyLen);
#else
            FTW_SERIAL_get(FTW_SERIAL_1, (UINT8 *)httpConfInfo.reqBodyPtr,httpConfInfo.reqBodyLen);
#endif
...
AppS2wCmd_HttpClientSend(UINT8 *ptr)
{
    ...
    if(method == FTW_HTTP_METHOD_GETRESP)
    {
    	method = GSN_HTTP_METHOD_GETRESP;
    }
    else if(method == FTW_HTTP_METHOD_POSTRESP)
    {
    ...
    httpConfInfo.reqVersion = GSN_HTTP_VERSION_1_1;
#endif
#if (defined(FTW_GSLINK_XML) || defined(FTW_GSLINK_RAW )) &&  defined(FTW_WEB_SERVER)
	...
	if (!p)
    {
        ...
        httpConfInfo.reqBodyLen = 0;
        pHttpData = (FTW_HTTP_DATA_T *)AppS2wHal_HttpHandleGet(cid);
        if(pHttpData != NULL)
        {
            GsnOsal_SemAcquire(&pHttpData->opInProgress, GSN_OSAL_WAIT_FOREVER);
 ...
 *
 * @return FTW_SUCCESS - operation successful.
 * @retval FTW_EINVAL  - operation failure-invalid command.
 ******************************************************************/
PUBLIC UINT8
...
AppS2wHdrAndIpv6_Print()
{
    ...
    if((s2wLinkState != FTW_LINK_STATE_DISCONNECTED)  && (GsnNwif_Ipv6RouterGet(&s2wappMainTaskCtxt->if0.nwifCtx,ipv6Router) == GSN_SUCCESS))
    {
...
AppS2wCmd_Nctcp(UINT8 *ptr)
{
    ...
    peerData.proto  = FTW_NETDATA_PROTO_TCP;
    peerData.mode   = FTW_NETDATA_MODE_CLIENT;
...
AppS2wCmd_scanTimeSet(UINT8 *ptr)
{
    ...
    if (ptr[0] == '?' && ptr[1] == '\0')
    {
        S2w_Printf("\r\nMinScanTime=%d\r\nMaxScanTime=%d\r\n", s2wCurrent.minScanTime,
                    s2wCurrent.registers[FTW_REG_SCAN_TIME]);
        return FTW_SUCCESS;
    }
    p = AppS2wParse_NextParamGet(&ptr);
    if (!p)
   ...
   /* update the default in profile */
    s2wCurrent.registers[FTW_REG_SCAN_TIME] = maxTime;
 ...
 * @param ptr Pointer to the user input string.
 * @return status, FTW_SUCCESS or FTW_FAILURE.
 ******************************************************************/
PUBLIC UINT8
AppS2wCmd_Nstcp(UINT8 *ptr)
{
    ...
    if (status != FTW_SUCCESS || !is_valid_port(port))
    {
        return FTW_EINVAL;
    }
	p = AppS2wParse_NextParamGet(&ptr);
    ...
    peerData.port = port;
    peerData.proto = FTW_NETDATA_PROTO_TCP;
    peerData.mode = FTW_NETDATA_MODE_SERVER;
...
AppS2wCmd_Nsudp(UINT8 *ptr)
{
    ...
    peerData.port = port;
    peerData.proto = FTW_NETDATA_PROTO_UDP;
    peerData.mode = FTW_NETDATA_MODE_SERVER;
 ...
 * @param ptr Pointer to the user input string.
 * @return status, FTW_SUCCESS ,FTW_FAILURE or FTW_EINVAL.
 ******************************************************************/
PUBLIC UINT8
AppS2wCmd_SetSockOp(UINT8 *ptr)
{
    UINT8 status;
    FTW_SOCK_OP_T params;
    UINT8 *p;
        ...
        UINT8 channel, UINT8 rssi,  UINT8 adhoc)
{
     ...
     */
    FTW_SERIAL_put(FTW_SERIAL_0, prefix, sizeof(prefix) - 1);
    S2w_Printf(" %02x:%02x:%02x:%02x:%02x:%02x, ",
           bssid[0], bssid[1], bssid[2], bssid[3], bssid[4], bssid[5]);

    FTW_SERIAL_put(FTW_SERIAL_0, ssid, ssidLen);
...
AppS2wCmd_Wmode(UINT8 *ptr)
{
    ...
    if(mode > FTW_WLANDATA_MODE_PROV_AP || FTW_WLANDATA_MODE_ADHOC == mode )
    {
        return FTW_EINVAL;
    }
    // remap the mode
    if(mode == 0)
    {
        mode = FTW_WLANDATA_MODE_INFRA;
    }
	if(mode == s2wCurrent.mode)
	{			
		if((mode == FTW_WLANDATA_MODE_AP) || (mode == FTW_WLANDATA_MODE_PROV_AP))
		{
			s2wappMainTaskCtxt->bcastSsid = FTW_DEFAULT_AP_SSID_BROADCAST;
    ...
    {
        s2wappMainTaskCtxt->bcastSsid = FTW_DEFAULT_AP_SSID_BROADCAST;
		s2wCurrent.lmtdApConf.noStations = FTW_DEFAULT_AP_SUPPORTED_STATIONS;
        p = AppS2wParse_NextParamGet(&ptr);
        if ((p) && (*p != '\0'))
        {
        ...
        if ((p) && (*p != '\0'))
        {
            ...
            if (status != FTW_SUCCESS || noStations == 0)
            {
                return FTW_EINVAL;
            }
		...
		if ((p) && (*p != '\0'))
		{
			...
			if (status != FTW_SUCCESS || dtimPeriod == 0)
			{
				return FTW_EINVAL;
			}
		...
		if ((p) && (*p != '\0'))
		{
			...
			if (status != FTW_SUCCESS || inactivityTimeoutInSec == 0)
			{
				return FTW_EINVAL;
			}
		...
		if ((p) && (*p != '\0'))
		{
			...
			if (status != FTW_SUCCESS || grpKeyUpdtIntrvl == 0)
			{
				return FTW_EINVAL;
			}
        ...
        if(prevWlanNwType == FTW_WLANDATA_MODE_ISOTX)
        {
    ...
    else if (FTW_WLANDATA_MODE_INFRA == mode)/*STA Mode*/
    {
    ...
    else if (FTW_WLANDATA_MODE_ISOTX == mode)/*STA Mode*/
    {
#ifdef FTW_ISOTX_SUPPORT       
	   App_InterfaceStop(s2wappMainTaskCtxt, &s2wappMainTaskCtxt->if0);
    ...
    // update the mac address after switching the mode
    {
        ...
        if(mode == FTW_WLANDATA_MODE_INFRA)
        {
 ...
 * @return FTW_SUCCESS - operation successful.
 * @retval FTW_EINVAL  - operation failure-invalid command.
 * @retval FTW_FAILURE - operation failure-Power save configuration failed.
 ******************************************************************/
UINT8
...
PUBLIC VOID
s2wScanResult_Print(FTW_SCAN_DISPLAY_INFO_T scanDisplayInfo[],UINT8 count)
{
    UINT8 len=0,i=0;
    for(i=0;i<count;i++)
    {
         ...
         */
         ...
         FTW_SERIAL_put(FTW_SERIAL_0, scanDisplayInfo[i].ssid.array, scanDisplayInfo[i].ssid.length);
 ...
 * @return FTW_SUCCESS - operation successful.
 * @retval FTW_EINVAL  - operation failure-invalid command.
 * @retval FTW_FAILURE - operation failure-scan failed.
 ******************************************************************/
PRIVATE UINT8
AppS2wCmd_Wscan(UINT8 *ptr)
{
    UINT32 status = FTW_SUCCESS;
    UINT8 allChScan=0,i=0;
    FTW_WLANDATA_T params;
    ...
    {
        memcpy(scanParams.bssid.array, params.bssid,FTW_MAC_ADDR_SIZE);
    }    
    ...
    for(i=1;i<15;i++)
    {
		...
		if((allChScan !=1) || ((scanParams.channel[0] == FTW_MAX_CH) && (s2wCurrent.regDomain == FTW_DOMAIN_FCC))
		   ||((scanParams.channel[0] == (FTW_MAX_CH+2)) && (s2wCurrent.regDomain == FTW_DOMAIN_ETSI)) ||
		     ((scanParams.channel[0] == (FTW_MAX_CH+3)) && (s2wCurrent.regDomain == FTW_DOMAIN_TELEC)))
			{
				break;
		}
...
S2w_getApSecurityConfig(GSN_WDD_WLAN_CFG_T *pWlanConfig, UINT8 securityType)
{
	switch(securityType)
    {
    	...
    	case SECTYPE_WEP:
    	{
            ...
            {
                pWlanConfig->securityCfg.wepConfig.KeyId = FTW_DEFAULT_GRP_PROV_WEP_KEY_ID -1  ;
                pWlanConfig->securityCfg.wepConfig.KeyLen = (GSN_WDD_WEP_KEY_LEN_T)FTW_DEFAULT_GRP_PROV_WEP_KEY_LEN ; /**< Key len, 5 or 13 */
                memcpy(&pWlanConfig->securityCfg.wepConfig.Key,FTW_DEFAULT_GRP_PROV_WEP_KEY,
                        FTW_DEFAULT_GRP_PROV_WEP_KEY_LEN);
                if(FTW_DEFAULT_GRP_PROV_WEP_AUTH == GSN_MAC_AUTH_SHARED)
                {
       ...
       (securityType == SECTYPE_WPA2PSK_TKIP))
    {
       if(s2wCurrent.mode == FTW_WLANDATA_MODE_PROV_AP)
       {
           pWlanConfig->securityCfg.pskConfig.passPhrase.Len = sizeof (FTW_DEFAULT_GRP_PROV_WPA_PASSPHRASE)- 1 ;
           memcpy( pWlanConfig->securityCfg.pskConfig.passPhrase.Passphrase, FTW_DEFAULT_GRP_PROV_WPA_PASSPHRASE,
...
AppS2wCmd_Wassoc(UINT8 *ptr)
{
    ...
    wpsConnDone = FALSE;
#endif
    if ((s2wLinkState != FTW_LINK_STATE_DISCONNECTED) || (GsnWdd_AssocStatusGet(&s2wappMainTaskCtxt->if0.wddCtx) == TRUE))
    {
     ...
     */
      ...
      //  GsnOsal_SemCreate  ( &s2wConnectSemaphore, 0 );
		...
		if(params.validBssid)
		{
			memcpy((INT8 *)pWlanConfig->bssid.array,params.bssid,FTW_MAC_ADDR_SIZE);
        ...
        /*FTW_NET_macAddrSet(s2wCurrent.macAddr);*/
		L2ConnectStatus = 0;
		...
		{
            pxCert = FTW_CERT_searchRamTable(FTW_CERT_NAME_CA);
            if(pxCert == NULL)
            {
                pxCert = FTW_CERT_flashFileLoad(FTW_CERT_NAME_CA);
            }
            if(pxCert != NULL)
            {
                    pWlanConfig->securityCfg.eapConfig.pCaCert = (char*)pxCert->pCertAddress + 2;
                    pWlanConfig->securityCfg.eapConfig.caCertLen = *(UINT16 *)pxCert->pCertAddress;
            }
		}
        pxCert = FTW_CERT_searchRamTable(FTW_CERT_NAME_CLIENT);
        if(pxCert == NULL)
        {
            pxCert = FTW_CERT_flashFileLoad(FTW_CERT_NAME_CLIENT);
        }
	    if(pxCert != NULL)
	    {
	    	pWlanConfig->securityCfg.eapConfig.pClientCert  = (char*)pxCert->pCertAddress + 2;
            pWlanConfig->securityCfg.eapConfig.clientCertLen = *(UINT16 *)pxCert->pCertAddress;
	    }
        pxCert = FTW_CERT_searchRamTable(FTW_CERT_NAME_KEY);
        if(pxCert == NULL)
        {
            pxCert = FTW_CERT_flashFileLoad(FTW_CERT_NAME_KEY);
	    }
	    if(pxCert != NULL)
	    {
	        pWlanConfig->securityCfg.eapConfig.pPrivateKey = (char*)pxCert->pCertAddress + 2;
            pWlanConfig->securityCfg.eapConfig.privateKeyLen = *(UINT16 *)pxCert->pCertAddress;
	    }
#endif
TRY_AGAIN:
		/* start the timer*/
       ...
       if(0 == eapFastRetryCnt && s2wLinkState == FTW_LINK_STATE_DISCONNECTED &&
            s2wCurrent.security.eap.outerAuth == GSN_EAP_TYPE_FAST)
       {
        ...
        if(params.mode == 2)
        {
            current_wlan_info.mode = FTW_WLANDATA_MODE_AP;
	    }
	    else
	    {
...
#ifdef FTW_IP2WIFI_SUPPORT  // ip2wifi
...
VOID DHCP_notify(INT32 status,void *ctx)
{
    ...
    {
        UINT32 message;
        if((L3ConnectStatus == FALSE) && (s2wLinkState != FTW_LINK_STATE_DISCONNECTED))
        {
        ...
        else if(s2wLinkState != FTW_LINK_STATE_DISCONNECTED)
        {
 ...
 * @return FTW_SUCCESS - operation successful.
 * @retval FTW_EINVAL  - operation failure-invalid command.
 * @retval FTW_FAILURE - operation failure-flash write failed.
 ******************************************************************/
PRIVATE UINT8
AppS2wCmd_read(UINT8 *ptr)
{
...
AppS2wCmd_Write(UINT8 *ptr)
{
    ...
    memcpy(profile, &s2wCurrent, sizeof(FTW_PROFILE_T));
#ifdef FTW_NCM_SUPPORT_ENABLE
 ...
 * @return FTW_SUCCESS - operation successful.
 * @retval FTW_EINVAL  - operation failure-invalid command.
 * @retval FTW_FAILURE - operation failure-ip config failed.
 ******************************************************************/
PRIVATE UINT8
AppS2wCmd_Ndhcp(UINT8 *ptr)
{
  ...
  if((p=(UINT8*)AppS2wParse_NextParamGet(&ptr)) != NULL)
  {
	...
	else
	{
       /* update the active profile paramater  */
		...
		if(s2wCurrent.mode == FTW_WLANDATA_MODE_AP)
		{
			memset(&s2wCurrent.dns1,0,sizeof(FTW_IPADDR_T));
			memset(&s2wCurrent.dns2,0,sizeof(FTW_IPADDR_T));
		}
 ...
 * @return FTW_SUCCESS - operation successful.
 * @retval FTW_EINVAL  - operation failure-invalid command.
 * @retval FTW_FAILURE - operation failure-MAC configuration failed.
 ******************************************************************/
PRIVATE UINT8
AppS2wCmd_Nmac(UINT8 *ptr)
{
    ...
    /* Call the hal function to configure the mac address
     */
    status = FTW_NET_macAddrSet(macAddr, TRUE);
    if (status != FTW_SUCCESS)
    {
        return FTW_FAILURE;
    }
 ...
 * @return FTW_SUCCESS - operation successful.
 * @retval FTW_EINVAL  - operation failure-invalid command.
 * @retval FTW_FAILURE - operation failure-uart configuration failed.
 ******************************************************************/
PRIVATE UINT8
AppS2wCmd_BaudRate(UINT8 *ptr)
{

    FTW_SERIAL_PORT_CONF_T params;
    UINT32 temp;
    UINT8 status;
    UINT8 *p;
    ...
    {
    case 9600:
        xConfig.xBaudRate = FTW_UART_BAUDRATE_9K6;
        break;
    case 19200:
        xConfig.xBaudRate = FTW_UART_BAUDRATE_19K2;
        break;
    case 38400:
        xConfig.xBaudRate = FTW_UART_BAUDRATE_38K4;
        break;
    case 57600:
        xConfig.xBaudRate = FTW_UART_BAUDRATE_57K6;
        break;
    case 115200:
        xConfig.xBaudRate = FTW_UART_BAUDRATE_115K2;
        break;
    case 230400:
        xConfig.xBaudRate = FTW_UART_BAUDRATE_230K4;
        break;
    case 460800:
        xConfig.xBaudRate = FTW_UART_BAUDRATE_460K8;
        break;
    case 921600:
        xConfig.xBaudRate = FTW_UART_BAUDRATE_921K6;
        break;
    default:
        return  FTW_EINVAL;
    }
    ...
    if (p && *p)
    {
        ...
        xConfig.xDataBits = (FTW_UART_DATABITS)temp;
    }
    ...
    if (p && *p)
    {
        ...
        {
        case 'N':
            xConfig.xParityBits = FTW_UART_PARITYBIT_NONE;
            break;

        case 'O':
            xConfig.xParityBits = FTW_UART_PARITYBIT_ODD;
            break;

        case 'E':
            xConfig.xParityBits = FTW_UART_PARITYBIT_EVEN;
            break;
    ...
    if (p && *p)
    {
        ...
        xConfig.xStopBits = (FTW_UART_STOPBITS)temp;
    }

    /* call the hal uart configuration function
     */

    if (FTW_UART_setConfig(FTW_UART0, &xConfig) != FTW_RET_OK)
    {
        return FTW_FAILURE;
    }
...
AppS2wCmd_UartFifoLevel(UINT8 *ptr)
{
    ...
    else
    {
        ...
        if ((status != FTW_SUCCESS) || (rxLevel < 1) || (rxLevel > 15))
        {
            return FTW_EINVAL;
        }
        ...
        if (p && *p)
        {
            ...
            if ((status != FTW_SUCCESS) || (txLevel > 15))
            {
                return FTW_EINVAL;
            }
 ...
 * @return FTW_SUCCESS - operation successful.
 * @retval FTW_EINVAL  - operation failure-invalid command.
 * @retval FTW_FAILURE - operation failure- profile setting failed.
 ******************************************************************/
PRIVATE UINT8
AppS2wCmd_SetDefaultProfile(UINT8 *ptr)
{
 ...
 * @return FTW_SUCCESS - operation successful.
 * @retval FTW_EINVAL  - operation failure-invalid command.
 * @retval FTW_FAILURE - operation failure- disconnect failed.
 ******************************************************************/
PRIVATE UINT8
AppS2wCmd_Disconnect(UINT8 *ptr)
{
 ...
 * @return FTW_SUCCESS - operation successful.
 * @retval FTW_EINVAL  - operation failure-invalid command.
 * @retval FTW_FAILURE - operation failure- radio configuration failed.
 ******************************************************************/
PRIVATE UINT8
AppS2wCmd_Wradio(UINT8 *ptr)
{
...
AppS2wCmd_Nstat(UINT8 *ptr)
{
    FTW_STATUS_INFO_T statusInfo;
    const char *security = "NONE";
    GSN_WIF_WLAN_RSSI_T    wRssi;

    if (ptr[0] == '?' && ptr[1] == '\0')
    {
        memset(&statusInfo,0,sizeof(FTW_STATUS_INFO_T));
        statusInfo.wlanInfo.mode = current_wlan_info.mode;
        FTW_NET_macAddrGet(statusInfo.macAddr);
        ...
        if(statusInfo.wstate)
        {
		  ...
		  GSN_WLAN_MAC_ADDR_T bssid;
           ...
           FTW_SERIAL_put(FTW_SERIAL_0, WddConf->ssid.array,(WddConf->ssid.length));
           S2w_Printf("\"");
...
AppS2wCmd_HardFlow(UINT8 *ptr)
{
    ...
    FTW_UART_setHWFlowCtrl(FTW_UART0, enable);
...
AppS2wCmd_SoftFlow(UINT8 *ptr)
{
    ...
    FTW_UART_setSWFlowCtrl(FTW_UART0, enable);
...
AppS2wHex_Print(UINT8 *data, UINT32 len)
{
    ...
    for (i = 0; i < len; i++)
    {
        FTW_SERIAL_putc(FTW_SERIAL_0, hex_asc[(data[i] & 0xf0) >> 4]);
        FTW_SERIAL_putc(FTW_SERIAL_0, hex_asc[(data[i] & 0x0f)]);
    }
 ...
 * @return FTW_SUCCESS - operation successful.
 * @retval FTW_EINVAL  - operation failure-invalid command.
 * @retval FTW_FAILURE - operation failure-socket close failed.
 ******************************************************************/
PUBLIC UINT8
...
VOID
AppS2wSecurity_Display(FTW_SECURITY_T *sec)
{
    INT32 i;
 ...
 ******************************************************************/
PUBLIC VOID
AppS2wIp_Print(FTW_IPADDR_T ip)
{
...
#endif
{
    ...
    S2w_Printf("%c,%d", c,
           profile->serialPortConf.conf.uart.stop == FTW_UARTCONF_STOP_2 ? 2 : 1);
    ...
    if(profile->mode == FTW_WLANDATA_MODE_INFRA)
    {
        S2w_Printf("\r\n+WM=%d ", 0);
    }
    else
...
#ifndef FTW_IP2WIFI_SUPPORT
    if(profile->autoWlandata.mode == FTW_WLANDATA_MODE_INFRA)
    {
    ...
    FTW_SERIAL_put(FTW_SERIAL_0, profile->autoWlandata.ssid,
    ...
    {
        FTW_SERIAL_put(FTW_SERIAL_0, vBuff,2);
    }
    ...
    *(ULONG*)profile->autoNetdata.ipAddr = ntohl(*(ULONG*)profile->autoNetdata.ipAddr);
    ...
    if((profile->autoNetdata.mode == 0) &&(profile->autoNetdata.proto == FTW_NETDATA_PROTO_UDP))
    {
...
PUBLIC VOID
AppS2wNcmProfile_Display(FTW_PROFILE_T *profile)
{
	#ifndef FTW_IP2WIFI_SUPPORT
	    UINT8 vBuff[4]={"\","};
    #endif
    ...
    *(ULONG*)profile->autoNetdata.ipAddr = ntohl(*(ULONG*)profile->autoNetdata.ipAddr);
    ...
    S2w_Printf("\r\n+WAUTO=%d,\"", 2);
    FTW_SERIAL_put(FTW_SERIAL_0, profile->lmtdApConf.ssid.array,
 ...
 * @return FTW_SUCCESS - operation successful.
 * @retval FTW_EINVAL  - operation failure-invalid command.
 * @retval FTW_FAILURE - operation failure-Retry configuration failed.
 ******************************************************************/
UINT8
 ...
 * @return FTW_SUCCESS - operation successful.
 * @retval FTW_EINVAL  - operation failure-invalid command.
 * @retval FTW_FAILURE - operation failure-power configuration failed.
 ******************************************************************/
UINT8
AppS2wCmd_Wpower(UINT8 *ptr)
{
    ...
    if ((status != FTW_SUCCESS) || (power[0] > 15))
    {
        return FTW_EINVAL;
    }
    ...
    for(i=1;i<5;i++)
    {
		...
		if (p && *p)
        {
		    ...
		    if ((status != FTW_SUCCESS) || (power[i] > 15))
    {
        return FTW_EINVAL;
    }
...
AppS2wCmd_Nsetv6(UINT8* ptr)
{
  ...
  switch(IPv6AddrType)
  {
    ...
    case GSN_IPv6_LINK_LOCAL:
      ...
      memcpy(Ipv6Params.linkLocalAddr,IPv6Addr,FTW_IPv6_ADDR_LENGTH);
      break;
    ...
    case GSN_IPv6_GLOBAL:
      ...
      memcpy(Ipv6Params.globalAddr,IPv6Addr,FTW_IPv6_ADDR_LENGTH);
      break;
  ...
  if(!p)
    Ipv6Params.prefixLen = FTW_IPv6_DEFAULT_PREFIX_LENGTH;
  else
  {
...
AppS2wCmd_Ndhcpv6(UINT8 *ptr)
{
  ...
  FTW_STATUS_INFO_T currentConfig;
  GSN_NWIF_DHCP_PARAMS_T dhcpv6Params;
  ...
  status = AppS2wParse_Boolean(p, &enable);
    ...
    if(enable)
    {
      if(GsnWdd_AssocStatusGet(&s2wappMainTaskCtxt->if0.wddCtx) == TRUE)
      {
          ...
          memset(&currentConfig,0,sizeof(FTW_STATUS_INFO_T));
          memset(&dhcpv6Params,0,sizeof(GSN_NWIF_DHCP_PARAMS_T));
...
AppS2wCmd_Ncudpv6(UINT8 *ptr)
{
  ...
  FTW_NETDATA_T remoteServerData;
  
  memset(&remoteServerData,0,sizeof(FTW_NETDATA_T));
  ...
  if (status != FTW_SUCCESS || !is_valid_port(remotePort))
  {
    return FTW_EINVAL;
  }
  ...
  if(p != NULL)
  {
    ...
    if (status != FTW_SUCCESS || !is_valid_port(sourcePort))
    {
      return FTW_EINVAL;
    }
    ...
    remoteServerData.proto = FTW_NETDATA_PROTO_UDP;
    remoteServerData.mode  = FTW_NETDATA_MODE_CLIENT;
...
AppS2wCmd_Nsudpv6(UINT8* ptr)
{
    ...
    serverData.proto = FTW_NETDATA_PROTO_UDP;
    serverData.mode = FTW_NETDATA_MODE_SERVER;
...
AppS2wCmd_Nctcpv6(UINT8* ptr)
{
  ...
  remoteServerData.proto = FTW_NETDATA_PROTO_TCP;
  remoteServerData.mode  = FTW_NETDATA_MODE_CLIENT;
...
AppS2wCmd_Nstcpv6(UINT8* ptr)
{
    ...
    serverData.port = port;
    serverData.proto = FTW_NETDATA_PROTO_TCP;
    serverData.mode = FTW_NETDATA_MODE_SERVER;
...
AppS2wCmd_Ncudp(UINT8 *ptr)
{
    ...
    if (p)
    {
        ...
        if (status != FTW_SUCCESS || !is_valid_port(srcPort))
        {
            return FTW_EINVAL;
        }
 ...
 * @return FTW_SUCCESS - operation successful.
 * @retval FTW_EINVAL  - operation failure-invalid command.
 * @retval FTW_FAILURE - operation failure- Register setting failed.
 ******************************************************************/
UINT8
AppS2wCmd_Settings(UINT8 *ptr)
{
    ...
    else if (isdigit(*ptr))
    {
        ...
        if (status != FTW_SUCCESS || value > 65535)
        {
            return FTW_EINVAL;
        }
        ...
        if(FTW_REG_SCAN_TIME == which)/* scan time in mSec. Set it in wlan Fw too*/
        {
            /* set the scan time
            */
...
#ifdef FTW_SSL_CLIENT_SUPPORT
        if( FTW_NETDATA_MODE_CLIENT == AppS2w_ConnModeGet(valCid))
        {
            status = AppS2w_SslClientOpen(valCid, (INT8*)pCertName, pClientCertName,
                                          pClientKeyName);
        }
#endif
#ifdef FTW_SSL_SERVER_SUPPORT
        if( FTW_NETDATA_MODE_SERVER == AppS2w_ConnModeGet(valCid))
        {
...
AppS2wCmd_ntpTimeSet(UINT8 *ptr)
{
    ...
    if(enable)
    {
	    ...
	    {
            if(FTW_DFLT_SNTP_PERIODIC)
            {
                periodic=1;
                s2wappMainTaskCtxt->sntpTimerPeriod = FTW_DFLT_SNTP_PERIODIC_PERIOD;
			}
			else
			{
				periodic=0;
			}
            s2wappMainTaskCtxt->sntpSrvIp = ntohl(FTW_DFLT_SNTP_SVR_IP);
            s2wappMainTaskCtxt->sntpSrvTimeout = FTW_DFLT_SNTP_TIMEOUT;
		}
...
AppS2wCmd_DhcpSrvrConfig(UINT8 *ptr)
{
   ...
   dhcpSrvrCfg.startIpAddr = ntohl(dhcpSrvrCfg.startIpAddr);
    ...
    if(dhcpSrvrCfg.numOfClients > FTW_DHCP_SRVR_MAX_CLIENTS)
    {
       return FTW_EINVAL;
    }
    ...
    dhcpServerFlags |= FTW_DHCP_SRVR_CFG_DONE;
...
AppS2wCmd_DhcpSrvr(UINT8 *ptr)
{
    ...
    if(mode != 0)
    {
       ...
       if(!(dhcpServerFlags & FTW_DHCP_SRVR_CFG_DONE))
       {
        ...
        if(!dns)
        {
          ...
          if(FTW_DNS_CLIENT_FALGS & FTW_DNSv4_SERVER_SET)
          {
        ...
        if(!gateway)
        {
			...
			else if(FTW_WLANDATA_MODE_AP == s2wCurrent.mode )
			{			
	            memcpy(&dhcpSrvrCfg.gateway, &s2wCurrent.netConf.gateway, 4);
			}
			else if(FTW_WLANDATA_MODE_INFRA == s2wCurrent.mode )
			{
				 memcpy(&dhcpSrvrCfg.gateway, &s2wCurrent.netConf.gateway, 4);
			}
		...
		{
                        dhcpSrvrCfg.leaseTime = htonl((UINT32)(FTW_DFLT_AP_DHCPSRVR_LEASE_TIME));
    ...
    {
        dhcpServerFlags |= FTW_DHCP_SRVR_ENABLED;
        return FTW_SUCCESS;
    }
}
...
VOID S2wDnsConfigGet(GSN_DNS_INIT_PARAM_T *gsnDnsInitParams)
{
    INT32 ip;
    GSN_NWIF_IP_CONFIG_T  ipconfig;
    memcpy(&ip, s2wCurrent.netConf.ipAddr, sizeof(FTW_IPADDR_T));
    ...
    memcpy(&ip, &ipconfig.ipv4.ipAddr, sizeof(FTW_IPADDR_T)); 
#ifdef FTW_IPv6_SUPPORT    
...
AppS2wCmd_SslCertificateAdd(UINT8 *ptr)
{
	...
	if(FTW_CERT_findFile((char*)name) == -1)
	{
		S2w_Printf("\n\rFILE EXISTS\n\r");
		return FTW_FAILURE;
	}
    if(certStore)
    {
    	pxCert = FTW_CERT_create(certFileSize);
	    //S2w_Printf("\nmem-alloc :%d\n",i);
	    if(pxCert == NULL)
        {
	    	return FTW_FAILURE;
        ...
        pxCert->pName = p; //copy pointer to SRAM entry
        pxCert->xLocation = FTW_CERT_IN_RAM;
        _pxCert = pxCert;
    }
    else
    {
...
AppS2wCertificateData(UINT8 ch1)
{
    ...
    if ( (_pxCert == NULL) && (certFile == NULL))
            return FTW_FAILURE;

    if( certFileSize == 0)
            return FTW_FAILURE;

	if((_pxCert != NULL) && (FTW_CERT_IN_RAM == _pxCert->xLocation ) )
	{
		buf = FTW_MEM_allocZero((certFileSize << certFormat) + 2 );
		certInRam=1;
    }
	else
    {
        buf = FTW_MEM_allocZero((certFileSize << certFormat) );
    }
    ...
    FTW_SERIAL_get(FTW_SERIAL_0, (buf), certFileSize);
    ...
    {
        /* store in SRAM */
        memcpy((UINT8 *) _pxCert->pCertAddress, tempBuff, ((certFileSize >> certFormat) + 2));
    }
    else
    {
        /* store in flash */
...
AppS2wCmd_SslCertificateDelete(UINT8 *ptr)
{
    ...
    pxCert = FTW_CERT_searchRamTable((char*)p);
    if(pxCert != NULL)
    {
    	FTW_MEM_free(pxCert->pCertAddress);
        FTW_MEM_free(pxCert->pName);
        pxCert->pName = NULL;
        pxCert->pCertAddress = NULL;
        if( pxCert->xLocation == FTW_CERT_IN_FLASH )
    	{
			retVal = FTW_CERT_delete(p);
    	}
    }
    else
    {
    ...
    if(retVal == FTW_SUCCESS)
    {
    	if(strcmp((const char*)p,FTW_CERT_NAME_CA) == 0)
    	{
...
AppS2wCmd_Weap(UINT8 *ptr)
{
    ...
    if(certStore)
    {
        ...
        name = FTW_MEM_allocZero(32);
        if(certType == 0)
            strcpy((INT8 *)name, FTW_CERT_NAME_CA);
        else if(certType == 1)
            strcpy((INT8 *)name, FTW_CERT_NAME_CLIENT);
        else
            strcpy((INT8 *)name, FTW_CERT_NAME_KEY);
        pxCert->pName = name;
        pxCert->xLocation = FTW_CERT_IN_RAM;
		_pxCert = pxCert;
    }
    else
    {
		...
		if(status != F_NO_ERROR || status != F_ERR_DUPLICATED )
		{
			fileLen= sprintf((char*)buf,"certs/");
            if(certType == 0)
                memcpy(buf + fileLen, FTW_CERT_NAME_CA, strlen(FTW_CERT_NAME_CA));	           
            else if(certType == 1)
                memcpy(buf + fileLen, FTW_CERT_NAME_CLIENT, strlen(FTW_CERT_NAME_CLIENT));
            else
                memcpy(buf + fileLen, FTW_CERT_NAME_KEY, strlen(FTW_CERT_NAME_KEY));
...
{
  UINT8 status = FTW_FAILURE, enable = 0;
  UINT8 *p;
...
AppS2wCmd_Wwpa(UINT8 *ptr)
{
    ...
    len = strlen((char *)p);
    if ((len > FTW_PSK_PASSPHRASE_LEN_MAX - 1) ||
        (len < FTW_PSK_PASSPHRASE_LEN_MIN))
    {
        return FTW_EINVAL;
    }

    memset(params.wpa.wpaPassPhrase, 0, FTW_PSK_PASSPHRASE_LEN_MAX);
    memcpy(params.wpa.wpaPassPhrase, p, len);
    params.wpa.passPhraseLen = len;
    params.validKeys |= FTW_SECURITY_VALID_WPA;
 ...
 * @return FTW_SUCCESS - operation successful.
 * @retval FTW_EINVAL  - operation failure-invalid command.
 * @retval FTW_FAILURE - operation failure- psk key calculation failed.
 ******************************************************************/
UINT8
AppS2wCmd_WpaPsk(UINT8 *ptr)
{
    ...
    len = strlen((char *)p);
    if ((len > FTW_PSK_PASSPHRASE_LEN_MAX - 1) ||(len < FTW_PSK_PASSPHRASE_LEN_MIN))
    {
        return FTW_EINVAL;
    }
    ...
    if(s2wappMainTaskCtxt->apConf)
    {
	    ...
	    memcpy(&(s2wCurrent.lmtdApConf.lmtdApPsk), &wpaPsk.psk,FTW_PSK_LEN);		
...
AppS2wCmd_Wwep(UINT8 *ptr)
{
    ...
    params.validKeys |= FTW_SECURITY_VALID_WEP;
...
AppS2wCmd_RegDomainSet(UINT8 *ptr)
{
    ...
    UINT32 domainId=0;
    UINT8 status = FTW_EINVAL,domain=0;
    FTW_FLASH_PARAMS_T s2wProfile;
    ...
    if(domainId >= FTW_MAX_REG_DOMAIN)
    {
       return FTW_EINVAL;
    }
...
#ifdef FTW_EXT_FLASH_FWUP_PULL_METHOD
FTW_OTA_FWUP_CONF_T OtaFwupConfParam;
UINT8
AppS2wCmd_OtaFwupConf(UINT8* ptr)
{
    UINT8* p;
     ...
     */
     ...
     switch(id)
     {
        ...
        case FTW_OTAFWUP_SRVRPORT:
           ...
           break;
        case FTW_OTAFWUP_PROXY_PRESENT:
        ...
        case FTW_OTAFWUP_PROXY_SRVRIP:
            ...
            break;
        case FTW_OTAFWUP_PROXY_SRVRPORT:
            if(! OtaFwupConfParam.proxyFlag)
            {
                return FTW_EINVAL;
            }
        ...
        case FTW_OTAFWUP_CERT_NAME:
            ...
            if(pxCert != NULL)
            {
                OtaFwupConfParam.certAddr = pxCert->pCertAddress;
            }
            else
            {
        ...
        break;
        case FTW_OTAFWUP_APP_IMAGE_URL:
			/* Configure wlan url */
            if(OtaFwupConfParam.urls[FTW_OTAFU_URL_FW_IMAGE] != NULL)
                gsn_free(OtaFwupConfParam.urls[FTW_OTAFU_URL_FW_IMAGE]);
            OtaFwupConfParam.urls[FTW_OTAFU_URL_FW_IMAGE]= gsn_malloc(strlen((const char *)p)+1);
            strncpy((INT8 *)OtaFwupConfParam.urls[FTW_OTAFU_URL_FW_IMAGE],(const char *)p,strlen((const char *)p)+1);
        break;
		case FTW_OTAFWUP_APP_SIGN_URL:
			/* Configure wlan url */
            if(OtaFwupConfParam.urls[FTW_OTAFU_URL_SIGN_IMAGE] != NULL)
                gsn_free(OtaFwupConfParam.urls[FTW_OTAFU_URL_SIGN_IMAGE]);
            OtaFwupConfParam.urls[FTW_OTAFU_URL_SIGN_IMAGE]= gsn_malloc(strlen((const char *)p)+1);
            strncpy((INT8 *)OtaFwupConfParam.urls[FTW_OTAFU_URL_SIGN_IMAGE],(const char *)p,strlen((const char *)p)+1);
        break;
		case FTW_OTAFWUP_INFO_URL:
			/* Configure informational xml url */
			if(OtaFwupConfParam.urls[FTW_OTAFU_URL_XML] != NULL)
                gsn_free(OtaFwupConfParam.urls[FTW_OTAFU_URL_XML]);
            OtaFwupConfParam.urls[FTW_OTAFU_URL_XML]= gsn_malloc(strlen((const char *)p)+1);
            strncpy((INT8 *)OtaFwupConfParam.urls[FTW_OTAFU_URL_XML],(const char *)p,strlen((const char *)p)+1);
			break;
		case FTW_OTAFWUP_SRVR_HOST_NAME:
			...
			break;
		case FTW_OTAFWUP_APP_ENABLE_DNS_ERR:
...
AppS2wCmd_OtaFwupExtFlashStart(UINT8* ptr)
{
              ...
              && (OtaFwupConfParam.fwSrvHostName == NULL))
     {
         ...
         OtaFwupConfParam.sockAddr.addr.ipv4.sin_addr.s_addr = htonl(FTW_DFLT_OTAFU_PULL_SRVR_IP);
#ifdef FTW_IPv6_SUPPORT
		OtaFwupConfParam.sockAddr.addr.ipv4.sin_family = AF_INET6;
#else
		OtaFwupConfParam.sockAddr.addr.ipv4.sin_family = AF_INET;
#endif
        OtaFwupConfParam.sockAddr.ss_port = FTW_DFLT_OTAFU_PULL_SRVR_PORT;
        OtaFwupConfParam.proxyFlag = FTW_DFLT_OTAFU_PULL_PROXY_SRVR;
#ifdef FTW_IPv6_SUPPORT		
		...
		OtaFwupConfParam.proxySockAddr.addr.ipv4.sin_family = AF_INET;
         ...
         OtaFwupConfParam.sslFlag = FTW_DFLT_OTAFU_PULL_SSL;
        OtaFwupConfParam.proxySockAddr.addr.ipv4.sin_addr.s_addr = htonl(FTW_DFLT_OTAFU_PULL_PROXY_SRVR_IP);
         OtaFwupConfParam.proxySockAddr.ss_port = FTW_DFLT_OTAFU_PULL_PROXY_SRVR_PORT;
         OtaFwupConfParam.urls[FTW_OTAFU_URL_FW_IMAGE]= gsn_malloc(sizeof(FTW_DFLT_OTAFU_PULL_BIN_URL)+1);
         memcpy((INT8 *)OtaFwupConfParam.urls[FTW_OTAFU_URL_FW_IMAGE],FTW_DFLT_OTAFU_PULL_BIN_URL,sizeof(FTW_DFLT_OTAFU_PULL_BIN_URL));
		if((httpConfAtCmd != 1) &&(!httpConfInfo.reqHeaderCount))
		{
			memset(&headerTypes[0], GSN_HTTP_HEADER_INVAL, sizeof(headerTypes));
			memset(&headerValues[0], 0, sizeof(headerValues));
...
AppS2wCmd_DnsServerSet(UINT8 *ptr)
{
 ...
 while(((p=(UINT8*)AppS2wParse_NextParamGet(&ptr)) != NULL) && (index < 2))
 {
    ...
    if (status == FTW_SUCCESS)
    {
      ...
      FTW_DNS_CLIENT_FALGS |= FTW_DNSv4_SERVER_SET;
    }
      ...
      switch(IPv6AddrType)
      {
        ...
        case GSN_IPv6_GLOBAL:
           ...
           /* Clear the previous DNSv6 server */
            ...
            if (status != FTW_SUCCESS)
              return FTW_FAILURE;
            FTW_DNS_CLIENT_FALGS |= FTW_DNSv6_SERVER_SET;
            break;
    ...
    FTW_DNS_CLIENT_FALGS |= FTW_DNS_CLIENT_ENABLED;
#ifdef FTW_DHCP_SERVER_ENABLE
    /* Restart DHCP server */
    if(dhcpServerFlags & FTW_DHCP_SRVR_ENABLED)
    {
...
AppS2wCmd_DnsLookup(UINT8 *ptr)
{
   ...
   p=(UINT8*)AppS2wParse_NextParamGet(&ptr);
    ...
    retries = (retries) ? retries : FTW_DNS_REQ_DEF_RETRIES;
    ...
    timeout = (timeout) ? timeout : FTW_DNS_REQ_DEF_TIME_OUT;
   ...
   status = GsnNwIf_DnsHostByNameGet(&s2wappMainTaskCtxt->if0.nwifCtx, (UINT8*)temp, &hostIpAddress, timeout);
    ...
    if (status != FTW_SUCCESS || hostIpAddress == 0)
      return FTW_FAILURE;
...
AppS2wCmd_DnsLookupv6(UINT8 *ptr)
{
   ...
   NXD_ADDRESS     host_ipduo_address;
    ...
    if(!(FTW_DNS_CLIENT_FALGS & FTW_DNS_CLIENT_ENABLED))
    {

      FTW_DNS_CLIENT_FALGS |= FTW_DNS_CLIENT_ENABLED;
    }
    
    if(!(FTW_DNS_CLIENT_FALGS & FTW_DNSv6_SERVER_SET))
    {
   ...
   p=(UINT8*)AppS2wParse_NextParamGet(&ptr);
    ...
    timeout = FTW_DNS_REQ_DEF_TIME_OUT;
    p=(UINT8*)AppS2wParse_NextParamGet(&ptr);
    if(p != NULL)
    {  
      status = AppS2wParse_Int(p, &timeout);
...
AppS2wCmd_PowerSaveStby(UINT8 *ptr)
{
    ...
    FTW_ALARM_INFO_T alarm;
    UINT8 *p;
    memset(&alarm,0,sizeof(FTW_ALARM_INFO_T));
    p = AppS2wParse_NextParamGet(&ptr);
    if (!p)
    {
        return FTW_EINVAL;
 ...
 * @retval FTW_SUCCESS - operation successful.
 * @retval FTW_LINK_LOST_EVT - operation failure- not connected to wlan.
 ******************************************************************/
PUBLIC UINT8
	...
	*/
	...
	)
	{
		...
		*/
		...
		if(s2wappMainTaskCtxt->appNwConnMgrCtx.config.roamingCfgParams.roamEnable == 1 &&
			...
			AppS2wRoam_SatusNotify(FTW_ENEWIP,FTW_ASYNC_DHCP_NEW_IP);
		else		
#endif
	 ...
	 {	
   	 	...
   	 	if((!s2wAutoConnState ) &&( !profile_params.autoConnect))
    	{
            ...
            {
                 S2w_Printf("\r\n");
                 AppS2wProcess_AsyncStatusNotify(FTW_ASYNC_ASSO_SUCCESS_MSG, 0);
            }
            else
#endif
            {
                 AppS2wProcess_StatusNotify(FTW_ASSO_SUCCESS_MSG, 0);
        	}
		}
		GsnOsal_SemRelease(&s2wSyncSemID);
	}
	...
	if(s2wCurrent.ncmAutoLvl)
	{
	   ...
	   */
	   ...
	   {
		   ncmautoconnectcount = FTW_AUTO_CONCT_L4RETRY_INFINITE;
	   }
	   else
	   {
    ...
    switch(status)
    {
        case GSN_NCM_NOTIF_ID_L2_CONNECTED:
        {
            ...
            if(ncmAutoMode == 0) // client case only
            {
                ...
                {
                    appRtcMemData.KeepAliveIntrvl = FTW_DEFAULT_KEEP_ALIVE_INTRVL;
                }
                FTW_PS_pollIntervelSet(appRtcMemData.KeepAliveIntrvl);
            }
#endif
           ...
           // if auto connection is up then make the gpio 19 low
            if((s2wAutoConnState ) && (s2wIsAutoconnected))
            {
		  	...
		  	if(s2wappMainTaskCtxt->appNwConnMgrCtx.config.roamingCfgParams.roamEnable == 1 &&
		   	  s2wappMainTaskCtxt->appNwConnMgrCtx.config.roamingCfgParams.maintainL4Con == 1)			   	
		  	{	
				AppS2wRoam_SatusNotify(FTW_DISASSO_EVT,FTW_ASYNC_DISASSO_EVT);
		 	}
		  	else
#endif                          
			{
		...
		case GSN_NCM_NOTIF_ID_L3_CONNECT_FAIL:
    	{
			...
			if(s2wappMainTaskCtxt->appNwConnMgrCtx.config.roamingCfgParams.roamEnable == 1 &&
		   		s2wappMainTaskCtxt->appNwConnMgrCtx.config.roamingCfgParams.maintainL4Con == 1)			   	
		  	{
				AppS2wRoam_SatusNotify(FTW_ENOIP,FTW_ASYNC_DHCP_FAIL_EVT);			
		  	}
		  	else	
#endif                          
			{
...
AppS2wCmd_NcmAuto(UINT8* ptr)
{
    ...
    if(ncmAutoMode) // start the AP with the nvds info..
    {
        ...
        if(s2wCurrent.mode == FTW_WLANDATA_MODE_PROV_AP)
        {
             gNwConnConfig.wlanCfg.apSpecificCfg.beaconInterval = FTW_DEFAULT_GRP_PROV_BEACON_INTERVAL;
             gNwConnConfig.wlanCfg.ssid.length = sizeof(FTW_DEFAULT_GRP_PROV_SSID) - 1;;
             memcpy(gNwConnConfig.wlanCfg.ssid.array, FTW_DEFAULT_GRP_PROV_SSID,
             ...
             gNwConnConfig.wlanCfg.channel[0] = FTW_DEFAULT_GRP_PROV_CHANNEL;
             gNwConnConfig.wlanCfg.channel[1] = 0;
             S2w_getApSecurityConfig(&gNwConnConfig.wlanCfg,FTW_DEFAULT_GRP_PROV_SECURITY);

             gNwConnConfig.ipConfig.flag =  GSN_NWIF_IPCONFIG_FLAG_IPV4_INIT;
             gNwConnConfig.ipConfig.ipv4.staticIp = 1;//nwMib.staticIp;
             gNwConnConfig.ipConfig.ipv4.ipAddr = FTW_DEFAULT_GRP_PROV_IP_ADDRESS;
             gNwConnConfig.ipConfig.ipv4.subnetMask = FTW_DEFAULT_GRP_PROV_SUBNET_ADDRESS;
             gNwConnConfig.ipConfig.ipv4.gatewayIpAddr = FTW_DEFAULT_GRP_PROV_GATE_ADDRESS;
        ...
        else
        {
			gNwConnConfig.wlanCfg.apSpecificCfg.beaconInterval = s2wCurrent.lmtdApConf.beaconInterval;
				gNwConnConfig.wlanCfg.apSpecificCfg.hiddenSsid = s2wCurrent.ncmAutoConf[FTW_NCM_HIDDENSSID_VALUE];
			
			if(s2wCurrent.ncmAutoConf[FTW_NCM_DTIMPERIOD_VALUE])
			{
				gNwConnConfig.wlanCfg.apSpecificCfg.dtimPeriod= s2wCurrent.ncmAutoConf[FTW_NCM_DTIMPERIOD_VALUE];
			}
			else
			{
				gNwConnConfig.wlanCfg.apSpecificCfg.dtimPeriod= FTW_NCM_DFLT_DTIMPERIOD_VALUE;
			}
			
			if(s2wCurrent.ncmAutoConf[FTW_NCM_INACTIVITY_TMEOUT_VALUE])
			{
				gNwConnConfig.inactivityTimeoutInSec= s2wCurrent.ncmAutoConf[FTW_NCM_INACTIVITY_TMEOUT_VALUE];
			}
			else
			{
				gNwConnConfig.inactivityTimeoutInSec= FTW_NCM_DFLT_INACTIVITY_TMEOUT_VALUE;
			}			
			if(s2wCurrent.ncmAutoConf[FTW_NCM_GRP_KEY_UPDATE_INTERVAL])
			{
				gNwConnConfig.groupKeyUpdateTime= s2wCurrent.ncmAutoConf[FTW_NCM_GRP_KEY_UPDATE_INTERVAL];
			}
			else
			{
				gNwConnConfig.groupKeyUpdateTime = FTW_NCM_DFLT_GRP_KEY_UPDATE_INTERVAL;
			}
	...
	{
			memcpy((INT8 *)gNwConnConfig.wlanCfg.bssid.array,s2wCurrent.autoWlandata.bssid,FTW_MAC_ADDR_SIZE);
    ...
    if( s2wCurrent.security.eap.useCaCert ||
      ...
      ( 26 == s2wCurrent.security.eap.innerAuth )))
    {
			index = S2wCert_findFile(FTW_CERT_NAME_CA);
        if(index != -1)
        {
		  		gNwConnConfig.wlanCfg.securityCfg.eapConfig.pCaCert = (char*)pxCert->pCertAddress;
        }
    }
		index = S2wCert_findFile(FTW_CERT_NAME_CLIENT);
    if(index != -1)
    {
		  gNwConnConfig.wlanCfg.securityCfg.eapConfig.pClientCert  = (char*)pxCert->pCertAddress;
    }
		index = S2wCert_findFile(FTW_CERT_NAME_KEY);
    if(index != -1)
    {
		  gNwConnConfig.wlanCfg.securityCfg.eapConfig.pPrivateKey =(char*) pxCert->pCertAddress;
    }
#else
	if(s2wCurrent.eapCertTimeValidChkDisbl)
	{
      ...
      ( 26 == s2wCurrent.security.eap.innerAuth )))
    {
        ...
        if(pxCert != NULL)
        {
            gNwConnConfig.wlanCfg.securityCfg.eapConfig.pCaCert = (char*)pxCert->pCertAddress + 2;
            gNwConnConfig.wlanCfg.securityCfg.eapConfig.caCertLen = *(UINT16 *)pxCert->pCertAddress;
        }
    }
    pxCert = FTW_CERT_searchRamTable(FTW_CERT_NAME_CLIENT);
    if(pxCert == NULL)
    ...
    if(pxCert != NULL)
    {
        gNwConnConfig.wlanCfg.securityCfg.eapConfig.pClientCert  = (char*)pxCert->pCertAddress + 2;
        gNwConnConfig.wlanCfg.securityCfg.eapConfig.clientCertLen = *(UINT16 *)pxCert->pCertAddress;
    }
    pxCert = FTW_CERT_searchRamTable(FTW_CERT_NAME_KEY);
    if(pxCert == NULL)
    {
    ...
    if(pxCert != NULL)
    {
	    gNwConnConfig.wlanCfg.securityCfg.eapConfig.pPrivateKey = (char*)pxCert->pCertAddress + 2;
        gNwConnConfig.wlanCfg.securityCfg.eapConfig.privateKeyLen = *(UINT16 *)pxCert->pCertAddress;
    }
#endif
    if(s2wCurrent.ncmAutoConf[FTW_NCM_CPUWAIT_PERIOD])
    {
        gNwConnConfig.retryPeriodConfig.cpuWaitPeriod = s2wCurrent.ncmAutoConf[FTW_NCM_CPUWAIT_PERIOD];
    }
    else
    {
        gNwConnConfig.retryPeriodConfig.cpuWaitPeriod = FTW_NCM_DFLT_CPUWAIT_PERIOD;
    }
    if(s2wCurrent.ncmAutoConf[FTW_NCM_SCANKNOWN_CH_PERIOD])
    {
        gNwConnConfig.retryPeriodConfig.scanKnownChRetryIntrvl = s2wCurrent.ncmAutoConf[FTW_NCM_SCANKNOWN_CH_PERIOD];
    }
    else
    {
        gNwConnConfig.retryPeriodConfig.scanKnownChRetryIntrvl = FTW_NCM_DFLT_KNOWNCH_SCAN_PERIOD;
    }
    if(s2wCurrent.ncmAutoConf[FTW_NCM_SCANSPEC_CH_PERIOD])
    {
        gNwConnConfig.retryPeriodConfig.scanSpecChRetryIntrvl = s2wCurrent.ncmAutoConf[FTW_NCM_SCANSPEC_CH_PERIOD];
    }
    else
    {
        gNwConnConfig.retryPeriodConfig.scanSpecChRetryIntrvl = FTW_NCM_DFLT_SPECCH_SCAN_PERIOD;
    }
    if(s2wCurrent.ncmAutoConf[FTW_NCM_SCANALL_CH_PERIOD])
    {
        gNwConnConfig.retryPeriodConfig.scanAllChRetryIntrvl = s2wCurrent.ncmAutoConf[FTW_NCM_SCANALL_CH_PERIOD];
    }
    else
    {
        gNwConnConfig.retryPeriodConfig.scanAllChRetryIntrvl = FTW_NCM_DFLT_ALLCH_SCAN_PERIOD;
    }
    if(s2wCurrent.ncmAutoConf[FTW_NCM_L3CONNECT_PERIOD])
    {
        gNwConnConfig.retryPeriodConfig.l3ConnectRetryIntrvl = s2wCurrent.ncmAutoConf[FTW_NCM_L3CONNECT_PERIOD];
    }
    else
    {
        gNwConnConfig.retryPeriodConfig.l3ConnectRetryIntrvl = FTW_NCM_DFLT_L3CONNECT_PERIOD;
    }
    if(s2wCurrent.ncmAutoConf[FTW_NCM_SCANKNOWN_CH_COUNT])
    {
        gNwConnConfig.retryCountsConfig.maxScanKnownChCnt = s2wCurrent.ncmAutoConf[FTW_NCM_SCANKNOWN_CH_COUNT];
    }
    else
    {
        gNwConnConfig.retryCountsConfig.maxScanKnownChCnt = FTW_NCM_DFLT_KNOWNCH_SCAN_COUNT;
    }
    if(s2wCurrent.ncmAutoConf[FTW_NCM_SCANSPEC_CH_COUNT])
    {
        gNwConnConfig.retryCountsConfig.maxScanSpecChCnt = s2wCurrent.ncmAutoConf[FTW_NCM_SCANSPEC_CH_COUNT];
    }
    else
    {
        gNwConnConfig.retryCountsConfig.maxScanSpecChCnt = FTW_NCM_DFLT_SPECCH_SCAN_COUNT;
    }
    if(s2wCurrent.ncmAutoConf[FTW_NCM_SCANALL_CH_COUNT])
    {
        gNwConnConfig.retryCountsConfig.maxScanAllChCnt = s2wCurrent.ncmAutoConf[FTW_NCM_SCANALL_CH_COUNT];
    }
    else
    {
        gNwConnConfig.retryCountsConfig.maxScanAllChCnt = FTW_NCM_DFLT_ALLCH_SCAN_COUNT;
    }
    if(s2wCurrent.ncmAutoConf[FTW_NCM_L3CONNECT_COUNT])
    {
        gNwConnConfig.retryCountsConfig.maxL3ConnectCnt = s2wCurrent.ncmAutoConf[FTW_NCM_L3CONNECT_COUNT];
        gNwConnConfig.dhcpV4Cfg.dhcpMaxRetryCnt = 6;
        gNwConnConfig.dhcpV4Cfg.radioMode= s2wCurrent.ncmAutoConf[FTW_NCM_DHCP_RADIO_MODE];
        gNwConnConfig.dhcpV4Cfg.requestLeaseTime = 
            s2wCurrent.ncmAutoConf_extn[FTW_NCM_EXTN_DHCP_RQST_LEASE - FTW_NCM_EXTN_EXTN_BASE];
        gNwConnConfig.dhcpV4Cfg.dhcpRetryTimeout= 
            s2wCurrent.ncmAutoConf_extn[FTW_NCM_EXTN_DHCP_RETRANS_TIMEOUT - FTW_NCM_EXTN_EXTN_BASE];

    }
    else
    {
        gNwConnConfig.retryCountsConfig.maxL3ConnectCnt = FTW_NCM_DFLT_L3CONNECT_COUNT;
        gNwConnConfig.dhcpV4Cfg.dhcpMaxRetryCnt = 6; //s2wCurrent.ncmAutoConf[FTW_NCM_L3CONNECT_COUNT];         
        gNwConnConfig.dhcpV4Cfg.radioMode= s2wCurrent.ncmAutoConf[FTW_NCM_DHCP_RADIO_MODE];         
        gNwConnConfig.dhcpV4Cfg.requestLeaseTime = 
            s2wCurrent.ncmAutoConf_extn[FTW_NCM_EXTN_DHCP_RQST_LEASE - FTW_NCM_EXTN_EXTN_BASE];        
        gNwConnConfig.dhcpV4Cfg.dhcpRetryTimeout= 
...
#ifdef FTW_NCM_ROAMING_SUPPORT 
	if(s2wCurrent.ncmAutoConf[FTW_NCM_ROAM_ENABLE_ID])
		gNwConnConfig.roamingCfgParams.roamEnable = FTW_NCM_ROAM_DFLT_ROAMEN;	
	else
		gNwConnConfig.roamingCfgParams.roamEnable = FTW_NCM_ROAM_DFLT_ROAMDIS;
	if(s2wCurrent.ncmAutoConf[FTW_NCM_ROAM_LOWER_THID] )
		gNwConnConfig.roamingCfgParams.rssiLowerTh = s2wCurrent.ncmAutoConf[FTW_NCM_ROAM_LOWER_THID];
	else
		gNwConnConfig.roamingCfgParams.rssiLowerTh = FTW_NCM_ROAM_DFLT_RSSILOWERTH;
    if(s2wCurrent.ncmAutoConf[FTW_NCM_ROAM_HIGHER_THID]  )
		gNwConnConfig.roamingCfgParams.rssiHigherTh = s2wCurrent.ncmAutoConf[FTW_NCM_ROAM_HIGHER_THID];
	else
		gNwConnConfig.roamingCfgParams.rssiHigherTh = FTW_NCM_ROAM_DFLT_RSSIHIGHERTH;
    if(s2wCurrent.ncmAutoConf[FTW_NCM_ROAM_RSSICHKINTERVAL_ID])
		gNwConnConfig.roamingCfgParams.rssiLevelChkInterval= (s2wCurrent.ncmAutoConf[FTW_NCM_ROAM_RSSICHKINTERVAL_ID] * TIMER_INTERRUPT_1_MS);
	else
		gNwConnConfig.roamingCfgParams.rssiLevelChkInterval = FTW_NCM_ROAM_DFLT_RSSICHKINTERVAL;
	if(s2wCurrent.ncmAutoConf[FTW_NCM_ROAM_THCROSSEDCNT_ID])
		gNwConnConfig.roamingCfgParams.thresholdCrossedCnt= s2wCurrent.ncmAutoConf[FTW_NCM_ROAM_THCROSSEDCNT_ID];
	else
	   gNwConnConfig.roamingCfgParams.thresholdCrossedCnt = FTW_NCM_ROAM_DFLT_THCROSSEDCNT;
	if(s2wCurrent.ncmAutoConf[FTW_NCM_ROAM_MAINTAINL3CON_ID] == 0 )
		gNwConnConfig.roamingCfgParams.maintainL3Con= s2wCurrent.ncmAutoConf[FTW_NCM_ROAM_MAINTAINL3CON_ID];
	else
	   gNwConnConfig.roamingCfgParams.maintainL3Con = FTW_NCM_ROAM_DFLT_MAINTAINL3CON;
	if(s2wCurrent.ncmAutoConf[FTW_NCM_ROAM_MAINTAINL4CON_ID])
		gNwConnConfig.roamingCfgParams.maintainL4Con= s2wCurrent.ncmAutoConf[FTW_NCM_ROAM_MAINTAINL4CON_ID];
	else
	   gNwConnConfig.roamingCfgParams.maintainL4Con = FTW_NCM_ROAM_DFLT_MAINTAINL4CON;
	if(s2wCurrent.ncmAutoConf[FTW_NCM_ROAM_SCANRETRYCNT_ID])
		gNwConnConfig.roamingCfgParams.scanRetryCnt= s2wCurrent.ncmAutoConf[FTW_NCM_ROAM_SCANRETRYCNT_ID];
	else
	   gNwConnConfig.roamingCfgParams.scanRetryCnt = FTW_NCM_ROAM_DFLT_SCANRETRYCOUNT;
	if(s2wCurrent.ncmAutoConf[FTW_NCM_ROAM_SCANPAUSETIME_ID])
		gNwConnConfig.roamingCfgParams.scanPauseTimeMs= s2wCurrent.ncmAutoConf[FTW_NCM_ROAM_SCANPAUSETIME_ID];
	else
	   gNwConnConfig.roamingCfgParams.scanPauseTimeMs = FTW_NCM_ROAM_DFLT_SCANPUASETIME;
...
AppS2wCmd_NcmAutoConf(UINT8* ptr)
{
    ...
    UINT32 value;
     ...
     status = AppS2wParse_Int(p, &id);
     if ((status != FTW_SUCCESS)||((id > 5)&&(id < 8))
#ifdef FTW_NCM_ROAMING_SUPPORT 				 
         || (id > 27)
#else
         || (id > 15 && (id < 25 || id > 27))
...
#ifdef FTW_NCM_ROAMING_SUPPORT 				 
	 if((id == FTW_NCM_ROAM_HIGHER_THID || id == FTW_NCM_ROAM_LOWER_THID) && (*p == '-'))
	 {
		status = AppS2wParse_Int(p+1, &value);
		s2wCurrent.ncmAutoConf[id] = (unsigned char)(value * -1);
	 }
	 else if((id == FTW_NCM_ROAM_HIGHER_THID || FTW_NCM_ROAM_LOWER_THID == 3) && (*p != '-'))		
	 {
		return FTW_EINVAL;
	 }
	else
     ...
     if(id >= 26  && id <= 27)
     {
        ...
        s2wCurrent.ncmAutoConf_extn[id - FTW_NCM_EXTN_EXTN_BASE] = value;        
     }
     else 
     {
...
AppS2wCmd_Wrate(UINT8 *ptr)
{
    ...
    else
    {
		...
		status = AppS2wParse_Int(p, &val);
        if((status != FTW_SUCCESS)||( FTW_SUCCESS != S2wRateValidation(val)))
            {
			return FTW_EINVAL;
			    }
...
AppS2wCmd_Nauto(UINT8 *ptr)
{
    ...
    if(params.mode == 0)//client-tcp/udp
    {
       ...
       p = AppS2wParse_NextParamGet(&temp);
        ...
        {
           /* Can be URL  */
          if((s2wLinkState != FTW_LINK_STATE_DISCONNECTED)  && (L3ConnectStatus == TRUE))
          {
    ...
    if((params.mode == 0) &&(params.proto == FTW_NETDATA_PROTO_UDP))//client-/udp
    {
	    p = AppS2wParse_NextParamGet(&temp);
	    if ( (p) &&(*p != '\0'))
	    {
...
AppS2wCmd_IdOutput(UINT8 *ptr)
{
    ...
    switch (which)
    {
        ...
        case FTW_ID_HW_VERSION:
            {
                S2w_Printf("\r\n%s", FTW_HW_VERSION);
            }
            break;

        case FTW_ID_SW_VERSION:
            S2w_Printf("\r\n%s", FTW_SW_VERSION);
            break;
...
AppS2wCmd_Ping(UINT8* ptr)
{
  UINT8 *p,status=FTW_FAILURE;
  FTW_PINGDATA_T pingParams;
    ...
    memset (&pingParams, 0, sizeof (FTW_PINGDATA_T));
    p = AppS2wParse_NextParamGet(&ptr);
    if (!p)
    {
        return status;
    ...
    memcpy(&s2wappMainTaskCtxt->pingParams,&pingParams,sizeof(FTW_PINGDATA_T));
...
AppS2wCmd_GoOnline(UINT8 *ptr)
{
    ...
    if (!AppS2wHal_NetIsCidOpen(curCid)
        && (s2wCurrent.autoNetdata.proto != FTW_NETDATA_PROTO_TCP
...
skipArgument:	
	memset(&s2wCidList[validCid].stats,0,sizeof(FTW_TPSTATS_T));	
...
AppS2wCmd_IpMulticastJoin(UINT8 *ptr)
{
  ...
  status = AppS2wParse_Ip(p, (UINT8 *)&ip);
   ...
   if(nx_igmp_multicast_join(&s2wappMainTaskCtxt->if0.nwifCtx.ipHndl,ip) != FTW_SUCCESS)
     return FTW_FAILURE;
...
AppS2wCmd_IpMulticastLeave(UINT8 *ptr)
{
  ...
  status = AppS2wParse_Ip(p, (UINT8 *)&ip);
   ...
   if(nx_igmp_multicast_leave(&s2wappMainTaskCtxt->if0.nwifCtx.ipHndl,ip) != FTW_SUCCESS)
     return FTW_FAILURE;
...
AppS2wCmd_Wwps(UINT8 *ptr)
{
    ...
    if(s2wCurrent.mode == 3)
    {
        ...
        {
            status = FTW_NET_WPS_associate(&params, &ap_ssid,0);
        }
    }
    else
#endif
 ...
 * @return FTW_SUCCESS - operation successful.
 * @return EMU STATUS   
 * @return FTW_EINVAL   - invalid input
 ******************************************************************/
...
AppS2wCmd_EmuGetStatus(UINT8 *ptr)
{
    ...
    if (ptr[0] == '\0')
    {
        ...
        if(GSN_EMU_STATUS_SUCCESS == status)
        {
           ...
           status =( GSN_EMU_STATUS_T )FTW_SUCCESS;
        }
    }
    else 
    {
        status = (GSN_EMU_STATUS_T) FTW_EINVAL;
    }
    return status ;
...
#define FTW_ADC_SINGLE 1
#define FTW_ADC_CONTI  0
#define FTW_ADC_PERIODIC 2
 ...
 *
 * @return FTW_SUCCESS - operation successful.
 * @retval FTW_EINVAL  - oeration failure-invalid command.
 ******************************************************************/
PRIVATE UINT8
AppS2wCmd_AdcConf(UINT8 *ptr)
{
...
AppS2wCmd_AdcRead(UINT8 *ptr)
{
    ...
    if (status != FTW_SUCCESS || (size >256))
    {
        return FTW_EINVAL;
    }
    ...
    if (status != FTW_SUCCESS || (channel > 7))
    {
        return FTW_EINVAL;
    }
...
AppS2wCmd_I2cStart(UINT8 *ptr)
{
	...
	appCtx.i2cStatus = FTW_I2C_STARTED;
    return status;
...
AppS2wCmd_I2cWrite(UINT8 *ptr)
{
    ...
    if(appCtx.i2cStatus != FTW_I2C_STARTED)
    {
		return FTW_EINVAL;
	}
    p = AppS2wParse_NextParamGet(&ptr);
...
AppS2wCmd_I2cStop(UINT8 *ptr)
{
    ...
    appCtx.i2cStatus = FTW_I2C_STOPPED;
    return status;
}
...
AppS2wCmd_WeapConf(UINT8 *ptr)
{
    ...
    AppS2wParse_Int(p, &s2wCurrent.security.eap.outerAuth);
       ...
       if(!( FALSE
#if defined(FTW_SECURITY_ENTERPRISE_FAST_MSCHAPV2_AVAILABLE) || defined(FTW_SECURITY_ENTERPRISE_FAST_GTC_AVAILABLE)
          || s2wCurrent.security.eap.outerAuth == GSN_EAP_TYPE_FAST
#endif
#ifdef FTW_SECURITY_ENTERPRISE_TTLS_AVAILABLE
          || s2wCurrent.security.eap.outerAuth == GSN_EAP_TYPE_TTLS
   #endif
#if defined(FTW_SECURITY_ENTERPRISE_PEAP_V0_AVAILABLE)|| defined(FTW_SECURITY_ENTERPRISE_PEAP_V1_AVAILABLE)
          || s2wCurrent.security.eap.outerAuth == GSN_EAP_TYPE_PEAP
   #endif
#if defined(FTW_SECURITY_ENTERPRISE_TLS_AVAILABLE)
    ...
    if(s2wCurrent.security.eap.innerAuth == 253)
    {
        ...
        if(!(FALSE
   #if defined(FTW_SECURITY_ENTERPRISE_TLS_AVAILABLE) || defined(FTW_SECURITY_ENTERPRISE_FAST_MSCHAPV2_AVAILABLE) || defined(FTW_SECURITY_ENTERPRISE_PEAP_V0_AVAILABLE)|| defined(FTW_SECURITY_ENTERPRISE_TTLS_AVAILABLE)
//#ifdef  GSN_SECURITY_ENTERPRISE_TTLS_AVAILABLE
           || s2wCurrent.security.eap.innerAuth == GSN_EAP_TYPE_MSCHAPV2
#endif
   #if defined(FTW_SECURITY_ENTERPRISE_FAST_GTC_AVAILABLE)|| defined(FTW_SECURITY_ENTERPRISE_PEAP_V1_AVAILABLE)
...
//        s2wCurrent.security.eap.innerAuth = GSN_EAP_TYPE_MSCHAPV2;
    ...
    len = strlen((char *)p);
    if(len > FTW_MAX_EAP_USR_NAME_LEN)
    {
        return FTW_EINVAL;
    }
    memset(s2wCurrent.security.eap.usrName, 0, FTW_MAX_EAP_USR_NAME_LEN);
...
AppS2wExternalFlashData(UINT8 ch1)
{
    ...
    if( extFlashFileSize == 0)
      ...
      while(extFlashFileSize)
      {
          ...
          /* store incoming byte in temparory bufer */
          FTW_SERIAL_get(FTW_SERIAL_0, extFlashGetDatabuf, rwSize);
...
AppS2wCmd_CoapSend(UINT8* ptr)
{
	...
	GSN_COAP_CONN_PARAMS_T* coapConnParams;
	FTW_COAP_OPTION_T * start = optionHead;
...
#ifndef FTW_DUAL_INTERFACE_SUPPORT
    		S2w_Printf("\r\n");
    		FTW_SERIAL_put(FTW_SERIAL_0, respBuf,numBytes);
#else
        FTW_SERIAL_put(FTW_SERIAL_1, respBuf,numBytes);
#endif
    	}
...
AppS2wCmd_CoapSetOption(UINT8* ptr)
{
			...
			key == 35 )
	{
		...
		if( q)
		{
			...
			{
				node = (FTW_COAP_OPTION_T*)gsn_malloc(sizeof(FTW_COAP_OPTION_T));
...
AppS2wCmd_CoapRemoveOption(UINT8* ptr)
{
	UINT8* p;
	UINT32 optionKey;
	FTW_COAP_OPTION_T * prev = optionHead;
	FTW_COAP_OPTION_T * curr = optionHead;
	FTW_COAP_OPTION_T * temp = NULL;
...
AppS2wHal_CoapContentDataGet( UINT8 hCid)
{
	...
	FTW_SERIAL_get(FTW_SERIAL_0, (UINT8 *)requestBuf,dataLen);
...
UINT8 App_ClkTuneCmd(UINT8 *ptr)
{
    ...
    if(AppS2wParse_Int(p, &period) != FTW_SUCCESS)
    {
        return FTW_FAILURE;
    }
    if(start == TRUE)
