...
#ifdef 	FTW_GSLINK_SEND_FULL_URI
UINT8 *
App_McuUserURIGet();
#endif
/*******************************************************************************
 ...
 * @param cid - connectio identifier
 * @return FTW_SUCESS - on suceesful initialisation
 ********************************************************************/
UINT8
...
//		GsnOsal_SemAcquire(&s2wSyncSemID, GSN_OSAL_WAIT_FOREVER);
	...
	{
		headerLen=sprintf((char *)header,"%cG%d%04d", FTW_ESC,FTW_XML_ATTRIBUTE_ID,(xmlCbparams->attrNameLen+xmlCbparams->attrvalLen+ 1));
#ifndef FTW_DUAL_INTERFACE_SUPPORT
		FTW_SERIAL_put(FTW_SERIAL_0,  header, headerLen);
		FTW_SERIAL_put(FTW_SERIAL_0,  xmlCbparams->attrNamePtr, xmlCbparams->attrNameLen);					
		FTW_SERIAL_put(FTW_SERIAL_0,  &delim,1);
		FTW_SERIAL_put(FTW_SERIAL_0,  xmlCbparams->attrValPtr, xmlCbparams->attrvalLen);			
#else
		FTW_SERIAL_put(FTW_SERIAL_1, header, headerLen);
		FTW_SERIAL_put(FTW_SERIAL_1, xmlCbparams->attrNamePtr, xmlCbparams->attrNameLen);
		FTW_SERIAL_put(FTW_SERIAL_1, delim,1);
		FTW_SERIAL_put(FTW_SERIAL_1, xmlCbparams->attrValPtr, xmlCbparams->attrvalLen);
#endif
	}
	else if(xmlCbparams->status == GSN_XML_COMPLEX_ELEMENT)
	{
		headerLen=sprintf((char *)header,"%cG%d%04d", FTW_ESC,FTW_XML_COMPLEX_TAG_START_ID,xmlCbparams->tagLen);
#ifndef FTW_DUAL_INTERFACE_SUPPORT
		FTW_SERIAL_put(FTW_SERIAL_0,  header, headerLen);
		FTW_SERIAL_put(FTW_SERIAL_0,  xmlCbparams->tagPtr, xmlCbparams->tagLen);
#else
		FTW_SERIAL_put(FTW_SERIAL_1, header, headerLen);
		FTW_SERIAL_put(FTW_SERIAL_1, xmlCbparams->tagPtr, xmlCbparams->tagLen);
#endif
	}
	else if(xmlCbparams->status == GSN_OPERATION_IN_PROGRESS)
	{
		headerLen=sprintf((char *)header,"%cG%04d", FTW_ESC,(xmlCbparams->tagLen+xmlCbparams->valueLen + 1));
#ifndef FTW_DUAL_INTERFACE_SUPPORT
		FTW_SERIAL_put(FTW_SERIAL_0,  header, headerLen);
		FTW_SERIAL_put(FTW_SERIAL_0,  xmlCbparams->tagPtr, xmlCbparams->tagLen);
		FTW_SERIAL_put(FTW_SERIAL_0,  &delim,1);
		FTW_SERIAL_put(FTW_SERIAL_0,  xmlCbparams->valuePtr, xmlCbparams->valueLen);	
#else
		FTW_SERIAL_put(FTW_SERIAL_1, header, headerLen);
		FTW_SERIAL_put(FTW_SERIAL_1, xmlCbparams->tagPtr, xmlCbparams->tagLen);
		FTW_SERIAL_put(FTW_SERIAL_1, delim,1);
		FTW_SERIAL_put(FTW_SERIAL_1, xmlCbparams->valuePtr, xmlCbparams->valueLen);
#endif
	}
	else if(xmlCbparams->status == GSN_XML_COMPLEX_ELEMENT_END)
	{
		headerLen=sprintf((char *)header,"%cG%d", FTW_ESC,FTW_XML_COMPLEX_TAG_END_ID);
#ifndef FTW_DUAL_INTERFACE_SUPPORT
		FTW_SERIAL_put(FTW_SERIAL_0,  header, headerLen);
#else
		FTW_SERIAL_put(FTW_SERIAL_1, header, headerLen);
	...
	{
		headerLen=sprintf((char *)header,"%cG", FTW_ESC);
#ifndef FTW_DUAL_INTERFACE_SUPPORT
		FTW_SERIAL_put(FTW_SERIAL_0,  header, headerLen);
		S2w_Printf("0000");
#else
...
AppMcu_SendUri(UINT8 cid,UINT8 type, const char *uri)
{
	...
	headerLen=sprintf((char *)header,"%cK%x%04d%x%s",FTW_ESC,cid,len,type,uri);
#ifndef FTW_DUAL_INTERFACE_SUPPORT
	FTW_SERIAL_put(FTW_SERIAL_0,  header, headerLen);
#else
	FTW_SERIAL_put(FTW_SERIAL_1, header, headerLen);
 ...
 * @param cid - connectio identifier
 * @return validCid if the allocation is successful else S2WFTW_FAILURE 
 ********************************************************************/
UINT8
 ...
 * @return FTW_FAILURE -send failed
 * @return FTW_SUCCESS -send success
 ******************************************************************/
UINT8
AppS2w_HttpdSendData(ttHttpdUserConHandle conHandle,UINT8 cid)
{
	...
	do
	{
		...
		{	
			...
			s2wappMainTaskCtxt->gsLinkSendStatus = FTW_FAILURE;
			GsnOsal_SemRelease(&s2wCidList[cid].httpdCtx.httpdSendSem);
			AppS2w_GslinkDeInit(cid,1);
			return FTW_FAILURE;
		}
		s2wappMainTaskCtxt->gsLinkSendStatus  = FTW_SUCCESS;	
...
AppS2w_HttpPassHdr(ttHttpdUserConEntryPtr conEntryPtr, UINT8 cid)
{
    ...
    for(i = 1; i < TM_HTTP_HEADER_COUNT ; i++)
    {
         if((NULL != conEntryPtr->uconReqHeaders[i]) && (bitmap & count))
         {
             ...
             headerLen = sprintf((char *)header,"%cG%d%d%04d", FTW_ESC, cid, FTW_HTTP_HEADER, len);
#ifndef FTW_DUAL_INTERFACE_SUPPORT
             FTW_SERIAL_put(FTW_SERIAL_0,  header, headerLen);
             FTW_SERIAL_put(FTW_SERIAL_0,  tvHttpHeaderNames[i], strlen(tvHttpHeaderNames[i]));
             FTW_SERIAL_put(FTW_SERIAL_0,  &delim,1);
             FTW_SERIAL_put(FTW_SERIAL_0,  conEntryPtr->uconReqHeaders[i], strlen(conEntryPtr->uconReqHeaders[i]));
#else
             FTW_SERIAL_put(FTW_SERIAL_1, header, headerLen);
             FTW_SERIAL_put(FTW_SERIAL_1, tvHttpHeaderNames[i], strlen(tvHttpHeaderNames[i]));
             FTW_SERIAL_put(FTW_SERIAL_1, delim,1);
             FTW_SERIAL_put(FTW_SERIAL_1, conEntryPtr->uconReqHeaders[i], strlen(conEntryPtr->uconReqHeaders[i]));
#endif
         }
         count = (count << 1);
    }
...
#ifndef FTW_DUAL_INTERFACE_SUPPORT
            FTW_SERIAL_put(FTW_SERIAL_0,  header, headerLen);
            FTW_SERIAL_put(FTW_SERIAL_0,  conEntryPtr->uconCustomHeaderstype[i], strlen(conEntryPtr->uconCustomHeaderstype[i]));
            FTW_SERIAL_put(FTW_SERIAL_0,  &delim,1);
            FTW_SERIAL_put(FTW_SERIAL_0,  conEntryPtr->uconCustomHeadersval[i], strlen(conEntryPtr->uconCustomHeadersval[i]));
#else
            FTW_SERIAL_put(FTW_SERIAL_1, header, headerLen);
            FTW_SERIAL_put(FTW_SERIAL_1, conEntryPtr->uconCustomHeaderstype[i], strlen(conEntryPtr->uconCustomHeaderstype[i]));
            FTW_SERIAL_put(FTW_SERIAL_1, delim,1);
            FTW_SERIAL_put(FTW_SERIAL_1, conEntryPtr->uconCustomHeadersval[i], strlen(conEntryPtr->uconCustomHeadersval[i]));
#endif
        }
...
AppS2w_HttpPassData(ttHttpdUserConHandle conHandle, UINT8 cid)
{
    ...
    if((1 == s2wappMainTaskCtxt->WebServerCfg.sendReq) && (NULL != conEntryPtr->uconReqMethod))
    {
        ...
        headerLen = sprintf((char *)header,"%cG%d%d%04d%d%c", FTW_ESC, cid, FTW_HTTP_REQUEST_LINE, len, conEntryPtr->uconReqMethod, FTW_COLON);
#ifndef FTW_DUAL_INTERFACE_SUPPORT
        FTW_SERIAL_put(FTW_SERIAL_0,  header, headerLen);
        FTW_SERIAL_put(FTW_SERIAL_0,  conEntryPtr->uconReqPathPtr, (len - 2));
#else
        FTW_SERIAL_put(FTW_SERIAL_1, header, headerLen);
        FTW_SERIAL_put(FTW_SERIAL_1, conEntryPtr->uconReqPathPtr, (len - 2));
#endif
    }
    if((1 == s2wappMainTaskCtxt->WebServerCfg.sendCgi) && (0 != conEntryPtr->uconCgiArgCnt))
    {
        for(i = 0; i < conEntryPtr->uconCgiArgCnt; i++)
        {
            ...
            headerLen = sprintf((char *)header,"%cG%d%d%04d", FTW_ESC, cid, FTW_HTTP_CGI_PARAMS, len);
#ifndef FTW_DUAL_INTERFACE_SUPPORT
            FTW_SERIAL_put(FTW_SERIAL_0,  header, headerLen);
            FTW_SERIAL_put(FTW_SERIAL_0,  conEntryPtr->uconCgiArgNames[i], strlen(conEntryPtr->uconCgiArgNames[i]));
            FTW_SERIAL_put(FTW_SERIAL_0,  &delim,1);
            FTW_SERIAL_put(FTW_SERIAL_0,  conEntryPtr->uconCgiArgValues[i], strlen(conEntryPtr->uconCgiArgValues[i]));
#else
            FTW_SERIAL_put(FTW_SERIAL_1, header, headerLen);
            FTW_SERIAL_put(FTW_SERIAL_1, conEntryPtr->uconCgiArgNames[i], strlen(conEntryPtr->uconCgiArgNames[i]));
            FTW_SERIAL_put(FTW_SERIAL_1, delim,1);
            FTW_SERIAL_put(FTW_SERIAL_1, conEntryPtr->uconCgiArgValues[i], strlen(conEntryPtr->uconCgiArgValues[i]));
#endif
        }
...
App_McuHttpApiInit()
{
    ...
    status = GsnHttpd_UriHandlerConfig((char *)FTW_DFLT_HTTP_SRVR_URI,
    		GSN_HTTPD_URI_METHOD_GET_POST_PUT_DEL, APP_MCU_PROFILE_DESC,
            McuHttpCb, FTW_DFLT_HTTP_SRVR_CONTENT_TYPE);
	memset(s2wappMainTaskCtxt->currenturi,0,GSN_HTTPD_MAX_URI_LENGTH);	
	memcpy(s2wappMainTaskCtxt->currenturi,FTW_DFLT_HTTP_SRVR_URI,strlen(FTW_DFLT_HTTP_SRVR_URI)  );
#if 0
 ...
 * @return FTW_SUCCESS if timeout value is set else FTW_FAILURE
 ********************************************************************/
PUBLIC UINT8
