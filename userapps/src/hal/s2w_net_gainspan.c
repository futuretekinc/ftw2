...
#define FTW_TLS_DEFAULT_FRAGMENT_SIZE 1398
#define FTW_TLS_HEADER_LENGTH 5
#define FTW_TLS_CONN_ALERT_FATAL 2
#define FTW_TLS_CONN_STATUS_FAIL 0
#define FTW_TLS_CONN_STATUS_SUCCESS 1

/* Socket descriptor value to indicate unused FTW_CID_T
 */
 ...
 */
//PRIVATE FTW_TIMER_T s2wTcpConnectTimer;

/* Timer for UDP connection close
 */
//PRIVATE FTW_TIMER_T s2wUdpCloseTimer;
...
__no_init FTW_CID_T s2wCidList[MAX_CID_RANGE];

/**
 * @brief Buffer to hold the recieved data.
 */
__no_init UINT8 s2wRxBuf[FTW_RX_BUF_SIZE];

/* Events to register the socket callback for
 */
#define FTW_RXCALLBACK_FLAGS (NX_BSD_CB_TCP_RECIEVE | NX_BSD_CB_UDP_RECIEVE | NX_BSD_CB_TCP_LISTEN | NX_BSD_CB_TCP_DISCONNECT |	NX_BSD_CB_SOCKET_ERROR)
//#define FTW_RXCALLBACK_FLAGS NX_BSD_CB_TCP_LISTEN
...
__no_init UINT32 s2wNetQueueBuffer[ ((APP_CFG_NET_RX_MAX_MSG_IN_MBOX) * sizeof(FTW_MSG_T)) / sizeof(UINT32)];
 ...
 ******************************************************************/
PUBLIC UINT8
AppS2wHal_NetUdpClient(FTW_NETDATA_T *peerData, UINT8 *cidNo, UINT16 srcPort)
{
    UINT8 cid;
    ...
    s2wCidList[cid].conMode = FTW_NETDATA_MODE_CLIENT;
    ...
    nx_bsd_callback_register(s2wCidList[cid].sd, AppS2wHal_RxCallBack,FTW_RXCALLBACK_FLAGS);
    *cidNo = cid;
    return FTW_SUCCESS;
...
PUBLIC UINT8
AppS2wHal_NetUdpClientv6(FTW_NETDATA_T *remoteServerData, UINT8 *cidNo, UINT16 srcPort)
{
    UINT8 cid;
    INT status;
    INT32 sd;
...
PUBLIC UINT8
AppS2wHal_NetUdpServerv6(FTW_NETDATA_T *serverData, UINT8 *cidNo)
{
  UINT8 cid;
  INT32 sd;
  ...
  cid = AppS2wHal_CidGet();
    ...
    s2wCidList[cid].sd = sd;
    s2wCidList[cid].conType = UDP;
    s2wCidList[cid].conMode = FTW_NETDATA_MODE_SERVER;
...
PUBLIC UINT8
AppS2wHal_NetTcpClientv6(FTW_NETDATA_T *remoteServerData, UINT8 *cidNo)
{
...
PUBLIC UINT8
AppS2wHal_NetTcpServerv6(FTW_NETDATA_T *serverData, UINT8 *cidNo)
{
  INT32 sd;
  UINT8 cid;
  struct sockaddr_in6 servAddr;
 ...
 ******************************************************************/
PUBLIC UINT8
AppS2wHal_NetTcpClient(FTW_NETDATA_T *peerData, UINT8 *cidNo)
{
    struct sockaddr_in destAddr;
    UINT32 cid;
    INT32 sd;
    INT32 ret;
    ULONG connect_timeout =  (s2wCurrent.registers[FTW_REG_TCP_CONNECT_TIMEOUT]/100);
    ...
    nx_bsd_callback_register(sd, AppS2wHal_RxCallBack,FTW_RXCALLBACK_FLAGS);
    ...
    s2wCidList[cid].sd = sd;
    s2wCidList[cid].conType = TCP;
    s2wCidList[cid].conMode = FTW_NETDATA_MODE_CLIENT_TRANSIENT;
    s2wCidList[cid].remotePort = peerData->port;
 ...
 ******************************************************************/
PUBLIC UINT8
AppS2wHal_NetUdpServer(FTW_NETDATA_T *serverData, UINT8 *cidNo)
{
    UINT8 cid;
    INT32 sd, ret;
    struct sockaddr_in srcAddr;
 ...
 ******************************************************************/
PUBLIC UINT8
AppS2wHal_NetTcpServer(FTW_NETDATA_T *serverData, UINT8 *cidNo)
{
    INT32 sd, ret;
    struct sockaddr_in srcAddr;
    UINT8 cid;
...
AppS2wHal_NetClose(UINT8 cid)
{
    ...
    if((p->conType == TCP) && (p->conMode == FTW_NETDATA_MODE_CLIENT))
    {
...
#if 0   
     UINT32 i=0;
     if((p->conType == TCP) && (p->conMode == FTW_NETDATA_MODE_SERVER))
     {
                 ...
                 VOID *buf, UINT32 len)
{
	...
	UINT8 *outDataBuf;
    p = &s2wCidList[cid];
    if (p->conMode == FTW_NETDATA_MODE_SERVER)
    {
    ...
    else if((p->conType == UDP) && (p->conMode == FTW_NETDATA_MODE_CLIENT))
    {
#if defined(FTW_DTLS_CLIENT_SUPPORT)
...
AppS2wHal_CidInfoGet(FTW_CID_INFO_T cidInfo[],UINT32* nos)
{
    ...
    for(i=0;i<MAX_CID_RANGE;i++)
    {
        if(s2wCidList[i].sd != CID_NOTINUSE) // valid cid
        {
            ...
            if (s2wCidList[i].conMode != FTW_NETDATA_MODE_SERVER)
            {
...
AppS2wHal_NetRecvTask(UINT32 ctx)
{
...

    {
        ...
        {
			case FTW_RECV_TYPE_SOCK_DATA_DISCONNECT:
            case FTW_RECV_TYPE_SOCKET:
            {
                cid = AppS2wHal_CidFind(recvMsg.rData.dataSock);
                if (cid == INVALID_CID)
                {
                	#ifdef FTW_SEPARATE_NET_RX_TASK
                    	continue;
					#else
						break;
					#endif /*FTW_SEPARATE_NET_RX_TASK*/
                }
                FTW_ASSERT(s2wCidList[cid].conType != 0);
                ...
                do
                {
                    ...
                    if (s2wCidList[cid].conType == UDP)
                    {
                      ...
                      if(s2wCidList[cid].addrType & ADDR_TYPE_IPv6)
                        ...
                        if (dataLen > 0)
                        {
                            if( s2wCidList[cid].conMode == FTW_NETDATA_MODE_SERVER)
                            {
#ifdef FTW_IPv6_SUPPORT                              
                              if(s2wCidList[cid].addrType & ADDR_TYPE_IPv6)
                              {
                    ...
                    else
                    {
                        ...
                        if (s2wCidList[cid].sslflag == TRUE /*&& (dataLen > 0)*/)
                        {
							...
							if(TRUE == s2wCidList[cid].sslRemoteCliConctng)
							{
								...
								//msg = (FTW_MOD_NOTIF_CONN_START + cid);
                        ...
                        if((s2wCidList[cid].sslflag == TRUE) && (s2wCidList[cid].conType == TCP))
                        {
                        	...
                        	buffOfset=0;
                       		while(totalBytesToSnd)
                       		{
								...
								GsnSq_TaskMonitorStart(APP_CFG_SQ_NETRX_TASK_ID, cid);
								sentLen =totalBytesToSnd >= FTW_RX_BUF_SIZE? FTW_RX_BUF_SIZE:totalBytesToSnd;
                ...
                if(FTW_RECV_TYPE_SOCK_DATA_DISCONNECT == recvMsg.msgType)

                {
					...
					if(s2wCidList[cid].s2wUsrCloseFlag == 0 && status == FTW_SUCCESS)
					{
...
*/						
						...
						{
							AppS2wProcess_AsyncStatusNotify(FTW_ASYNC_ECIDCLOSE, cid);
						}
						else
#endif
                   		{
                        	AppS2wProcess_StatusNotify(FTW_ECIDCLOSE, cid);
                    	}
	                    GsnOsal_SemRelease(&s2wSyncSemID);
					}
            ...
            case FTW_RECV_TYPE_SOCK_ERROR:
				...
				*/						
                ...
                if((!s2wAutoConnState ) ||(( s2wState == FTW_STATE_CMD) && (s2wIsAutoconnected)))
                {
					...
					if(s2wCurrent.asyncMsgFormat)
					{
						AppS2wProcess_AsyncStatusNotify(FTW_ASYNC_SOCKFAILURE, cid);
					}
					else
#endif
	                {
	                    AppS2wProcess_StatusNotify(FTW_SOCKFAILURE, cid);
	                }
			    }
#ifdef FTW_UART_ENABLE
                // while(0 == S2wUart_TxStatusGet());
            ...
            case FTW_RECV_TYPE_SOCK_DISCONNECT:
...
#endif
#ifdef FTW_HTTPC_SUPPORT
            case FTW_RECV_TYPE_HTTPS_CLOSE:
               ...
               // while(0 == S2wUart_TxStatusGet());
			    ...
			    case FTW_RECV_TYPE_SOCK_ACCEPT:
...
AppS2wHal_CidGet(VOID)
{
    ...
    for (i = j,k=0; k < MAX_CID_RANGE ;(i=((i+1)&(MAX_CID_RANGE-1))),k++)
    {
        ...
        {
        	memset((INT8 *)&s2wCidList[i], 0, sizeof(FTW_CID_T));
			s2wCidList[i].sd = CID_NOTINUSE;
...
AppS2wHal_RxCallBack(INT32 sock, NX_BSD_CB_EVENT_TYPE_T flags, UINT port)
{
    ...
    if ((NX_BSD_CB_TCP_RECIEVE & flags) || (NX_BSD_CB_UDP_RECIEVE & flags))
    {
		...
		{
	        recvMsg.msgType = FTW_RECV_TYPE_SOCK_DATA_DISCONNECT;
		}
		else
		{
			recvMsg.msgType = FTW_RECV_TYPE_SOCKET;
                }
    ...
    if (NX_BSD_CB_TCP_LISTEN & flags)
    {
        ...
        if (s2wCidList[cid].sslflag != TRUE)
        {
            ...
            s2wCidList[newcid].conMode = FTW_NETDATA_MODE_CLIENT;
		    ...
		    recvMsg.msgType = FTW_RECV_TYPE_SOCK_ACCEPT;
        ...
        *((ULONG*)s2wCidList[newcid].remoteIp) = htonl(*(ULONG*)s2wCidList[newcid].remoteIp);
        ...
        nx_bsd_callback_register(s2wCidList[newcid].sd,AppS2wHal_RxCallBack, FTW_RXCALLBACK_FLAGS);
		GsnOsal_SemAcquire ( &(s2wappMainTaskCtxt->s2wTcpClientCntSyncSem), GSN_OSAL_WAIT_FOREVER);
    ...
    if (NX_BSD_CB_SOCKET_ERROR & flags)
    {
        if (s2wCidList[cid].conType != 0)
        {
           ...
           recvMsg.msgType = FTW_RECV_TYPE_SOCK_ERROR;
           recvMsg.rData.dataSock = sock;
           recvMsg.msgLen = 0;
           GsnOsal_QueuePut(&s2wNetQueue,(UINT8*)&recvMsg);
        }
    ...
    if (NX_BSD_CB_TCP_DISCONNECT & flags)
    {

        {
           ...
           recvMsg.msgType = FTW_RECV_TYPE_SOCK_DISCONNECT;
           recvMsg.rData.dataSock = sock;
           recvMsg.msgLen = 0;
                           ...
                           UINT8 *clientCertName, UINT8 *clientKeyName)
{
    ...
    if(caCertName != NULL)
    {
        ...
        pxCert = FTW_CERT_searchRamTable(caCertName);
        if(pxCert == NULL)
        {
        	pxCert = FTW_CERT_flashFileLoad(caCertName);
        }
        ...
        sslConnParams.caCertLen = *(UINT16 *)pxCert->pCertAddress;
        sslConnParams.caCert = pxCert->pCertAddress + 2;
    ...
    if(clientCertName != NULL)
    {
        ...
        pxCert = FTW_CERT_searchRamTable((char*)clientCertName);
        if(pxCert == NULL)
        {
        	pxCert = FTW_CERT_flashFileLoad((char*)clientCertName);
        }
        if(pxCert == NULL)
        {
        ...
        sslConnParams.clientCertLen = *(UINT16 *)pxCert->pCertAddress;
        sslConnParams.clientCert =  pxCert->pCertAddress + 2;
    ...
    if(clientKeyName != NULL)
    {
        ...
        pxCert = FTW_CERT_searchRamTable((char*)clientKeyName);
        if(pxCert == NULL)
        {
        	pxCert = FTW_CERT_flashFileLoad((char*)clientKeyName);
        }
        if(pxCert == NULL)
        {
        ...
        sslConnParams.clientKeyLen = *(UINT16 *)pxCert->pCertAddress;
        sslConnParams.clientKey =  pxCert->pCertAddress + 2;
...
#ifdef FTW_SSL_EXT_MAX_FRAG_LEN
    s2wCidList[cid].ssl.sslConn->maxFragLen = FTW_SSL_EXT_MAX_FRAG_LEN;
#else
...
GSN_STATUS AppS2w_SslServerOpen(UINT8 cid, char* sslCACertName)
{
    ...
    /* Fill the certificate details here. For testing it takes the below
       certificates */
 #ifndef FTW_SSL_SERVER_TEST
 	memset((INT8 *)&certDetail, 0, sizeof(certDetail));

    pxCert = FTW_CERT_searchRamTable(FTW_SSL_CERT_NAME_CA);
    if(pxCert == NULL)
    {
    	pxCert = FTW_CERT_flashFileLoad(FTW_SSL_CERT_NAME_CA);
    }
    if(pxCert == NULL)
    {
        S2w_Printf("\n\rCertificate %s Not Found", FTW_SSL_CERT_NAME_CA);
        return FTW_FAILURE;
    }

    certDetail.CaCertName = (char const *)pxCert->pName;
    certDetail.CaCertLocation = pxCert->pCertAddress + 2;
    certLength = *(pxCert->pCertAddress + 1);
    certLength <<= 8;
    certLength = certLength | (*(pxCert->pCertAddress));
    certDetail.CaCertLen = certLength;

	pxCert = FTW_CERT_searchRamTable(FTW_SSL_CERT_NAME_SERVER);
    if(pxCert == NULL)
    {
    	pxCert = FTW_CERT_flashFileLoad(FTW_SSL_CERT_NAME_SERVER);
    }
    if(pxCert == NULL)
    {
        S2w_Printf("\n\rCertificate %s Not Found", FTW_SSL_CERT_NAME_SERVER);
        return FTW_FAILURE;
    }

    certDetail.ClientCertName = (char const *)pxCert->pName;
    certDetail.ClientCertLocation = pxCert->pCertAddress + 2;
    certLength = *(pxCert->pCertAddress + 1);
    certLength <<= 8;
    certLength = certLength | (*(pxCert->pCertAddress));
    certDetail.ClientCertLen = certLength;

	pxCert = FTW_CERT_searchRamTable(FTW_SSL_CERT_NAME_KEY);
    if(pxCert == NULL)
    {
    	pxCert = FTW_CERT_flashFileLoad(FTW_SSL_CERT_NAME_KEY);
    }
    if(pxCert == NULL)
    {
        S2w_Printf("\n\rCertificate %s Not Found", FTW_SSL_CERT_NAME_KEY);
        return FTW_FAILURE;
    }
    
    certDetail.PrivateKeyName = (char const *)pxCert->pName;
    certDetail.PrivateKeyLocation = pxCert->pCertAddress + 2;
    certLength = *(pxCert->pCertAddress + 1);
    certLength <<= 8;
    certLength = certLength | (*(pxCert->pCertAddress));
...
AppS2w_SslClose(UINT8 cid)
{
    if (s2wCidList[cid].sslflag == TRUE)
    {
    	if(s2wCidList[cid].conMode == FTW_NETDATA_MODE_CLIENT)
    	{
...
AppS2w_SslLocalClose(UINT8 cid)
{
	...
	else if(s2wCidList[cid].conMode == FTW_NETDATA_MODE_SERVER)
	{
#ifdef FTW_SSL_SERVER_SUPPORT
...
#ifdef FTW_SSL_EXT_MAX_FRAG_LEN
    connParams.sslExtnMaxFragLen = FTW_SSL_EXT_MAX_FRAG_LEN;
#else
                     ...
                     ttHttpcCustomHdr  *customHdrPtr)
{
    ...
    if((1 == httpConfInfo.sendStatus) && (NULL != reasonPhrase))
    {
        ...
        headerLen = sprintf((char *)header,"%cG%x%d%04d%03d%c", FTW_ESC, cid, FTW_HTTP_RESPONSE_LINE, len, statusCode, FTW_COLON);
#ifndef FTW_DUAL_INTERFACE_SUPPORT
        FTW_SERIAL_put(FTW_SERIAL_0, header, headerLen);
        FTW_SERIAL_put(FTW_SERIAL_0, reasonPhrase, (len - 4));
#else
        FTW_SERIAL_put(FTW_SERIAL_1, header, headerLen);
        FTW_SERIAL_put(FTW_SERIAL_1, reasonPhrase, (len - 4));
#endif
    }
    ...
    for(i = 1; i < TM_HTTP_HEADER_COUNT ; i++)
    {
         if((NULL != resHeadersPtr[i]) && (bitmap & count))
         {
             ...
             headerLen = sprintf((char *)header,"%cG%x%d%04d", FTW_ESC, cid, FTW_HTTP_HEADER, len);
#ifndef FTW_DUAL_INTERFACE_SUPPORT
             FTW_SERIAL_put(FTW_SERIAL_0, header, headerLen);
             FTW_SERIAL_put(FTW_SERIAL_0, tvHttpHeaderNames[i], strlen(tvHttpHeaderNames[i]));
             FTW_SERIAL_put(FTW_SERIAL_0, &delim,1);
             FTW_SERIAL_put(FTW_SERIAL_0, resHeadersPtr[i], strlen(resHeadersPtr[i]));
#else
             FTW_SERIAL_put(FTW_SERIAL_1, header, headerLen);
             FTW_SERIAL_put(FTW_SERIAL_1, tvHttpHeaderNames[i], strlen(tvHttpHeaderNames[i]));
             FTW_SERIAL_put(FTW_SERIAL_1, &delim,1);
             FTW_SERIAL_put(FTW_SERIAL_1, resHeadersPtr[i], strlen(resHeadersPtr[i]));
#endif
         }
         count = (count << 1);
    }
...
#ifndef FTW_DUAL_INTERFACE_SUPPORT
            FTW_SERIAL_put(FTW_SERIAL_0, header, headerLen);
            FTW_SERIAL_put(FTW_SERIAL_0, customHdrPtr->hconCustomHeaderstype[i], strlen(customHdrPtr->hconCustomHeaderstype[i]));
            FTW_SERIAL_put(FTW_SERIAL_0, &delim,1);
            FTW_SERIAL_put(FTW_SERIAL_0, customHdrPtr->hconCustomHeadersval[i], strlen(customHdrPtr->hconCustomHeadersval[i]));
#else
            FTW_SERIAL_put(FTW_SERIAL_1, header, headerLen);
            FTW_SERIAL_put(FTW_SERIAL_1, customHdrPtr->hconCustomHeaderstype[i], strlen(customHdrPtr->hconCustomHeaderstype[i]));
            FTW_SERIAL_put(FTW_SERIAL_1, &delim,1);
            FTW_SERIAL_put(FTW_SERIAL_1, customHdrPtr->hconCustomHeadersval[i], strlen(customHdrPtr->hconCustomHeadersval[i]));
#endif
        }
    }
...
#endif				
	{
    sprintf(startMark,"%c%c%x%04d", FTW_ESC, 'H', cid,httpRxDataLen);
#ifndef FTW_DUAL_INTERFACE_SUPPORT
    FTW_SERIAL_put(FTW_SERIAL_0, startMark, strlen(startMark));
    FTW_SERIAL_put(FTW_SERIAL_0, httpRxDataBuf, httpRxDataLen);
#else
    FTW_SERIAL_put(FTW_SERIAL_1, startMark, strlen(startMark));
    FTW_SERIAL_put(FTW_SERIAL_1, httpRxDataBuf, httpRxDataLen);
#endif
 ...
 * @retval FTW_SUCCESS     - operation successfull.
 * @retval FTW_EBADCID     - cid is not valid.
 * @retval FTW_FAILURE     - operation failed.
 ******************************************************************/
PUBLIC GSN_STATUS 
S2wHttp_Send(UINT8 cid)
{
    ...
    UINT32 ret = FTW_SUCCESS;//et1= FTW_SUCCESS, bytesLeft = 0;
    ...
    if((bytesLeft > 0) && (conEntryPtr->hconHttpType == GSN_HTTPS))
    {
        ...
        if(ret1 == GSN_SSL_CLOSE_ALERT)
        {
            ...
            recvMsg.msgType = FTW_RECV_TYPE_HTTPS_CLOSE;
...
AppS2wHal_MdnsRegCb(void *pCtx, UINT8 *rrName, UINT32 status)
{
    ...
    {
	    syncId =  FTW_MDNS_REG_FAILED_ID;
		asyncId = FTW_ASYNC_MDNS_REG_FAILED_ID;
    }
    else
    {
	    syncId =  FTW_MDNS_REG_SUCESS_ID;
		asyncId = FTW_ASYNC_MDNS_REG_SUCESS_ID;
    }
...
//GSN_MDNS_T mdns;
FTW_MDNS_HN_PARAM_T mdnsHnParam;
...
PUBLIC UINT8
AppS2wHal_MdnsHostNameReg(FTW_MDNS_HN_PARAM_T *pHNparam)
{
    GSN_STATUS retVal;
...
PUBLIC UINT8
AppS2wHal_MdnsServiceReg(FTW_MDNS_SRV_PARAM_T *pSrvParam)
{
    GSN_STATUS retVal;
...
PUBLIC UINT8
AppS2wHal_MdnsHostNameDereg(FTW_MDNS_HN_PARAM_T *pHNparam)
{
...
PUBLIC UINT8
AppS2wHal_MdnsSrvcDiscover(FTW_MDNS_SRVCDISCVR_PARAM_T *mdnsSrvDscr)
{
    GSN_STATUS retVal;
...
#endif
#define FTW_GS2000_TIMER_TICK_IN_MILLI_SECOND   0.030517578125
#ifdef FTW_PING_TRACE
FTW_PING_STATS_T s2wPingStats;
    ...
    if(s2wPingStarted == TRUE)
    {
     ...
     icmpRetVal =  _nxd_icmp_ping(&s2wappMainTaskCtxt->if0.nwifCtx.ipHndl,&s2wappMainTaskCtxt->pingParams.ip_address, (INT8*)pingBuf,strlen((const char*)pingBuf),&response_ptr, interval);
      ...
      icmpRetVal = nx_icmp_ping(&s2wappMainTaskCtxt->if0.nwifCtx.ipHndl,(s2wappMainTaskCtxt->pingParams.Ipv4Addr),(INT8*)pingBuf,strlen((const char*)pingBuf)/*strlen((const char*)s2wappMainTaskCtxt->pingParams.payload)*/,&response_ptr,interval);
        ...
        GsnOsal_SemAcquire(&s2wSyncSemID, GSN_OSAL_WAIT_FOREVER);
        FREE(pingBuf);
      if(icmpRetVal == FTW_SUCCESS && response_ptr != NULL )
        {
            rtt1 = GsnTod_Get();
#ifdef FTW_IPv6_SUPPORT    
            if(s2wappMainTaskCtxt->pingParams.IpVersion & ADDR_TYPE_IPv6)
            {
                S2w_Printf("\r\nReply from %s bytes=%d time=%d ms TTL %d",ipv6str,response_ptr->nx_packet_length,(ULONG)((rtt1-rtt0)*FTW_GS2000_TIMER_TICK_IN_MILLI_SECOND),ttl );
            }
            else
#endif     
            {
                S2w_Printf("\r\nReply from %d.%d.%d.%d: bytes=%d time=%d ms TTL %d",(s2wappMainTaskCtxt->pingParams.Ipv4Addr& 0xff000000)>>24,(s2wappMainTaskCtxt->pingParams.Ipv4Addr& 0x00ff0000)>>16,(s2wappMainTaskCtxt->pingParams.Ipv4Addr& 0x0000ff00)>>8,(s2wappMainTaskCtxt->pingParams.Ipv4Addr & 0x000000ff),response_ptr->nx_packet_length,(ULONG)((rtt1-rtt0)*FTW_GS2000_TIMER_TICK_IN_MILLI_SECOND),ttl );
            }
            s2wPingStats.pingRecvCnt++;
            if(((rtt1-rtt0)*FTW_GS2000_TIMER_TICK_IN_MILLI_SECOND)>s2wPingStats.rttMax)
            {
                s2wPingStats.rttMax=(ULONG)((rtt1-rtt0)*FTW_GS2000_TIMER_TICK_IN_MILLI_SECOND);
            }
            if(!s2wPingStats.rttMin)
            {
                s2wPingStats.rttMin=s2wPingStats.rttMax;
            }
            if(((rtt1-rtt0)*FTW_GS2000_TIMER_TICK_IN_MILLI_SECOND) < s2wPingStats.rttMin)
            {
                s2wPingStats.rttMin=(ULONG)((rtt1-rtt0)*FTW_GS2000_TIMER_TICK_IN_MILLI_SECOND);
            }
            s2wPingStats.rttAvg +=((rtt1-rtt0)*FTW_GS2000_TIMER_TICK_IN_MILLI_SECOND);
...
VOID s2wPingStop()
{
  if( s2wPingStarted == FALSE)
  {
      GsnOsal_SemAcquire(&s2wSyncSemID, GSN_OSAL_WAIT_FOREVER);
        ...
        GsnOsal_SemRelease(&s2wSyncSemID);
        memset(&s2wPingStats,0,sizeof(FTW_PING_STATS_T));
  }
...
UINT32 AppS2w_isForValidCid(UINT16 dst_port , UINT16 src_port)
{
    ...
    for (i = 0; i < MAX_CID_RANGE; i++)
    {
        if (((s2wCidList[i].sd != CID_NOTINUSE) &&((dst_port == s2wCidList[i].localPort ) ||
            (s2wCidList[i].conMode == FTW_NETDATA_MODE_CLIENT_TRANSIENT)))||
            ((s2wCidList[i].conMode == FTW_NETDATA_MODE_CLIENT_TRANSIENT) && 
            (s2wCidList[i].httpflag == 1) &&  ((src_port == FTW_HTTPS_PORT ) || (src_port == FTW_HTTP_PORT ))))
           {
               filterPass = TRUE;
               break;
           }
    ...
    for (i = 0; i < MAX_CID_RANGE; i++)
    {
        if ((s2wCidList[i].sd != CID_NOTINUSE) &&
            ...
            (src_port == FTW_COAP_PORT ) && (s2wCidList[i].coapContext != NULL))
           {
               filterPass = TRUE;
               break;
           }
    ...
    if(s2wappMainTaskCtxt->l2ConfigBitField)
    {
	    if (GSN_ETH_PROTO_TYPE_IP == protocol)
	    {
		    ...
		    if((GSN_NW_ICMP_PROTO == *(pEthFrame->Data + GSN_NW_IP_PROTO_OFFSET)) &&
		       (s2wappMainTaskCtxt->l2ConfigBitField & FTW_ICMP_MASK))
		    {
				filterPass = FALSE;
			}
		    ...
		    (GSN_NW_TCP_PROTO == *(pEthFrame->Data + GSN_NW_IP_PROTO_OFFSET)))&&
			    (s2wappMainTaskCtxt->l2ConfigBitField & FTW_UDP_TCP_MASK))
		    {
				filterPass = FALSE;
                /* It has to be packet for an existing socket */
				if((GSN_NW_UDP_PROTO == *(pEthFrame->Data + GSN_NW_IP_PROTO_OFFSET)))
				{
					if((FTW_DNS_PORT == src_port)|| (FTW_SNTP_PORT == src_port))
					{
						filterPass = TRUE;
                ...
                if( (FTW_MDNS_PORT == dst_port) || (GSN_NW_BOOTPS_PORT == dst_port)
                    ||  (GSN_NW_BOOTP_PORT == dst_port)|| (FTW_DNS_PORT == dst_port)                   
#ifdef FTW_WEB_SERVER                   
                     ||  (((FTW_HTTPS_PORT == dst_port) || (FTW_HTTP_PORT == dst_port)) && (GsnHttpd_IsRunning()))
#endif                    
                    )
                {
                    filterPass = TRUE;
	...
	if(s2wappMainTaskCtxt->dpSleep)
	{
	    			...
	    			(GSN_NW_TCP_PROTO == *(pEthFrame->Data + GSN_NW_IP_PROTO_OFFSET)))
	    {
			...
			UINT16 src_port =htons(AppS2w_NwIfU16FieldGet(pEthFrame->Data+ GSN_NW_IP_HEADER_LEN
																+ GSN_NW_UDP_SRC_PORT_OFFSET));				
#ifdef FTW_WEB_SERVER
	        if(((FTW_HTTPS_PORT == dst_port) || (FTW_HTTP_PORT == dst_port)) && (GsnHttpd_IsRunning()))
	        {
	            getOutOfDpSleep = 1;
			}
#endif          			
                           ...
                           UINT8 *clientCertName, UINT8 *clientKeyName)
{
    ...
    //if(caCertName != NULL)
    {
        ...
        sslConnParams.caCertLen = sizeof(dtlsClientCA);//*(UINT16 *)pxCert->pCertAddress;
        sslConnParams.caCert =  (UINT8*)dtlsClientCA;//pxCert->pCertAddress + 2;
    ...
    //if(clientCertName != NULL)
    {
        sslConnParams.clientCertLen = sizeof(dtlsClientClient);//*(UINT16 *)pxCert->pCertAddress;
        sslConnParams.clientCert =  (UINT8*)dtlsClientClient;//pxCert->pCertAddress + 2;
    ...
    //(clientKeyName != NULL)
    {
        ...
        sslConnParams.clientKeyLen = sizeof(dtlsClentKey);//*(UINT16 *)pxCert->pCertAddress;
        sslConnParams.clientKey =  (UINT8*)dtlsClentKey;//pxCert->pCertAddress + 2;
