...
#define APP_FTW_FWUP_HTTP_TIMEOUT 75
#define APP_FTW_FWUP_DNS_REQ_DEF_TIME_OUT        5
#define APP_FTW_FWUP_DNS_REQ_DEF_RETRIES         3
...
extern FTW_OTA_FWUP_CONF_T OtaFwupConfParam;
...
#ifdef FTW_EXT_FLASH_FWUP_PULL_METHOD
//PRIVATE FTW_HTTP_DATA_T S2wFwupHttpData;
...
PRIVATE VOID*
AppS2w_FwupExtFlashConnOpen(struct sockaddr_storage *pSockAddr, FTW_OTA_FWUP_CONF_T *pOtaFwupConf)
{
    GSN_STATUS status;
                            ...
                            UINT8 *url)
{
    ...
    if(url != NULL)
    {
		...
		retVal = GsnHttp_Send(pHttpsConn, (INT8 *)pHttpRxDataBuf,
                       &httpRxDataLen, APP_FTW_FWUP_HTTP_TIMEOUT, &httpConfInfo);
                          ...
                          UINT8 *url)
{
    ...
    if(url != NULL)
    {
		...
		compiled with header type array being defined as array of UINT8 */
		{
			...
			for( i = 0; i < FTW_HTTPC_MAX_HDR_SUPPORTED; i++)
	        {
	        	if(headerTypes[i] >= (GSN_HTTP_USER_HEADER_TYPE_T)FTW_HTTPC_CFG_PARAM_HEADER_CUSTOM)
				{
					headerTypesModified[i] = (GSN_HTTP_USER_HEADER_TYPE_T)TM_HTTP_HEADER_CUSTOM;
				}
				else
...
APPS2w_ExtFlashStart( FTW_OTA_FWUP_CONF_T *pOtaFwupConf)
{
    ...
    if(pOtaFwupConf->proxyFlag == 1)
    {
        ...
        GsnNwIf_DnsRetryCountSet(&appCtx.if0.nwifCtx.dnsCliHndl,APP_FTW_FWUP_DNS_REQ_DEF_RETRIES);
        status = GsnNwIf_DnsHostByNameGet(&appCtx.if0.nwifCtx, (UINT8*)pOtaFwupConf->fwSrvHostName, &pOtaFwupConf->sockAddr.addr.ipv4.sin_addr.s_addr, APP_FTW_FWUP_DNS_REQ_DEF_TIME_OUT);
        ...
        else
        {
            ...
            status = nxd_dns_host_by_name_get(&appCtx.if0.nwifCtx.dnsCliHndl,(UINT8*)pOtaFwupConf->fwSrvHostName, &host_ipduo_address, APP_FTW_FWUP_DNS_REQ_DEF_TIME_OUT*100, 0x6);
#endif                
            if (status != GSN_SUCCESS)
			{
...
#ifdef ADK_OTAFU_DSV	
    {
        ...
        if(pOtaFwupConf->sslFlag && (pOtaFwupConf->fwSrvHostName != NULL))
        {
            ...
            if((pOtaFwupConf->dnsError) && (FTW_SUCCESS != status))
            {
		...
		retVal = AppS2w_ExtFwupDownLoadDsv(httpsConn, &otafuPullCtx,
					pOtaFwupConf->urls[FTW_OTAFU_URL_SIGN_IMAGE]);
        if(retVal != GSN_SUCCESS)
        {
#if defined(FTW_SSL_CLIENT_SUPPORT)
            if(NULL != s2wSslConf.dnsNameBuf)
...

	{
		...
		retVal = AppS2w_ExtFwupDownLoadStart(httpsConn, &otafuPullCtx,
					pOtaFwupConf->urls[FTW_OTAFU_URL_FW_IMAGE]);
		if(retVal != GSN_SUCCESS)
		{
...
GSN_STATUS
AppS2w_ExtFlashInfoXmlGet(FTW_OTA_FWUP_CONF_T *pOtaFwupConf)
{
    ...
    else
    {
        if(pOtaFwupConf->fwSrvHostName != NULL)
    	{
	    	...
	    	FTW_DNSINFO_T dnsInfo;
			UINT8 len;
			memset(&dnsInfo, 0, sizeof(FTW_DNSINFO_T));
		...
		else if(memcmp(pOtaFwupConf->fwSrvIp, fwSrvIp, sizeof(FTW_IPADDR_T)) != 0)
		{
...
#endif
		/* Save into FTW_OTA_FWUP_CONF structure*/
		if(pOtaFwupConf->urls[FTW_OTAFU_URL_WLAN] != NULL)
			gsn_free(pOtaFwupConf->urls[FTW_OTAFU_URL_WLAN]);
		if(pOtaFwupConf->urls[FTW_OTAFU_URL_APP1] != NULL)
			gsn_free(pOtaFwupConf->urls[FTW_OTAFU_URL_APP1]);
		if(pOtaFwupConf->urls[FTW_OTAFU_URL_APP2] != NULL)
			gsn_free(pOtaFwupConf->urls[FTW_OTAFU_URL_APP2]);
		if(pOtaFwupConf->urls[FTW_OTAFU_URL_WEB] != NULL)
			gsn_free(pOtaFwupConf->urls[FTW_OTAFU_URL_WEB]);
#ifdef GS1500M
		if(pOtaFwupConf->urls[FTW_OTAFU_URL_MAC] != NULL)
					gsn_free(pOtaFwupConf->urls[FTW_OTAFU_URL_MAC]);
#endif
		pOtaFwupConf->urls[FTW_OTAFU_URL_WLAN] = pWlan;
		pOtaFwupConf->urls[FTW_OTAFU_URL_APP1] = pApp0;
		pOtaFwupConf->urls[FTW_OTAFU_URL_APP2] = pApp1;
		pOtaFwupConf->urls[FTW_OTAFU_URL_WEB] = pWebImage;
#ifdef GS1500M
        pOtaFwupConf->urls[FTW_OTAFU_URL_MAC] = pMac;
#endif
