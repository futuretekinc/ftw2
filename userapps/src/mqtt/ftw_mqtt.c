...
uint_32 FTW_MQTT_connect(FTW_MQTT_CTX_PTR pxCTX)
{
    ...
    xPeerData.proto = FTW_NETDATA_PROTO_TCP;
    xPeerData.mode  = FTW_NETDATA_MODE_CLIENT;
    ...
    if (AppS2w_SslClientOpen(pxCTX->xCID, "ca-cert", NULL, NULL) != FTW_SUCCESS)
    {
...
int FTW_MQTT_sendPacket(void* pSocketInfo, void const * pBuf, unsigned int ulCount)
{
    ...
    if (AppS2wHal_NetTx(pxCTX->xCID, NULL, 0, (void *)pBuf, ulCount) == FTW_SUCCESS)
    {
