...
AppConfig_OTPMacRead(UINT8* mac, UINT8 id)
{
    ...
    UINT8 data[8] ={0},data1[OTP_MAC_LEN];
    ...
    if((id == 0xF8) || (id == 0xF9) || (id == 0xFA))
    {
        ...
        if(dataLen ) // since mac is now stored in reverse order
          {
...
APP_CONFIG_T*
AppConfig_RTCPtrGet() // right after rtc contents
{
    return (APP_CONFIG_T *)(GSN_RTC_SRAM_BLOCK0_ADDR + sizeof(APP_RTC_SECNDRY_MEM_INFO_T));
}
...
AppConfig_DefaultInit(APP_CONFIG_T * pConfig)
{
    ...
    if(GSN_WIF_SYS_BOOT_NORMAL_BOOT != GSN_BOOT_REASON_GET())
    {
        if(AppConfig_Load(pConfig) != GSN_SUCCESS)
        {
            ...
            /*GROUP_PROV Config)*/
            {
                ...
                pConfig->networkConfig.groupProvMode = GSN_PROV_AP;
            #else
                pConfig->networkConfig.groupProvMode = GSN_USER_AP;
            #endif
            }
        ...
        else
        {
            sprintf((char*)pConfig->sysIdConfig.uid, "%02x%02x%02x%02x%02x%02x",
                            ...
                            pConfig->networkConfig.macAddr[4],
                            pConfig->networkConfig.macAddr[5]);
            /* update the sysIdConfig name and uid in accordance with pConfig in provisioning ap-mode*/
            #ifdef ADK_PROV_GROUP_SECURE
                if((GSN_LIMITED_AP == pConfig->networkConfig.networkMode) && 
                    (GSN_PROV_AP == pConfig->networkConfig.groupProvMode))
                {
                    pAPCfg = &pConfig->networkConfig.apConfig;
                    strncpy((char*)pConfig->sysIdConfig.name, (void*)pAPCfg->ssid.array, pAPCfg->ssid.length + 1);
                    /*
                     * When in Prov-AP mode, we reset the System Name to be the SSID.
                     * The same System Name will be sent when the device is configured as a client
                     */
                    AppConfig_Save(pConfig);
                }
            #endif
    ...
    {
        /* Normal boot, read config from RTC sram */
        APP_CONFIG_T *pAppCfg = AppConfig_RTCPtrGet();
        memcpy(pConfig, pAppCfg, sizeof(APP_CONFIG_T));
    }
}
