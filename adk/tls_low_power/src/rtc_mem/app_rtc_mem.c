...
APP_RtcInit(UINT32 clearRtcMem)
{
   ...
   // UINT32 rstSize = 1024 - offsetof(APP_RTC_LATCH_MEM_INFO_T, ifc_1) - sizeof(GSN_SYS_RESERVD_RTC_INFO_T);
	...
	else if(clearRtcMem == 2)
	{
		...
		MEMSET((INT8 *)pAppRtcLtchMemInfo + sizeof(GSN_SYS_RESET_INFO_T) , 0,
				GSN_RTC_LATCH_MEM_BLOCK_SIZE - sizeof(GSN_SYS_RESERVD_RTC_INFO_T) - sizeof(GSN_SYS_RESET_INFO_T));
		MEMSET((INT8 *)GSN_RTC_SRAM_BLOCK0_ADDR, 0, GSN_RTC_SRAM_BLOCK_SIZE);
	}
	else if(clearRtcMem == 3)
	{
		// erase latch memory except boot info + seqNO
		MEMSET((INT8 *)pAppRtcLtchMemInfo + sizeof(GSN_SYS_RESET_INFO_T) + sizeof(UINT32) , 0,
				GSN_RTC_LATCH_MEM_BLOCK_SIZE - sizeof(GSN_SYS_RESERVD_RTC_INFO_T) -
				(sizeof(GSN_SYS_RESET_INFO_T) + sizeof(UINT32)) );
		MEMSET((INT8 *)GSN_RTC_SRAM_BLOCK0_ADDR, 0, GSN_RTC_SRAM_BLOCK_SIZE);
	}
