...
#pragma section = "APP_CODE_SECTION_1"
#pragma section = "APP_CODE_SECTION_2"

UINT8 noFlashFetchFlag = 0;
...
GSN_BYTE_POOL_LINK_T bytePool[3] ;//= {{,NULL}, {, NULL}, {,NU}};
unsigned int* codeAddrArray[4];
...
void GsnStartup_Main( void )
{
    ...
    unsigned int* app_code_1_start = __section_begin("APP_CODE_SECTION_1");
    unsigned int* app_code_1_end = __section_end("APP_CODE_SECTION_1");
    unsigned int* app_code_2_start = __section_begin("APP_CODE_SECTION_2");
    unsigned int* app_code_2_end   = __section_end("APP_CODE_SECTION_2");

    unsigned int length, src, dst, heap1Start/*, heap2Start*/;
    int heap1Length/*, heap2Length*/;

codeAddrArray[0] = app_code_1_start ;
codeAddrArray[1] = app_code_1_end ;
codeAddrArray[2] = app_code_2_start ;
codeAddrArray[3] = app_code_2_end ;
	...
	heap2Length = 0x2008FFFF - heap2Start;*/
...
#else
	pGsnBytePoolHead = &bytePool[0];
	pGsnBytePoolHead->pNext = NULL;
#endif
...
#define SRAM_END_ADDR				0x2007FFFF /* 512 + 64 KB*/

#define FPT_LOCATION 				0x20004000 /* 0x20004000 Location in RAM where the ROM
                                             	 function pointers has to be copied */
#define APP_CODE_SRAM_BLOCK_1_ADDR	0x20030000 /* 192 KB */
#define APP_CODE_SRAM_BLOCK_1_SIZE	0x10000 /* 16KB */

#define APP_CODE_SRAM_BLOCK_2_ADDR	0x20040000 /* at 256KB */
#define APP_CODE_SRAM_BLOCK_2_SIZE	0x40000 /* 128 */
...
VOID App_CodePostFetch();
/*****************************************************************************
 *Function definitions
 ****************************************************************************/
#pragma location = "StartupSection1"
...
void GsnStartup_Main( void )
{
    ...
    UINT32 *appCode1Start = __section_begin("APP_CODE_SECTION_1");
	UINT32 *appCode1End = __section_end("APP_CODE_SECTION_1");
    UINT32 *appCode2Start = __section_begin("APP_CODE_SECTION_2");
	UINT32 *appCode2End = __section_end("APP_CODE_SECTION_2");
    UINT32 totalCode;
    ...
    totalCode = ((UINT32)appCode1End - (UINT32)appCode1Start) + ((UINT32)dataInitStart - (UINT32)appCode2Start);
	...
	/*Copy App code*/
	sectionSize = (UINT32)appCode1End - (UINT32)appCode1Start;

	sectionFlshAddr = appFwImageFlashAddr + CB1_RESERVED_SIZE +
						((UINT32)autoPatchEnd - (UINT32)autoPatchStart ) +
						FPT_RESERVED_SIZE + GSN_STARTUP_RESERVED_SIZE;

	APP_STARTUP_COPY_PROG((UINT32)APP_CODE_SRAM_BLOCK_1_ADDR, sectionFlshAddr, sectionSize);
	/*remap*/
	remap.codeAddr = (UINT32)appCode1Start + 1;
    remap.remapIndex = remapIndx++;
    remap.remapSize = APP_CODE_SRAM_BLOCK_1_SIZE;
    remap.sramAddr = APP_CODE_SRAM_BLOCK_1_ADDR;
    ...
    heap[0].size = APP_CODE_SRAM_BLOCK_1_ADDR - heap[0].start;
    tx_byte_pool_create(&heapBytePool[0].bPool, NULL, (void*)heap[0].start,
    					(UINT32)heap[0].size);
    					
	// allocating second heap memory
    heap[1].start = APP_CODE_SRAM_BLOCK_2_ADDR +
            ((UINT32)appCode2End - (UINT32)appCode2Start);
    heap[1].start = ((heap[1].start & ~3 ) + 8 );/* World Align the start address*/
    heap[1].size = SRAM_END_ADDR - heap[1].start;

    tx_byte_pool_create(&heapBytePool[1].bPool, NULL, (void*)heap[1].start,
                                                                    (UINT32)heap[1].size);

    pGsnBytePoolHead =  &heapBytePool[0];
    pGsnBytePoolHead->pNext = &heapBytePool[1];
    heapBytePool[1].pNext = NULL;
...
#if 0
#pragma location = "StartupSection2"
PUBLIC UINT
...
#pragma location = "StartupSection3"
BOOL
App_FlashIfFlashCheckAndConfigure()
{
...
VOID App_CodePostFetch()
{
	...
	/*Copy App code*/
	sectionSize = (UINT32)appCode2End - (UINT32)appCode2Start;
	sectionFlshAddr = appFwImageFlashAddr + CB1_RESERVED_SIZE +
						((UINT32)autoPatchEnd - (UINT32)autoPatchStart ) +
						FPT_RESERVED_SIZE + GSN_STARTUP_RESERVED_SIZE +
                         ((UINT32)appCode1End - (UINT32)appCode1Start) ;

	APP_STARTUP_COPY_PROG((UINT32)APP_CODE_SRAM_BLOCK_2_ADDR, sectionFlshAddr, sectionSize);
	/*remap*/
	remap.codeAddr = (UINT32)appCode2Start + 1;
    remap.remapIndex = 2;
    remap.remapSize = APP_CODE_SRAM_BLOCK_2_SIZE;
    remap.sramAddr = APP_CODE_SRAM_BLOCK_2_ADDR;
    GsnFlashCache_BlkRemapSet(&remap);
